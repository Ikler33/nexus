...
| **DP-2** | сайдбар: icon-rail (files/search/tags/starred), side-nav (Home/Новая заметка), звёзды в дереве, tags-панель (+команда `list_tags`), starred (localStorage v1), search-панель | DP-1 (side-nav) | ✅ (этот PR) |
| **DP-3** | редактор: floating-табы на chrome-фоне (m-tabin, dirty/close, tab-add), **DnD табов между панами** (`text/nexus-tab`, drop-target), tab-tools sticky, **mode-float пилюля** (⌘E, иконка-действие), doc-meta чипы, md-fidelity (буллеты-акцент, quote, inline-code, wikilink-скобки, нумерация), reading-mode обработка табов, backlinks-bar collapsible twist | DP-0 | ✅ (этот PR) |
| **DP-4** | титлбар: **AI-dropdown** (Дайджест/Цели/Противоречия под sparkles — разгрузка бара), кнопка reading-mode, порядок по макету; статусбар: sync-дот, **прогресс-бар индексации**, conflict-пилюля, Local/UTF-8/Markdown; theme store: 4 темы + цикл | DP-0 | ✅ (этот PR) |
...
| **DP-12** | AI-панель по `ai-panel.jsx`: шапка ai-head + бейдж провайдера (E9: Локально/Офлайн; Облако — срез 3), layouts side/bottom/overlay (prefs `aiLayout`), RAG-стили cards/chips/footnotes (prefs `ragSources`), reasoning-индикатор из #97 (BrandThinking + стрим сводки), empty-state с пилюлями, composer-foot, i18n `EgressDenied`-баннер (**AC-EGR-14**, `denied_kind` в Error-событии). #97 закрыт, код пересобран отсюда (решение владельца 2026-06-11). Summary-таб макета не перенесён (суммаризация в inline-LLM) — BACKLOG | #97 закрыт | ✅ |
| **DP-13** | хром по `app.jsx`: вертикальный **ActivityBar** (Home/Новости/Файлы-тогл/Граф + Sync/Настройки снизу, акцент-планка активной), титлбар разгружен (бренд «Nexus»→Home, плейсхолдер «Поиск файлов и команд…», справа только AI-меню|reading|RU/EN|тема|panel-right); плагины и «Открыть vault» — только из палитры; +команда «Свернуть боковую панель» | выявлено владельцем после «приёмки» | ✅ |
| **DP-14** | статусбар по макету: слева «Синхронизировано»/«Изменения · N» + прогресс/«Проиндексировано · N» (`notes_count`), справа конфликт-пилюля → resolver (стор sync), локально/UTF-8/Markdown; путь vault — в тултип, лейбл темы убран | DP-13 | ✅ |
| **DP-15** | `.md`-стрип в табах/дереве/беклинках (title-first), ★-бейджи избранного (с DP-2, подтверждены), заголовок секции «ФАЙЛЫ +», doc-meta дополнена clock-чипом (`file_mtime`) | DP-13 | ✅ |
...
**Проход 2026-06-10** (после хвостов: граф-теги #136, реиндекс #137, frontmatter-теги,
GC кэша): экраны-вью и токены 4 тем сверены (визуально 2+ темы на экран, токены программно) —
по вью расхождений нет; осознанные отличия — honest-адаптации в BACKLOG (маркетплейс плагинов
без данных; метрики истории слов; фильтр тег-чипа гасит узлы, не вырезает).
**⚠️ Проход пропустил ХРОМ приложения** (выявлено владельцем по скриншот-паре): нет вертикального
activity-bar, титлбар перегружен против макета, статусбар без synced/индексации и конфликт-пилюли,
`.md`-расширения в табах/дереве, нет ★-бейджей и заголовка «ФАЙЛЫ». Урок: финальная приёмка обязана
сверять и app-shell (`app.jsx` ActivityBar/Titlebar/StatusBar), не только вью. Закрытие — DP-13..15
(таблица выше). Не покрыт: DP-12 (AI-панель) — blocked #97.
