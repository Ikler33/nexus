...
игнорирует mtime-шорткат и переиндексирует всё. Та же модель/dim — СТРОГИЙ no-op (пользовательские
индексы не пересобираются на ровном месте). `dim` берётся из конфига или пробным эмбеддингом
(`OpenAiEmbedder::probe_dim`) — НЕ хардкод.
