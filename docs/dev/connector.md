...
frontend + `scripts/test-all.sh`): кладёт ВРЕМЕННЫЙ ядровой файл с запрещённым импортом → eslint ОБЯЗАН
упасть (exit≠0). Если бы правило не работало — красный CI. Плюс позитив-контроль: реальные манифесты
(легитимные self-импорты) остаются зелёными (нет ложных срабатываний).
