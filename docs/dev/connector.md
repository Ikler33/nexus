...
### F-9b: i18n per-module namespace — рассмотрено, ОТЛОЖЕНО (analysis-driven, нулевой дифф кода)

F-9 оставил i18n-ключи модулей в монолитных `i18n/{ru,en}.json` (хвост «i18n-EP»). F-9b провёл
разведку и **сознательно отложил** физический вынос — по фактам, а не по инерции. Нулевой дифф кода;
процесс переводов не тронут. Причины (числа — по `ru.json`, всего 1296 листьев):

1. **North-star (сторонние плагины) уже имеет СВОЙ путь i18n — рантайм-namespace `plugin`**
   (`ui.addTranslations` → `plugin:<dir>:<key>`, AC-I18N-7, live-проверено — см. `i18n.md`). Монолит
   `ru/en.json` — это **first-party/ядровые** строки. Дробление first-party-переводов по модулям НЕ
   двигает «модуль самодостаточен для third-party» (у third-party путь ДРУГОЙ и уже есть) — оно лишь
   переставляет, где лежат ЯДРОВЫЕ строки. Главный аргумент «за» вынос — отпадает.
2. **Ключи модулей НЕ образуют чистого разбиения JSON** — они переплетены с ядром:
   - **shared-объекты:** `commands` (плоские точечные ключи: 12 модульных `view.<mod>` вперемешку с 9
     ядровыми) и `settings` (подмодуль `settings.news`=модуль, НО `settings.agent` (28) = ЯДРО — общая
     ai-секция `SettingsView`, у agent-модуля секции настроек нет; ловушка «settings-секции модулей?!»);
     `chrome.aiPanel` (titleKey панели chat) — в ядровом `chrome`.
   - **shared-namespace, читаемые И ядром И модулем:** `conflict` (29) + `git` (27) — ядровой
     `ConflictResolver` (вынесен в `components/common`, F-10c) И `sync`/`SyncPanel`; `prop` (4) — board.
   - **«чистые» namespace тоже протекают в ЯДРО:** `commands-core.ts` (ядро) читает `chat.memorySaved`/
     `chat.memorySaveFailed` + `board.promote.*` (7 листьев) → удаление файла модуля chat/board
     **сломало бы ядро**. «Delete module → строки уходят чисто» — ложно для chat/board.
   - **`sync` не владеет ПОЧТИ НИЧЕМ:** его единственный собственный лист — заголовок команды
     `commands.view.sync`; все UI-строки (conflict/git) — общие с ядром. Любая схема per-module-файлов
     сразу имеет sync как исключение.
3. **Цена миграции реальна, выигрыш маргинален:** нужен deep-merge в `setup.ts` + сборка ~13 файлов-
   фрагментов; `settings`/`commands` разъезжаются по файлам (переводчик теряет «все заголовки команд /
   все настройки в одном месте» — ровно то, что взвешивает вариант (в)); 2 задокументированных
   исключения (sync, `settings.agent`).
4. **Инвариант «байт-идентичный bundle» серии НЕ достигается наивным сплитом.** Характеризационная
   проверка (декомпозиция → deep-merge обратно, обе локали) даёт **content-identical=True**, но
   **byte-identical=False** (вырезание+вклейка переупорядочивают ключи). Байт-идентичность потребовала
   бы order-preserving-машинерии — ещё цена. (Разбивка: ~535/1296 ru-ключей ушли бы в фрагменты, 761
   осталось ядром; sync-фрагмент = 1 ключ.)

Итог совпадает с духом коннектора (везде «оставить ядром с обоснованием» вместо преждевременной
машинерии: titlebar-menu-реестр, store-agnostic-оверлей, apiVersion/capabilities). **Триггер пересмотра:**
появление СТОРОННЕГО модуля, которому монолит не подходит (у него уже есть `plugin`-namespace) ЛИБО
разрастание монолита до неуправляемого (сейчас 1296 листьев, читаемо). Запись — `BACKLOG.md`, `i18n.md`.
...
## F-8b — миграция фича-эффектов App.tsx на `ctx.events` (ЗАКРЫТО)
Хвост коннектора: перевод оставшихся фича-`useEffect` App.tsx на подписки модулей через `ctx.events`.
Делался поэтапно вместе с вырезом фич:
- **F-10b** перевёл оверлей-эффекты: goals-reload (`vault:changed`) → `modules/goals.ts`;
  digest/contradictions-refetch (`jobs:changed`, combined-эффект расщеплён по фичам) →
  `modules/{digest,contradictions}.ts`.
- **F-8b** закрыл остаток — **episodic-sync** (`episode.getEnabled` → фронт-pref `aiEpisodicMemory` при
  смене vault) переехал в `modules/episodes.ts` через `ctx.events.on('vault:opened')` (+`ctx.api`): фича
  «Эпизоды» вырезана (F-10b), эффект уехал к своему владельцу-модулю. `vault:opened` = window-событие
  `vault:switched` (`stores/vault.openVault`), диспатчится УЖЕ ПОСЛЕ переключения бэка, до `set({info})`.
  **Тайминг-сдвиг (документируется):** прежний App-эффект `[vaultRoot]` вешался в `useEffect` ПОСЛЕ
  маунта тела App (после `set({info})` и завершения онбординга); подписка модуля активна с импорта и
  ловит `vault:opened` РАНЬШЕ (на `openVault`, до `set`). Это **behavior-preserving**: фетч идёт в бэк,
  уже переключённый на новый vault (не зависит от фронт-root), а pref — чисто фронтовый; при онбординге
  pref не читается до завершения flow, к тому времени sync завершён. Идемпотентность фетча уже
  предполагалась (StrictMode dev двоил прежний эффект). Мелкое отличие дедупа (React deps не ре-фетчил
  при повторном open ТОГО ЖЕ root; событие — ре-фетчит) benign: `getEnabled` идемпотентен.

**Осознанно ОСТАВЛЕНО ядром (не F-8b-цель):**
- **`chat.hydrate(vaultRoot)`** (история чата #17 по смене vault) — НЕ в списке отложенного F-8b; ядро-
  эффект жизненного цикла vault, которому НУЖЕН новый root аргументом. `vault:opened` — window-событие
  БЕЗ payload и диспатчится ДО `set({info})`, поэтому подписчик прочитал бы `getState().info?.root` =
  СТАРЫЙ root → миграция сломала бы `hydrate`. Расширять обёртку payload'ом = строить новую шину (против
  «events — обёртка, не шина», YAGNI). Остаётся ядром.
- **`aiFeatures.sync()`** (тогглы «Инсайты»=Home + «Поиск противоречий» по смене vault) — общий кросс-
  фичный тоггл-стор с АТОМАРНЫМ dual-fetch под общим seq-гардом и БЕЗ единого владельца-модуля (Home не
  модуль коннектора). Расщепить его на два модуля = сломать seq-координацию/атомарность; целиком отдать
  одному модулю неверно (спанит две фичи). F-8b мигрирует эффект фичи с ЕДИНЫМ вырезанным владельцем →
  aiFeatures остаётся ядром (кросс-фичный chrome ai-секции настроек).
...
- **i18n-ключи news** (namespace `news.*` ~100 + `settings.news.*` 23 + `commands.view.news`) живут в
  монолитных `i18n/ru.json`/`en.json`. Их вынос в per-module namespace — отдельная фича (i18n-EP), НЕ
  часть вырезания. news-компоненты читают их через `t('news.*')` без изменений. → **F-9b рассмотрел
  вынос и ОТЛОЖИЛ по анализу** (см. «### F-9b: i18n per-module namespace — рассмотрено, отложено» ниже).
