...
## Приватность логов / crash-reporter (V4.2, AC-SEC-6)
- **`Redacted<T>`** (`src/redact.rs`): обёртка с безопасными `Debug`/`Display` — печатают `<redacted>`,
  значение достаётся только явно через `expose()` (каждое раскрытие видно на ревью). Конвенция:
  оборачивать пользовательский контент/секрет, если он может попасть в лог/`{:?}`/payload ошибки.
- **Crash-scrub** (`src/crash.rs`): panic-hook → `~/.nexus/crashes/crash-*.log`, **локально, без сети**
  (отправка opt-in, BACKLOG). `scrub`: HOME→`~` + абсолютные пути вне дома → `<path>/<basename>`
  (структура каталогов скрыта, имя файла оставлено); относительные (`src/x.rs:10`) и `~/…` не трогаются.
- **Аудит tracing:** ядро НЕ логирует контент заметок (проверено) → `Redacted` сейчас страховка от
  регрессий + инструмент для будущих web/import/agent-memory-фич, где контент пойдёт через логи.
  Широкое оборачивание Debug-полей структур — инкрементально (BACKLOG, нет силент-капа).
- **Чеклист ревью (кросс-план #33).** При добавлении кода проверять:
  1. Новое `#[derive(Debug)]`-поле, способное нести **контент заметки / путь vault / секрет**, который
     может попасть в `tracing::*`/`{:?}`/payload ошибки → обернуть в `Redacted<T>` (или не выводить).
  2. Новый сетевой эгресс ядра → только через будущий `net::guarded_client` (#16), не голый `reqwest`.
  3. Новый `panic!`/`expect` с пользовательскими данными в сообщении → данные скрабятся (`crash::scrub`)
     или не включаются. CI-линт автодетекта «секрет-полей» — фуззи, отложен (BACKLOG); пока — ревью.
