...
| **4. Web-агент / News Feed** | `EgressFeature::{Web,NewsFeed}`; SearXNG-host (consent-on-save, W2); лимиты W3 (≤3 поиска/ход, body-cap, таймауты connect/read/total, News Feed раз/сутки); outbound `scan_secrets` (W4); DNS-rebinding-гард для доменов; untrusted-канал web-контента (anti-injection, tool-use заблокирован) | ✅ **News-Feed-половина ✅** (NF-4, 2026-06-10, спека `docs/specs/news-feed.md`): `EgressFeature::NewsFeed` (дефолт ВЫКЛ, consent = `news.json`; `allow_private=false`), "news"-скоуп allowlist, лимиты: connect 10 с/read 20 с/total 120 с + body-cap 4 МБ (2026-06-11: единый 20 с рвал медленные фиды «error decoding», Substack >2 МБ), **DNS-гард с пином проверенного IP** (resolve-then-connect-check без TOCTOU), anti-injection-маркеры на фид-контент, W4 n/a (GET без тел). **Web-половина ✅** (W-1..W-4, PR #149/#152, 2026-06-11): `EgressFeature::Web` (web-класс, дефолт ВЫКЛ, consent = непустой URL в `websearch.json` + тоггл), "web"-скоуп allowlist, W3-лимиты (≤3 поиска/ход, 2 МБ, 20 с), **W4 outbound `scan_secrets` на поисковый запрос** (секрет → запрос не уходит), DNS-гард с пином IP, режим «Web» в чате (план→поиск→ответ с источниками-маркерами, tool-use запрещён); SearXNG поднят на VPS владельца (HTTP :8888; HTTPS в бэклоге до отдельного VPS). Live-smoke этапов — `live_smoke.rs` (`cargo test live_ -- --ignored`) |
