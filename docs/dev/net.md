...
| **2. UI/контроль** | тоггл «офлайн» + per-feature opt-in в настройках; чат-бейдж local/cloud/offline (E9); i18n-рендер `EgressDenied` (RU/EN, `AC-EGR-14`); персист политики в OS config-dir (E5) | 🚧 частично (2026-06-10): **бэкенд+настройки ✅** — персист E5 (`egress.json` в OS config-dir, fail-safe-дефолты), команды `get_egress_state`/`set_egress_offline`/`set_egress_feature`, блок «Сеть (egress)» в «AI / Модели» (применяется мгновенно). **Остаётся:** чат-бейдж E9 + i18n-рендер отказов (AC-EGR-14) — после мержа PR #97 (дизайн-ветка держит чат-файлы) |
