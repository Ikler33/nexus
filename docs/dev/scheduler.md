...
| **6. расписание (recurring) + дедуп** | авто-переназначение LLM-kind раз в сутки (`reschedule_if_absent` + `recurring`-карта в `run_due`/`spawn_worker`); дедуп ручного запуска (`has_ready_job`) | ✅ |
| **7. on-change-триггер (S4)** | перезапуск LLM-kind после правок vault: воркер опрашивает `max_file_mtime` → дебаунс-детектор `onchange_step` (120с тишины) → `enqueue` (дедуп `has_ready_job`) | ✅ |

**Триггеры закрыты:** manual (команды) · run-if-overdue (seed на открытии, S2) · recurring (24ч, slice 6) · on-change (S4, slice 7).
...
- **Детали dead (S7 «разбираемость», 2026-06-11).** «⚠ N» — кнопка → модалка `DeadJobsModal`: на каждую
  dead-джобу — человеческое имя kind, `last_error`, попытки, давность; «Повторить» (`retry_dead`:
  `dead → pending`, `attempts=0` — жмут после исправления причины, напр. URL модели) и «Очистить все»
  (`clear_dead`). Команды `get_dead_jobs` / `retry_dead_job` / `clear_dead_jobs` (без vault — пусто/no-op).
- **Тесты:** `run_due_defers_llm_job_under_interactive` (busy → отложена/не выполнена; !busy → выполнена);
  `counts_reports_states`; `dead_jobs_listed_retried_and_cleared` (список/retry с чистыми attempts/clear);
  фронт `StatusBar.test.tsx` (индикатор по данным / пусто → скрыт; модалка: детали, retry, очистка).
...
## Расписание (recurring) + дедуп (slice 6)
- **`reschedule_if_absent(writer, kind, run_at, max_attempts)`** — ставит следующий запуск `kind`, ТОЛЬКО
  если нет уже ожидающей (`pending`) джобы этого kind (анти-стакинг: ≤1 будущая периодическая).
- **`run_due(.., recurring: &HashMap<kind, интервал>)`** — после успешного `complete` переназначает
  recurring-kind на `now + интервал`. `spawn_worker(.., recurring)` пробрасывает карту; `open_vault`
  наполняет её для зарегистрированных LLM-kind (дайджест/противоречия → 24ч). Так фоновые фичи
  «живут» сами, пока открыт vault; backpressure (S5) держит их позади интерактива.
- **`has_ready_job(reader, kind, now)`** (бывш. `has_active`) — `running` ИЛИ `pending` с `run_at<=now`.
  Будущая периодическая (`run_at>now`) НЕ блокирует ручной запуск → команды `generate_digest`/
  `generate_contradictions` дедупят повторный клик, но «Найти/Сгенерировать сейчас» всегда срабатывает.
- **Тесты:** `run_due_reschedules_recurring_kind`, `reschedule_if_absent_is_idempotent`,
  `has_ready_job_ignores_future`.
- **Дальше (slice 7):** on-change-триггер (S4) — перезапуск по правкам vault с дебаунсом. ✅ (ниже).

## On-change-триггер (slice 7, S4)
- **`max_file_mtime(reader)`** — макс. `updated_at` не-удалённых заметок (0 — пусто): «когда vault менялся».
- **`onchange_step(state, mtime, now) -> (state, fire)`** — чистый дебаунс-детектор (без часов внутри →
  детерминированный тест): новый `mtime` взводит всплеск; после `ONCHANGE_DEBOUNCE_SECS=120` тишины
  → `fire=true`. Первый замер — база (не правка); правка в окне дебаунса пере-взводит таймер.
- **Воркер** (`spawn_worker(.., reader, on_change)`): каждый тик опрашивает `max_file_mtime`, гоняет
  `onchange_step`; на `fire` — для каждого on-change-kind `enqueue(now)` **если** `!has_ready_job`
  (поверх будущего recurring, без дублей). `open_vault` передаёт reader + те же LLM-kind, что recurring.
- Опрос — не событие `vault:changed`, а дешёвый поллинг mtime в уже existing-тике (без event-пламбинга
  Rust↔Tauri и связности с индексатором).
- **Тесты:** `onchange_step_debounces_then_fires_once`, `onchange_step_rearms_on_new_edit`,
  `max_file_mtime_empty_is_zero`.
