...
| 2. Воркер-луп | tokio-interval-движок (S1), приоритет чата (S5), event-канал статусов | ⏳ |
| 3. Триггеры | on-open / on-change (от завершения реиндекса, S4) / scheduled (run-if-overdue, S2) | ⏳ |
| 4. Первые kind | Карта компетенций, Поиск противоречий (локальные, несетевые — S3) | ⏳ |
| 5. UI | StatusBar N/M поверх event-канала, видимый `dead`/pending (S7/S8) | ⏳ |
...
`claim_respects_run_at_and_completes` (future-джоба не берётся; running не переклеймится; complete→done);
`fail_retries_with_backoff_then_dead` (backoff → готова после задержки → исчерпание → dead);
`requeue_and_gc` (running→pending; GC завершённых). Все офлайн на temp-БД (без сети/LLM).
