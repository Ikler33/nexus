...
| 4b. UI дайджеста + backpressure + расписание | панель дайджеста (refetch по `jobs:changed`); приоритет чата над LLM-джобами (S5); дедуп kind в очереди, on-change (S4) | ⏳ |
...
- **Дальше:** slice 4b — UI-панель (последний дайджест + «сгенерировать сейчас», refetch по
  `jobs:changed`); backpressure чата (S5); дедуп одинаковых `kind` в очереди; on-change-триггер (S4).
