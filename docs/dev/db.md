...
**v3** (`003_frontmatter_fields.sql`, typed-frontmatter): `frontmatter_fields` (file_id, key, value;
`UNIQUE(file_id,key)` + `idx_frontmatter_fields_file/_key`) — плоские скаляры frontmatter для
кросс-файловых запросов (цели/stale-radar/Dataview). Наполняет индексатор (полная замена на файл).

### Примитив пересборки производного FTS (#13, ADR-007/§5.1)
`Migration` имеет флаг **`rebuild_fts: bool`**: миграция, инвалидирующая FTS5 (смена схемы/конфига
`fts_chunks`), ставит его → раннер после её SQL вызывает **`rebuild_fts(conn)`** (`INSERT INTO
fts_chunks(fts_chunks) VALUES('rebuild')` — встроенная пересборка external-content из `chunks`, без
переразбора файлов). Это **примитив резюмируемости**: будущие схемо-миграции (`#14` re-chunk, jobs
планировщика и т.п.) не заставляют пользователя сносить `.nexus`. **usearch** (смена размерности
эмбеддинга) этим хуком НЕ покрыт — для него нужен embedder; пересборка идёт реконсайлом индексатора на
открытии vault (`reconcile_vectors` в `scan_vault`). Тест: `rebuild_fts_restores_index_from_chunks`
(рассинхрон `delete-all` → `rebuild` → FTS снова находит чанк).
