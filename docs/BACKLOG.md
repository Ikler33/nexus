...
| ⏳ **auto-updater** (Tauri updater plugin) → **Фаза 5** | **эндпоинт решён** (VPS владельца — свой хостинг `latest.json` + артефактов, не GitHub Releases). Остаётся: Tauri-keypair (подпись манифеста) + pubkey в конфиге + CI-шаг подписи; OS-подпись (Apple notarize / Windows Authenticode) — отдельно | VPS есть → разблокировано | §12-Ф4, Tauri updater |
