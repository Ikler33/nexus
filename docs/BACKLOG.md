...
| ⏳ **Reconcile usearch после краха** (chunks в БД есть, вектора нет — крах между commit и `save`) | вне атомарности SQLite↔usearch; нужен diff-проход | конец Ф1 / Ф3-надёжность | §5.1, `indexer.md`, `vector.md` |
...
| Что | Почему отложено | Триггер | Источник |
|---|---|---|---|
| 🔬 **Eval-харнесс RAG** (recall@k / nDCG / MRR, golden-набор, регрессия) — гейт для всех изменений ранжирования | сам по себе срез **Ф1-10** | следующий по плану | §6/§12, AC-EVAL-*, REVIEW |
