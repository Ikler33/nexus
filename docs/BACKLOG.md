...
|---|---|---|
| **Вылет приложения при открытии графа на реальном vault** (Tauri-сборка; добавили свои папки с `.md` → открыли граф → мгновенный вылет). На мок-vault (браузер) не воспроизводится. | Исключено: паник в Rust-коде графа нет (только в тестах); раскладка NaN-безопасна (старт по кругу, 1-узловой граф пропускает FA2); OS-крэш-репорта (.ips) на момент запуска нет → вероятно **Rust-паника** (печатается в терминал `pnpm app:dev`) ИЛИ blank webview. **Нужен**: вывод терминала (`thread … panicked at src/…:LINE`) + ответы: процесс упал целиком или закрылось окно; режим Локальный/Весь vault; есть ли `.nexus/local.json` (могла параллельно упасть индексация новых файлов). Гипотеза: граничный случай на реальных данных (напр. лимит SQLite-переменных при супер-хабе, либо oklch в canvas WKWebView, либо индексация). | 🔴 отложен на фазу тестирования |
...
| ⏳ **Home-дашборд** (greeting + виджеты: daily-brief / recent / goals / stats / stale-radar / open-questions / focus-drift) | **концепция ещё не доработана** (решение владельца, июнь 2026): дизайн в хендоффе есть, но содержание/поведение виджетов финализируется отдельно | дозреет концепция | DESIGN, `docs/design/handoff/Nexus Home.html`, §12-Ф4 |
| ⏳ **auto-updater** (Tauri updater plugin) | нужна **подпись релизов** (приватный ключ) + pubkey в конфиге + release-эндпоинт/GitHub Releases с подписанными артефактами — это работа времени релиза, не «вслепую» | подготовка релиза | §12-Ф4, Tauri updater |
