...
| 🔬 **Нагрузочный тест 50k** (синтетический vault → замер индексации/поиска/старта, проверка целей AC-PERF) | структурная база перфа **готова** (виртуальное дерево, граф-layout в Worker, ленивая загрузка, батч-эмбеддинг); нужен замер на объёме + профиль узких мест | объёмный vault / профилирование | AC-DOD-Ф3, AC-PERF-* |
| 🔬 **Старт UI < 1 с** (индекс/граф фоном) — замер и доводка холодного старта на 50k | UI и так не ждёт индекс (фон); цель <1s нужно подтвердить замером | после нагрузочного теста 50k | AC-PERF-1 |
