...
6. **`SO_PEERCRED`/uid-check на proxy-listener:** только спавненный контейнер (его uid) может драйвить `egress/*` и `host/*`. Сокет в per-run-каталоге, 0600, teardown на `--rm`.
...
- **`sandbox_run_plan(cfg)` → argv-вектор `podman run`** (клон стиля `docker_plan`-тестов в `service.rs`). Ассертим, что argv содержит: `--network=none`, `--read-only`, `--tmpfs /tmp`, `--cap-drop=ALL`, `--security-opt=no-new-privileges`, `--pids-limit`, `--memory`/`--cpus`-кэпы, `--userns=keep-id`, vault `:ro` bind, proxy-socket bind. **И ассертим, что proxy-socket bind — DISTINCT mount, НЕ под vault-путём** (§4.4). **И ассертим `--env`-набор = только allow-list, без хост-секретов** (§5.4). NB: render-тест проверяет НАМЕРЕНИЕ (argv корректен), не runtime-enforcement флага — последнее только в Tier-2.
