...
- **Лимиты W3:** connect 10 с + read-inactivity 20 с + общий потолок 120 с/запрос (единый 20 с убивал медленные здоровые фиды — 2026-06-11), body-cap 4 МБ (Substack с полными текстами легитимно >2 МБ) (превышение → источник пропущен, видимая
...
- **AC-NF-7 (эгресс).** Все запросы фидов — через `GuardedClient` с `EgressFeature::NewsFeed`;
  ...
  allowlist; таймауты (connect/read/total) и body-cap 4 МБ энфорсятся (превышение — видимый пропуск источника).
...
| **NF-6** | reader: клик по заголовку → полный RU-перевод статьи in-app + кнопка «Сократить» (тезисы on-demand); фетч оригинала через guarded-фетчер NF-4 (fail-closed: хост вне news-allowlist → резюме + ссылка «Оригинал» + **per-host consent** — ревизия владельца 2026-06-11: кнопка «Разрешить <хост>» добавляет ПУБЛИЧНЫЙ хост в `news.json::extra_hosts` (вне vault/git), снимается из gear-меню; приватные/LAN отвергаются и так и так — defense-in-depth с политикой web-класса + DNS-гардом); кэш тела (миграция 011) | NF-4/5; добавлен владельцем на дизайн-итерации (не в брифе v1) | temp-DB + мок-чат юниты; превью; live-smoke перевода — с сервером владельца |
