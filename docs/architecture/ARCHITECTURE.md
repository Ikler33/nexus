...
**Миграции.** Версионированные SQL-файлы в `db/migrations/`; раннер на старте применяет недостающие в транзакции, версия — в `settings('schema.version')`.
- Обычные таблицы — `ALTER`/`CREATE`.
- `fts_chunks` (FTS5) и usearch **нельзя `ALTER`** — пересоздаются и переиндексируются из `chunks` (контент-таблица = источник). Фоновая операция с прогрессом.
