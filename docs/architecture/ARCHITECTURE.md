...
> ⚠️ **Статус реализации (фактчек кросс-плана; уточнено 2026-06-10 под egress-фундамент ADR-005-ext).**
...
> `EgressFeature::CloudFallback`), не сейчас. **Что реально вводит egress-фундамент (E1–E10):** тонкий
> фасад `AIClient { chat: Arc<dyn ChatProvider>, embedder: Arc<dyn EmbeddingProvider>, policy:
> Arc<EgressPolicy> }` — заменяет два независимых `Arc` в `VaultContext` одним полем (composition-root в
> `build_rag`/`build_chat`), БЕЗ `cloud_fallback`. Весь исходящий HTTP провайдеров идёт через
