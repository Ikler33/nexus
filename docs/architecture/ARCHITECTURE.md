...
**Egress-граница ядра (расширение ADR-005, принято 2026-06-05; codesign `docs/reviews/ADR_CODESIGN.md`, решения владельца E1–E10).** Весь исходящий HTTP ядра (chat/embed/probe + будущие web/cloud) проходит через единый chokepoint `net::GuardedClient` (оборачивает `core_client_builder`, redirect=none): глобальный kill-switch «офлайн» рубит **облако/web**, локальный LAN-LLM работает (E2) → **per-feature** opt-in Chat/Embed/Probe (probe к loopback/LAN без consent, к публичному — с consent; E6) → host-allowlist (хосты из `local.json ai.chat/ai.embedding` авто-в-allowlist; смена URL из git-pull → подтверждение; E4) → неотключаемый **in-memory** append-only audit host/bytes/feature (E8). §4.3-фантом `AIClient` чинится тем же срезом — тонкий фасад-потребитель GuardedClient (egress = расширение ADR-005, не новый ADR-007; E1). Egress по умолчанию **OFF** — облако не из коробки (E4). Политика — **app-local файл OS config-dir**, вне vault/git (E5). Добавляется предикат блокировки **metadata** `169.254.169.254`, отдельный от `is_private_host` (E7). kill-switch рвёт активный стрим сразу через `chat_cancel`; Denied → i18n-ошибка после #9 (E10). Индикация — переиспользовать чат-бейдж local/cloud/offline, экран «журнал сети» не в v1 (E9). Конфликт с AC-SEC-4 разрешён уточнением формулировки (E3, см. ACCEPTANCE §AC-SEC-4). **Реализация — после #5 (док-фикс §4.3) и #9 (типизированные ошибки).** Срезы и API фундамента — `docs/dev/net.md`; критерии — `AC-EGR-1..14`.

**Web-эгресс (второе расширение ADR-005, решения владельца W1–W4, принято 2026-06-10).** Сетевой класс фич — **web-агент** в чате (3-й режим «Web»: LLM решает «нужен интернет» → поиск через self-hosted **SearXNG** → ответ с цитатами-источниками), **News Feed** (RSS/открытые API → 2-этапный фильтр keyword→LLM → связь с vault, scheduled раз/сутки) и **cloud-fallback** chat — строится ПОВЕРХ egress-фундамента (E1–E10) как НОВЫЕ варианты `EgressFeature::{Web, NewsFeed, CloudFallback}`, добавляемые **вместе с фичей** (W1: форма — второе расширение ADR-005, не отдельный ADR; продолжает прецедент E1). **Хост SearXNG** (self-hosted VPS, публичный домен): сохранение URL в настройках = **явный consent** (warning-баннер «запросы уйдут на этот хост») → далее авто-в-allowlist (как E4 для `ai.*`); публичный хост без сохранённого URL → `Denied::HostNotAllowed` (W2). **Лимиты v1 — жёсткие именованные константы** (не UI-настройки): **≤3 web-поиска на один чат-ход** (анти-runaway-агент), **News Feed раз/сутки** + ручная кнопка «обновить», **cap тела ответа ~2 MB**, **таймаут 20 с** (W3). **Исходящие тела** (web-поисковый запрос, cloud-chat промпт+контекст vault) **сканируются `scan_secrets`** (AC-SEC-3) ДО отправки — найден секрет → запрос НЕ уходит, структурированная ошибка (W4; перенос git-сканера секретов на egress-payload). Web-feature ставит `allow_private=false` (web-хосты только публичные, LAN-исключение E2/E6 на них НЕ распространяется) и **обязывает DNS-rebinding-гард** (резолв-then-connect-check) — SearXNG-домен = первый ДОМЕН в allowlist (для chat/embed-LAN это было отложено, для web — обязательно). News Feed как сетевой scheduled-kind разблокируется ТОЛЬКО при egress (этот ADR) **И** планировщике (ADR-007, готов) одновременно; **tool-use в web-агенте запрещён** до закрытия untrusted-канала web-контента (indirect injection из web/News Feed может управлять `net.fetch`/`vault.write`). **Каскад.** §4.3, §11, `AC-EGR-*` (+ web-AC вместе с фичей), `docs/dev/net.md`.
...
### ADR-007 · Планировщик фоновых задач (принято 2026-06-05; codesign, решения владельца S1–S10)
**Решение.** Persistent-очередь джобов в SQLite (миграция `jobs`: kind/payload/state/run_at/attempts) + воркер-луп. Движок scheduled-тиков — **`tokio::interval`, пока vault открыт** (не будит закрытое приложение; single-binary, кроссплатформенно, local-first; S1); пропущенные прогоны — **run-if-overdue** (один прогон, без реплея N; S2). Триггеры on-open/scheduled/manual + **on-change от завершения реиндекса**, не от сырого VaultEvent (S4). Первая волна kind — **локальные несетевые** (Карта компетенций, Поиск противоречий); News Feed и сетевой класс — после egress (ADR-005-ext; S3). Backpressure — **жёсткий приоритет чата**: фон ждёт интерактивный стрим (S5). Ретраи — экспоненциальный backoff, `max_attempts ~5`; `dead` **видим в UI** (no-silent-caps), GC завершённых джоб (S7). Кэш производных (`derived_cache`) — ключ инвалидации **`indexed_at`** (не `updated_at` — немонотонен), лимит размера, `files.hash` для деструктивных (S9). Offline LLM-джобы → копятся в `pending` (best-effort), не `failed` (S10). Числа-дефолты (глубина канала, воркеры, фон-семафор ≤ embed) — измеримы в AC, компромиссы в BACKLOG (S6).
**HARD-deps.** (1) Примитив пересборки derived (FTS5/usearch) в раннере миграций — **#13** (иначе первая схемо-миграция `jobs` ломает `.nexus` у пользователя). (2) **Новый Tauri event-канал** backend→фронт (сейчас `0 .emit` в src; нужен для StatusBar N/M, отмены/паузы scheduled, i18n-статусов; S8) — бонусом разблокирует live-пересчёт #35 «Цели».
**Почему.** Разблокирует vision-класс (News Feed/Карта/Противоречия/Home/память-агента); иначе — N костыльных `tokio::spawn` + N кэш-таблиц. Наибольший рычаг на связность.
**Каскад.** §4.x (новый раздел планировщика), §5.1 (rebuild-примитив #13), §12 (StatusBar + event-канал), `docs/reviews/ADR_CODESIGN.md`.
...
> ⚠️ **Статус реализации (фактчек кросс-плана; реализовано 2026-06-10, egress-фундамент ADR-005-ext).**
> Трейты `ChatProvider`/`EmbeddingProvider` реализованы (`ai/chat.rs`, `ai/embedder.rs`). Развёрнутый
> `AIClient` НИЖЕ (с `chat_fallback`/`cloud_fallback_enabled`/`guard_first_token`) — **ПЛАН, вне текущего
> ADR**: cloud-fallback и guard-first-token приходят отдельным срезом ВМЕСТЕ с cloud-feature (ADR-005-ext,
> `EgressFeature::CloudFallback`), не сейчас. **Что реально ввёл egress-фундамент (E1–E10):** тонкий
> фасад `AIClient { chat, chat_fast, chat_util: Option<Arc<dyn ChatProvider>>, embedder:
> Option<Arc<dyn EmbeddingProvider>>, policy: Arc<EgressPolicy> }` (`ai/mod.rs`) — заменяет ЧЕТЫРЕ
> независимых `Arc` в `VaultContext` одним полем (к моменту реализации код ушёл вперёд исходных «двух
> Arc»: добавились `chat_fast`/R2 и `chat_util`/ai.fast; решение владельца 2026-06-10 — фасад несёт все
> провайдеры). Composition-root — `build_rag`/`build_chat`/`build_util_chat` от единого
> `policy`+`audit` из `AppState`, БЕЗ `cloud_fallback`. Весь исходящий HTTP провайдеров идёт через
> `net::GuardedClient` (chokepoint, см. ADR-005-ext выше и `docs/dev/net.md`); `commands/chat.rs`
> по-прежнему зовёт `ChatProvider`, но клиент внутри — guarded. Реализация — `CROSSCUT_PLAN.md` #5/#16.
