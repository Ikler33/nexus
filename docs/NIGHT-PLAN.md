...
- **Chunker по токенайзеру эмбеддера** (§6.1): у llama.cpp есть `/tokenize`, но сетевой токенайзер сделает чанкинг async/сетевым (сейчас он чистый/быстрый/офлайн) + изменит границы → перепрогон eval/реиндекс. Спорный дизайн (точность счёта vs скорость/офлайн) — решение пользователя.
- **Виртуализация ленты чата** (#3): UI-скролл, нужна визуальная проверка человеком (jsdom не верифицирует) — не делать автономно.
...
- ✅ **Ф2-2a — Capability-broker host-сторона (§7.4)**: `plugin/broker.rs` — sessions с identity-по-порту (анти-confused-deputy), authorize→check→audit (неотключаемый AuditLog), revoke, handle→HostDispatch-трейт. 6 тестов. Rust 91 тест. Доки/CHANGELOG. Коммичу.
- 🏁 **Крон-сессия #1 завершена** на чистой вехе. Сделано за ночь: condition-eval, crash-reconcile, Ф1-12 (bge-m3, AC-EVAL-6 закрыт), **Ф2-1** (модель прав), **Ф2-2a** (брокер host-сторона). Rust 91 тест + 7 живых, всё зелёное, всё закоммичено в `phase1/12`→`phase2/01-capability-model`.

## Хендофф / что дальше (важно для следующей крон-сессии И пользователя)
Автономно-безопасные **backend**-пункты по Фазе 2 в основном исчерпаны. Оставшееся требует либо человека, либо спорно:
- **Ф2-2b/Ф2-3 = ФРОНТ** (MessagePort/iframe-транспорт, capability-токены, реальный `HostDispatch`, `registerCommand(source:'plugin')`, плагинные i18n) — нужна визуальная/интеграционная проверка → **делать с пользователем, не вслепую**.
- **CI-eval-фикстура** (кэш golden-эмбеддингов): можно автономно (bge-m3 поднят), НО fragile — регенерация на любое изменение golden/chunker/модели; live-гейт уже есть. Tradeoff в BACKLOG.
- **Chunker по сетевому токенайзеру** (`/tokenize` у llama.cpp): спорный дизайн (чанкинг станет сетевым/async) → NEEDS-DECISION, не делать молча.
> Если крон-сессия #2 (завтра 03:33) не находит явно-безопасного backend-пункта — НЕ браться за фронт/fragile вслепую: записать статус и ждать пользователя. Перед стартом: `curl :8083/health`, иначе `bash ~/Documents/llm-models/start_servers.sh`.
