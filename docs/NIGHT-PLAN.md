...
## Ночь 2026-06-11 (план согласован владельцем; «выбирай рекомендуемое» — стендинг-мандат)

| # | Срез | Статус |
|---|---|---|
| 1 | Реальный прогресс индексации «Индексация N/M» в статусбаре (`Indexer::with_progress` → `vault:index-progress`) | ✅ PR #146 merged |
| 2 | Покрытие watcher 61.8→97.1 / eval 48.8→93.5 (юниты to_raw_changes + live-smoke; вынос ignored live-тел в `live_tests.rs`); floors 85/85, global 75 | ✅ PR #147 |
| 3 | Мини-приёмка свежего хрома: токены 4 тем программно + Home в midnight (скрин) — расхождений нет | ✅ (сводка владельцу утром) |
| 4 | Перф-де-риск графа: рендер-троттл «дыхания» (физика 60fps, React-рендер 20fps на остывшем симе) | ✅ этот PR |
| 5 | Прод-сборка `tauri build --bundles app` от ночного main — чисто (`target/release/bundle/macos/Nexus.app`; dmg — известная headless-грабля, вручную) | ✅ |
| 6 | Хаускипинг: стейл-строки BACKLOG (inline-LLM/News Feed реализованы), docstring SyncPanel (resolver есть), этот журнал | ✅ этот PR |

Не бралось (owner-gated): egress 3–4 (SearXNG на VPS — без явного «го» к VPS не подключаюсь), релиз-трек #29/#30, live-smoke LLM (сервер в пересборке), перф-эпик (выбор токенайзера).
...
### Прогон 2026-06-10 (вечер, решение владельца): ЭПИК ДИЗАЙН-ПАРИТЕТА DP-0..11 — ЗАКРЫТ

> Владелец: «Берём весь дизайн ПОЛНОСТЬЮ. Итоговый вид приложения должен быть как на макете».
> 12 линейных PR за вечер (план `docs/dev/DESIGN_PARITY_PLAN.md`), каждый: test-all →
> превью-сверка с handoff-HTML (правильный воркстри, cwd-чек) → PR → зелёный CI → мерж.

- ✅ **DP-0** #123 фундамент: рефреш `docs/design/handoff` (макет добавил темы **Midnight Ink /
  Platinum Slate** и экраны settings/sync/insights/news), `src/motion.css`, план эпика.
- ✅ **DP-1** #124 **Home-дашборд** (самый крупный пробел): бэкенд **H6** `get_home_activity`
  (heatmap 17 нед. по локальным дням/tz, серии, сироты, «Продолжить» со сниппетом) + страница —
  greeting/chips/hero-search/continue/quick-actions/все секции с AI-картами и thinking; Home —
  стартовая вью. Ограничения mtime-данных честно в BACKLOG (история правок, «слов написано»).
- ✅ **DP-2** #125 сайдбар: icon-rail, панель тегов (новая команда `list_tags`), звёзды/избранное.
- ✅ **DP-3** #126 редактор: **DnD вкладок между панами** (`text/nexus-tab`, moveTab в сторе),
  mode-float ⌘E (режим в сторе), dirty-XOR-close, типографика превью (wikilink-скобки, цитаты,
  буллеты-акцент), collapsible-беклинки.
- ✅ **DP-4** #127 хром: AI-меню (Дайджест/Цели/Противоречия в sparkles-dropdown), reading-тоггл,
  **цикл 4 тем**, статусбар (дот/прогресс задач/Local·UTF-8·Markdown).
- ✅ **DP-5** #128 палитра: секции **Файлы** (search_vault) + Команды, футер-хинты.
- ✅ **DP-6** #129 граф: **пан/зум-камера + fit** (закрыл BACKLOG v2c), BrandThinking-лоадер.
- ✅ **DP-7** #130 онбординг 4 шага (vault → честный AI-check → фоновая индексация); попутно фикс
  flaky-teardown inline-rAF (jsdom node25).
- ✅ **DP-8** #131 плагины: `PluginInfo.permissions` (чипы safe/caution/sensitive),
  **consent-sheet** с risk-бейджами, персист+отзыв; маркетплейс — BACKLOG (нет каталога).
- ✅ **DP-9** #132 инсайт-модалки: серифный дайджест + **bold**, thinking-знаки. ⚠️ Инцидент:
  транзиентный **gh-401** в мерж-цепочке + автоудаление ветки закрыло немерженный PR —
  восстановлен из reflog (`git branch <sha>` → push → reopen); с тех пор поллеры удаляют ветку
  ТОЛЬКО после подтверждённого MERGED.
- ✅ **DP-10** #133 sync/conflict: **сообщение коммита** (git-ядро `commit_*_with_message`),
  resolver с кликабельными сторонами/прогрессом/bulk/гейтом «Применить».
- ✅ **DP-11** #134 настройки: **4 темы**, density **auto** (1180px), chrome **minimal**
  (`--chrome`), **шрифт редактора**, spotlight-палитра, имя для приветствия Home.
- ⛔ **DP-12** (хвост эпика): AI-панель layouts/RAG-стили + чат-бейдж E9 + AC-EGR-14 — ждёт
  решения владельца по дизайн-PR #97 (держит чат-файлы). Осознанно не перенесено из бандла
  (BACKLOG): маркетплейс плагинов, тег-чипы графа, «Переиндексировать», метрики истории слов.

### Прогон 2026-06-10 (день, с владельцем): egress срез 2 ч.1 + News Feed ЦЕЛИКОМ (NF-1..6)
...
- ✅ **News Feed: vision→AC сессия #2 + ВЕСЬ бэкенд за день** (D1–D7 решены владельцем в диалоге):
  ...
  **Итог бэкенда: AC-NF 10 covered + 1 partial из 12.**
- ✅ **News Feed UI: дизайн пришёл в тот же день — NF-5 и NF-6 реализованы и смержены.**
  Handoff-бандл (`hermes/`: README + чат-транскрипт с финальными итерациями владельца — транскрипт
  обязателен к чтению, последние решения только там):
  - **NF-5** (PR #120): страница `NewsView` — AI-«Сводка дня» (мета прогона, раскрываемые ошибки
    источников — no silent caps), чипы тем + «Непрочитанные» (серверные фильтры), рубрики,
    карточки (резюме-фейл → «Резюме недоступно»), consent-CTA (источники из новой команды
    `news_sources`), скелетон/пустой день/ошибка прогона/офлайн-баннер, gear → «Выключить ленту»;
    `tauriApi.news` + стейтфул-мок с контентом макета (превью ≡ макет), вход Rss в титлбаре +
    палитра, i18n RU/EN. **AC-NF-10/12 → covered: ВСЕ 12 AC-NF закрыты.**
  - **NF-6** (PR #121): reader — клик по заголовку открывает статью in-app: `news_article`
    (кэш тела, миграция 011 → guarded-фетч через политику NF-4 → извлечение `<p>` с капом 24k и
    видимым усечением → **полный RU-перевод** chat_util батчами с injection-маркерами; RU-источники
    passthrough без LLM) + `news_summarize` («Сократить» → панель «Кратко»). **HN-кейс fail-closed:
    хост вне news-allowlist → typed `denied`, allowlist по клику НЕ расширяется** (резюме +
    «Оригинал» в браузере). Live-smoke перевода — по возвращении сервера.
  Пробелы дизайн-бандла озвучены владельцу: Home-UI (бэкенд готов), многошаговый онбординг,
  plugin-consent-sheet, fidelity AI-панели (#97) — ждут его решения о разделении с дизайн-чатом.
