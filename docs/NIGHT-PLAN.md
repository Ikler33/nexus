...
### 🏁 Статус очереди (после vision→AC #1)
**Автономная очередь Волн 2 + 4 (fic-часть) + фундамент-парсинг — ИСЧЕРПАНЫ;** идёт серия vision→AC.
Слито за дневную сессию: граф v2d (#44), V2.2 rename (#45), V4.4 общий чат (#46), V4.3 анти-инъекция
(#47), V4.5 eval-гейт (#48), V4.2 redaction (#49), typed-frontmatter (#50), + спека inline-LLM (#51).
Ранее (ночь): V1.1-1.3 тест-гейты, V2.1/2.3/2.4, V4.1. **Дальше:** vision→AC по остальным фичам (умные
шаблоны, память агента, News Feed, противоречия, карта компетенций) **ИЛИ** реализация inline-LLM по
спеке (IL-1..4) **ИЛИ** BLOCKED (нужны ADR/инфра) **ИЛИ** доводка графа (v2e/b/c). cron-guard: развилка
требует владельца.
