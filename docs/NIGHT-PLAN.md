...
- ⏭ Следующее: **Ф2-2b = фронт** (MessagePort/iframe-транспорт, capability-токены, реальный HostDispatch к vault/ai) — нужна визуальная/интеграционная проверка человеком → **отложить до доступности пользователя** (записать, не делать вслепую). Автономно лучше брать backend-пункты: например CI-eval-фикстура (#5, bge-m3 поднят) или другой самодостаточный из BACKLOG. Перед стартом крон-сессии: `curl :8083/health`, при недоступности — `bash ~/Documents/llm-models/start_servers.sh`.
