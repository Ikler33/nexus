...
- `#13` примитив rebuild FTS5/usearch в раннере миграций — **ПЕРВЫМ** (жёсткая зависимость всех схемо-миграций)
- `#9` AppState-аксессоры + типизированный `AppError` · `#12` Rust integration-крейт (git_* непокрыты; нужна git-identity в CI) · `#28` декомпозиция `indexer/mod.rs` (1290 строк)
- `#10` выборочный git-стейдж по одобренным типам (defense-in-depth; дизайн списка типов — `themes/**`=JS) · `#22` пагинация `list_notes` · `#25` discriminated Buffer (под граф-во-вкладку) · `#17` персист истории чата (через rebuild-раннер) · `#27` DNS-rebinding гард plugin-fetch
- **perf-эпик строго:** `#14` реальный токенайзер → `#15` cross-file batching (L, ломает инвариант одной задачи) → `#6` квантизация
- `#11` LLM-настройки UI (11a форма + 11b hot-apply — L-эпик, НЕ near-pure-frontend)

**🔴 Wave C — нужен владелец (ADR/инфра/решения):**
- `#29` подпись/нотаризация — **сертификаты получать СЕЙЧАС параллельно** (Apple $99/год + Authenticode, недели лид-тайма) → `#30` updater (ПОСЛЕ подпись-формы) → `#26` release.yml (auto) → `#31` E2E-смоук (auto)
