...
- 🔄 **Граф: тюнинг физики/узлов** (отзыв владельца vs Obsidian: «всё в центре», «резкие анимации»,
  «мало градаций»). `graph-sim.ts`: отталкивание 6800→16000, гравитация 0.012→0.004, пружины 96→150,
  decay 0.94→0.95, кламп скорости ±26 (плавность), радиус sqrt 5..30. 90 фронт-тестов + coverage 68%
  зелёные. **Визуал не вижу → ветка `track/10-graph-tuning`, владелец проверяет локально перед мержем**
  (числа покрутим по его глазам). Отложено в BACKLOG: пан/зум-камера + авто-fit (Obsidian-like). Дальше
  по списку: V4.4 (общий чат) / V2.2 (rename) / граф-теги.
