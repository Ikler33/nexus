...
- ✅ **Пункт #2 — Crash-reconcile usearch (§5.1)**: `reconcile_vectors` в `scan_vault` + тест; BACKLOG/доки/CHANGELOG обновлены. Rust 72 теста зелёные. Коммичу. (bge-m3 ~50%, ещё качается → Ф1-12 после докачки.)
- ⏭ Следующее: Ф1-12 (bge-m3) когда докачается; затем пункт #3 (виртуализация ленты чата) / Фаза 2.
