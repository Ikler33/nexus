...
### Бюджет прогона агента (wall-clock / шаги)

Границы ОДНОГО прогона агента ([`LoopBounds`]). Оба ключа опциональны и тип-толерантны: мусорный тип
(строка/булево/массив) → игнорируется (дефолт), НЕ роняет весь `local.json`. Читаются всеми точками входа,
собирающими прогон из `local.json` (десктоп-чат, безголовый `agentd` scheduler+коннектор, `nexus agent`,
`nexus acp`); субагенты/smoke/eval остаются на дефолте. BF-1 уже исключил время ожидания решения человека у
гейта из `wall_clock` — эти ключи дают владельцу ручку на бюджет РЕАЛЬНОЙ работы.

| Путь | Тип | Дефолт | Назначение / кламп |
|---|---|---|---|
| `ai.agent_wall_clock_secs` | u64 | `null` → 300 (5 мин) | Стенной бюджет всего прогона (сек). Санитарный кламп **30 .. 86400** (меньше 30 с — прогон не успевает стартовать; больше суток — по сути демон). Значения вне диапазона тихо клампятся. |
| `ai.agent_max_steps` | u64 | `null` → 8 | Потолок ходов модели (анти-зацикливание). Кламп **1 .. 10000**. Пара к `wall_clock`: длинный бюджет времени без большего числа шагов — половина ручки. |
...
- **Валидация:** `*.url` → SSRF/DNS-rebind guard на эгрессе (fail-closed); `agent_autonomy` неизвестное значение → не сохраняется, дефолт `confirm`; `shell_enable` некогерентный (без sandbox) не сохраняется; `research.max_rounds=0` → клам к 1; `agent_wall_clock_secs` клампится 30..86400 с, `agent_max_steps` — 1..10000 (мусорный тип обоих → дефолт, не роняет парс); не-Linux → sandbox/shell структурно инертны.
