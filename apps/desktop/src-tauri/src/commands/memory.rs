...
// фронт команду не вызывал (мёртвая API-поверхность). Ядро `memory::fact_history` живёт (его
// используют консолидация и core-тесты); вернуть мост тривиально, когда появится панель.
