...
pub enum VaultEvent {
    ...
    /// Ручной реиндекс vault (команда `rescan_vault`, quick action «Переиндексировать» из макета
    /// home.jsx): полный повторный обход `scan_vault`. Идёт через ТОТ ЖЕ канал, что fs-события —
    /// сериализован с ними в одной петле (без гонок на usearch/БД).
    Rescan,
}
