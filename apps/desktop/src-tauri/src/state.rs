...
pub struct AppState {
    ...
    /// CONN-1/2: активный агент-бэкенд. По умолчанию [`crate::agent_backend::EmbeddedBackend`] (in-process);
    /// при `ai.connection.mode="local"` `open_vault` свапает на `ConnectedBackend` (клиент agentd). `RwLock`
    /// — редкая смена (per-vault при открытии), много читателей-команд. Future (CONN-4): свап из настроек.
    pub agent_backend: RwLock<Arc<dyn crate::agent_backend::AgentBackend>>,
}
...
impl AppState {
    pub fn new() -> Self {
        ...
        Self {
            ...
            agent_backend: RwLock::new(Arc::new(crate::agent_backend::EmbeddedBackend)),
        }
    }

    /// CONN-1/2: активный агент-бэкенд (клон `Arc` — методы берут `&AppState` отдельным шаредом).
    pub async fn agent_backend(&self) -> Arc<dyn crate::agent_backend::AgentBackend> {
        self.agent_backend.read().await.clone()
    }
