...
describe('AgentView (UI-1b — фронт вкладки Агента на контракте UI-1a)', () => {
  ...
  it('MD: во время стрима ответ плейн-текст (markdown НЕ применяется к активному ходу)', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          ...
          report: null,
          error: null,
          status: 'running', // active → плейн
        },
      ],
    });
    const { container } = render(<AgentView />);
    // Активный ход → НЕ форматируем (нет <h2>), сырой markdown виден как текст.
