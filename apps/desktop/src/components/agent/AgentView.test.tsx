import { fireEvent, render, screen, waitFor } from '@testing-library/react';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { AgentView } from './AgentView';
import { tauriApi } from '../../lib/tauri-api';
import { useAgentStore, sessionStatus } from '../../stores/agent';
import { useToastStore } from '../../stores/toast';
import { useUIStore } from '../../stores/ui';
import * as mockAgent from '../../lib/mock/agent';

/** Статус сессии (последнего хода) и runId последнего хода — из мультитёрн-ленты. */
const curStatus = () => sessionStatus(useAgentStore.getState().turns);
const lastRunId = () => useAgentStore.getState().turns.at(-1)?.runId ?? null;

/** Сброс стора агента + мок-реестра прогонов между тестами (мок — память процесса). */
function reset() {
  useAgentStore.setState({
    turns: [],
    autonomy: 'confirm',
    perms: { read: true, write: true, web: false },
    context: null,
    approving: false,
  });
  useToastStore.setState({ toasts: [] });
  useUIStore.setState({ pendingAgentSeed: null });
  mockAgent.__reset();
}

beforeEach(reset);
afterEach(() => {
  vi.restoreAllMocks();
});

describe('AgentView (UI-1b — фронт вкладки Агента на контракте UI-1a)', () => {
  it('пустое состояние: шапка, подсказка и композер до запуска', () => {
    render(<AgentView />);
    // Шапка (заголовок) + кнопка «Новая сессия».
    expect(screen.getByText('Castor')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Новая сессия' })).toBeInTheDocument();
    // Пустое состояние — подсказка «поручите задачу».
    expect(screen.getByText('Поручите задачу агенту')).toBeInTheDocument();
    // Композер виден и его поле доступно (прогон не идёт).
    expect(screen.getByPlaceholderText('Поручите задачу агенту…')).not.toBeDisabled();
    expect(screen.getByRole('button', { name: 'Запустить' })).toBeDisabled(); // пустой ввод
  });

  // B10: фиктивный селектор моделей убран (значение никуда не передавалось — бэкенд выбирает модель
  // по конфигу сам). Меню настроек живо (автономность/права), но секции «Модель» и хардкод-имён нет;
  // футер композера не показывает выдуманное имя модели. Вернём при INFER-CFG.
  it('B10: селектора моделей нет — ни в меню настроек, ни в футере композера', () => {
    render(<AgentView />);
    fireEvent.click(screen.getByRole('button', { name: 'Настройки агента' }));
    // Меню открылось: секции автономности и прав на месте.
    expect(screen.getByText('Автономность')).toBeInTheDocument();
    expect(screen.getByText('Права в хранилище')).toBeInTheDocument();
    // Секции «Модель» и фиктивных имён моделей больше нет нигде в вью.
    expect(screen.queryByText('Модель')).toBeNull();
    for (const fake of ['qwen3:35b', 'llama3.3', 'gpt-5']) {
      expect(screen.queryByText(fake)).toBeNull();
    }
  });

  it('запуск через композер: мок-стрим наполняет ленту (ответ ассистента + шаг + changeset)', async () => {
    render(<AgentView />);
    const input = screen.getByPlaceholderText('Поручите задачу агенту…');
    fireEvent.change(input, { target: { value: 'Разбери входящие' } });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));

    // Задача отрисована как сообщение пользователя.
    expect(await screen.findByText('Разбери входящие')).toBeInTheDocument();

    // assistantToken-дельты склеились в ответ ассистента.
    await waitFor(() => expect(screen.getByText(/Принял задачу/)).toBeInTheDocument());

    // toolCall → шаг ленты с kind инструмента (fs.read из мока).
    await waitFor(() => expect(screen.getAllByText('fs.read').length).toBeGreaterThan(0));

    // proposal → changeset с файлами + заголовок «Изменения».
    await waitFor(() => expect(screen.getByText('Изменения')).toBeInTheDocument());
    expect(screen.getByText('RMS-B2B/Идея — кэш контекста.md')).toBeInTheDocument();
    // ACP-EXEC: exec-permission рисуется как команда (ярлык «Выполнить команду» + текст команды),
    // а НЕ как файл с ±строками/диффом.
    expect(screen.getByText('Выполнить команду')).toBeInTheDocument();
    expect(screen.getByText('git status --short')).toBeInTheDocument();
  });

  it('changeset apply/reject собирает decisions[] и шлёт agent_approve', async () => {
    const approveSpy = vi.spyOn(tauriApi.agent, 'approve');
    render(<AgentView />);
    fireEvent.change(screen.getByPlaceholderText('Поручите задачу агенту…'), {
      target: { value: 'задача' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));

    // Ждём появления changeset (proposal) и перехода в статус ожидания решения.
    await waitFor(() => expect(screen.getByText('Изменения')).toBeInTheDocument());
    await waitFor(() => expect(curStatus()).toBe('awaiting'));

    // Per-file: применяем первый файл, отклоняем второй, третий оставляем нерешённым (→ approve: «Подтвердить»
    // одобряет всё не отклонённое ЯВНО).
    // Кнопки перезапрашиваем после каждого клика — решённый файл сменил кнопки на бейдж (DOM-сдвиг).
    // Per-file reject = «Отклонить изменение» (отличается от bulk «Отклонить» — иначе клик попал бы в bulk).
    fireEvent.click(screen.getAllByRole('button', { name: 'Применить' })[0]); // файл 1 → applied
    fireEvent.click(screen.getAllByRole('button', { name: 'Отклонить изменение' })[0]); // файл 2 → rejected

    // Подтверждаем changeset → собранные decisions уходят в agent_approve.
    fireEvent.click(screen.getByRole('button', { name: 'Подтвердить' }));

    await waitFor(() => expect(approveSpy).toHaveBeenCalledTimes(1));
    const [runId, decisions] = approveSpy.mock.calls[0];
    expect(runId).toBe(lastRunId());
    // decisions[] = по одному на адресуемый файл (3 файла + 1 exec); reject только у ЯВНО отклонённого,
    // остальное (applied + нерешённые file3/exec) → approve («Подтвердить» утвердителен).
    expect(decisions).toHaveLength(4);
    const byApprove = decisions.map((d) => d.approve);
    expect(byApprove.filter(Boolean)).toHaveLength(3); // applied + file3 + exec (нерешённые → approve)
    expect(byApprove.filter((a) => !a)).toHaveLength(1); // только явно отклонённый файл 2
    // Каждое решение адресовано actionId из proposal (>= 0, не -1).
    expect(decisions.every((d) => d.actionId >= 0)).toBe(true);
  });

  it('bulk «Применить все» помечает все файлы applied → все approve=true', async () => {
    const approveSpy = vi.spyOn(tauriApi.agent, 'approve');
    render(<AgentView />);
    fireEvent.change(screen.getByPlaceholderText('Поручите задачу агенту…'), {
      target: { value: 'задача' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));
    await waitFor(() => expect(curStatus()).toBe('awaiting'));

    fireEvent.click(screen.getByRole('button', { name: 'Применить все' }));
    fireEvent.click(screen.getByRole('button', { name: 'Подтвердить' }));

    await waitFor(() => expect(approveSpy).toHaveBeenCalledTimes(1));
    const decisions = approveSpy.mock.calls[0][1];
    expect(decisions.every((d) => d.approve)).toBe(true);

    // После решения карточка СВОРАЧИВАЕТСЯ: интерактивных кнопок нет, остаётся строка-итог.
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: 'Подтвердить' })).not.toBeInTheDocument(),
    );
    expect(screen.queryByRole('button', { name: 'Применить все' })).not.toBeInTheDocument();
    // Заголовок «Изменения» остаётся (компактный итог), но без интерактива.
    expect(screen.getByText('Изменения')).toBeInTheDocument();
  });

  // Fix BF-1 MINOR-1: разметка per-file решений — лишь ВЫБОР до «Подтвердить»; отмена хода не должна
  // показывать «применено» (записи не было). Карточка сворачивается в честное «прогон отменён».
  it('MINOR-1: отмена после разметки НЕ показывает «применено» — компакт-строка «прогон отменён»', async () => {
    render(<AgentView />);
    fireEvent.change(screen.getByPlaceholderText('Поручите задачу агенту…'), {
      target: { value: 'задача' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));
    await waitFor(() => expect(curStatus()).toBe('awaiting'));

    // Размечаем ВСЕ файлы как «выбранные к применению»… но вместо «Подтвердить» — отменяем ход.
    fireEvent.click(screen.getByRole('button', { name: 'Применить все' }));
    fireEvent.click(screen.getByRole('button', { name: 'Остановить' }));
    await waitFor(() => expect(curStatus()).toBe('cancelled'));

    // Карточка свёрнута и честна: «прогон отменён», НЕ «применено: N» (запись не состоялась).
    await waitFor(() => expect(screen.getByText('прогон отменён')).toBeInTheDocument());
    expect(screen.queryByText(/применено:/)).toBeNull();
    // Интерактива не осталось (кнопки не «мёртвые», их просто нет).
    expect(screen.queryByRole('button', { name: 'Подтвердить' })).toBeNull();
    expect(screen.queryByRole('button', { name: 'Применить все' })).toBeNull();
  });

  it('autonomy=auto: changeset показывает авто-бейдж, аппрув не требуется', async () => {
    useAgentStore.setState({ autonomy: 'auto' });
    render(<AgentView />);
    fireEvent.change(screen.getByPlaceholderText('Поручите задачу агенту…'), {
      target: { value: 'авто-задача' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));

    // Auto-режим: дифы идут без proposal — changeset наполняется, бейдж «Авто».
    await waitFor(() => expect(screen.getByText('Изменения')).toBeInTheDocument());
    expect(screen.getByText(/Авто · агент ревьюит сам/)).toBeInTheDocument();
    // Нет кнопки «Подтвердить» (auto не ждёт аппрува).
    expect(screen.queryByRole('button', { name: 'Подтвердить' })).not.toBeInTheDocument();
  });

  it('мультитёрн: второе сообщение НЕ стирает первое (регрессия «переписка исчезла»)', async () => {
    useAgentStore.setState({ autonomy: 'auto' }); // auto → ход завершается сам (final → done)
    render(<AgentView />);
    const input = () => screen.getByPlaceholderText('Поручите задачу агенту…');

    // Ход 1 — отправляем и дожидаемся завершения (композер снова доступен).
    fireEvent.change(input(), { target: { value: 'первая задача' } });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));
    expect(await screen.findByText('первая задача')).toBeInTheDocument();
    await waitFor(() => expect(curStatus()).toBe('done'));

    // Ход 2 — отправляем второе сообщение.
    fireEvent.change(input(), { target: { value: 'вторая задача' } });
    fireEvent.click(screen.getByRole('button', { name: 'Запустить' }));
    expect(await screen.findByText('вторая задача')).toBeInTheDocument();

    // ОБА сообщения остаются в ленте — первое НЕ стёрто (суть фикса).
    expect(screen.getByText('первая задача')).toBeInTheDocument();
    expect(screen.getByText('вторая задача')).toBeInTheDocument();
    expect(useAgentStore.getState().turns).toHaveLength(2);
  });

  // W-14: правый dock «План» = РЕАЛЬНЫЕ шаги хода (tool-вызовы), а не статичная демо-заглушка (ST-G6).
  it('W-14: план показывает реальные kind-шаги прогона, без демо-меток', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'задача',
          assistantText: '',
          steps: [
            { id: 'a', kind: 'note.create', args: '{}', result: 'ok', isError: false },
            { id: 'b', kind: 'web.search', args: '{}', result: null, isError: false },
          ],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'running',
        },
      ],
    });
    render(<AgentView />);
    // dock 'plan' открыт по умолчанию → реальные kind'ы видны.
    expect(screen.getAllByText('note.create').length).toBeGreaterThan(0);
    expect(screen.getAllByText('web.search').length).toBeGreaterThan(0);
    // Старые демо-метки заглушки исчезли.
    expect(screen.queryByText('match.projects')).toBeNull();
  });

  // Граф выполнения (ExecGraph) в доке «Граф»: субагенты теперь = branch-узлы графа (вместо SubagentTree).
  it('граф: субагент показан как branch-узел в доке «Граф» (живые данные turn.subagents)', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'исследуй тему',
          assistantText: '',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [
            {
              childRunId: 2,
              parentRunId: 1,
              goal: 'найти источники',
              status: 'running',
              summary: undefined,
            },
          ],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'running',
        },
      ],
    });
    const { container } = render(<AgentView />);
    // Переключаем правый dock на «Граф» (aria-label из i18n ru).
    fireEvent.click(screen.getByLabelText('Граф выполнения'));
    // Цель субагента видна как подпись узла графа (в .fgLabel-div + дублируется в <title> узла).
    expect(screen.getAllByText('найти источники').length).toBeGreaterThan(0);
    // Есть branch-ребро (root→субагент) и узловые круги (root + субагент = 2).
    expect(container.querySelectorAll('[class*="fgEdgeBranch"]').length).toBe(1);
    expect(container.querySelectorAll('[class*="fgDot"]').length).toBe(2);
    // Демо-граф НЕ показан (ResearchGraph удалён — граф строится из живых данных).
    expect(screen.queryByText(/Демо-граф/)).toBeNull();
  });

  // Граф выполнения: exec теперь = command-узел графа (вместо ExecList) — на Linux-песочнице без command-step.
  it('граф: exec без command-step показан как command-узел в доке «Граф»', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'собери проект',
          assistantText: '',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [
            { runId: 1, actionId: 5, summary: 'shell.run · 2 args', exitCode: 0, finalized: true },
          ],
          researchReport: null,
          report: null,
          error: null,
          status: 'running',
        },
      ],
    });
    render(<AgentView />);
    fireEvent.click(screen.getByLabelText('Граф выполнения'));
    // Силуэт команды виден как подпись узла графа (в .fgLabel-div + дублируется в <title> узла).
    expect(screen.getAllByText('shell.run · 2 args').length).toBeGreaterThan(0);
    expect(screen.queryByText(/Демо-граф/)).toBeNull(); // ResearchGraph удалён
  });

  // W-25: карточка отчёта deep-research в доке «Отчёт» — живые данные из turn.researchReport.
  it('W-25: карточка deep-research показывает заголовок и кнопку открытия в доке «Отчёт»', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'исследуй',
          assistantText: '',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: {
            runId: 1,
            title: 'Отчёт по теме',
            path: 'Research/tema-2026.md',
            sourcesCount: 7,
            rounds: 3,
          },
          report: null,
          error: null,
          status: 'done',
        },
      ],
    });
    render(<AgentView />);
    fireEvent.click(screen.getByLabelText('Отчёт')); // rail aria-label из i18n ru (agent.dock.report)
    expect(screen.getByText('Отчёт по теме')).toBeTruthy();
    expect(screen.getByText('Открыть отчёт')).toBeTruthy();
  });

  // W-15: окно подтверждения changeset показывает не только ±N, но и inline-дифф контента по клику.
  it('W-15: inline-дифф файла раскрывается по тогглу (proposed из note.create-args)', async () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'создай заметку',
          assistantText: '',
          steps: [
            {
              id: 'w',
              kind: 'note.create',
              args: JSON.stringify({ path: 'Notes/X.md', content: 'строка раз\nстрока два' }),
              result: 'proposed',
              isError: false,
            },
          ],
          changeset: [
            { path: 'Notes/X.md', add: 2, del: 0, status: 'new', kind: 'file', actionId: 1, decision: undefined },
          ],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'awaiting',
        },
      ],
    });
    render(<AgentView />);
    expect(screen.getByText('Notes/X.md')).toBeInTheDocument();
    // До клика контента диффа нет.
    expect(screen.queryByText('строка раз')).toBeNull();
    // Тоггл диффа → строки proposed как добавленные (новый файл → current пуст).
    fireEvent.click(screen.getByRole('button', { name: 'Показать/скрыть дифф' }));
    expect(await screen.findByText('строка раз')).toBeInTheDocument();
    expect(screen.getByText('строка два')).toBeInTheDocument();
    // Ревью W-15: новый файл → чистый add-дифф, без ложной ведущей пустой `del`-строки (символ «−»).
    expect(screen.queryByText('−')).toBeNull();
  });

  // ACP-EXEC: exec-changeset-строка рисуется как команда (`$ cmd` + ярлык), БЕЗ ±строк/диффа;
  // файловая строка по-прежнему показывает ±строки и (при наличии proposed-контента) тоггл диффа.
  it('ACP-EXEC: exec-строка — команда без ±строк/диффа; файловая строка — с ±строками', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'собери проект',
          assistantText: '',
          steps: [
            {
              id: 'w',
              kind: 'note.create',
              args: JSON.stringify({ path: 'Notes/F.md', content: 'строка' }),
              result: 'proposed',
              isError: false,
            },
          ],
          changeset: [
            { path: 'Notes/F.md', add: 5, del: 2, status: 'edit', kind: 'file', actionId: 1, decision: undefined },
            { path: 'cargo build', add: 0, del: 0, status: 'edit', kind: 'exec', actionId: 2, decision: undefined },
          ],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'awaiting',
        },
      ],
    });
    render(<AgentView />);

    // Файловая строка: путь + ±строки (счётчики +5/−2 видны; шапка тоже их суммирует → ≥1 совпадение).
    expect(screen.getByText('Notes/F.md')).toBeInTheDocument();
    expect(screen.getAllByText('+5').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('−2').length).toBeGreaterThanOrEqual(1);
    // Тоггл диффа есть РОВНО один — только у файловой строки (exec-строка его не даёт).
    expect(screen.getAllByRole('button', { name: 'Показать/скрыть дифф' })).toHaveLength(1);

    // Exec-строка: командный текст + ярлык «Выполнить команду» (`$` — aria-hidden).
    expect(screen.getByText('cargo build')).toBeInTheDocument();
    expect(screen.getByText('Выполнить команду')).toBeInTheDocument();
    // Exec НЕ показывает свои ±строки (нет «+0» exec-строки).
    expect(screen.queryByText('+0')).toBeNull();
    // Но Approve/Reject у обеих строк есть (решение по permission через actionId).
    expect(screen.getAllByRole('button', { name: 'Применить' })).toHaveLength(2);
  });

  // MD: завершённый ход с markdown в assistantText рендерится отформатированным (как чат), а не сырым.
  it('MD: завершённый ответ агента рендерится как markdown (заголовок/жирный/код), не сырой текст', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'вопрос',
          assistantText: '## Heading\n\n**bold** and `code`',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'done',
        },
      ],
    });
    const { container } = render(<AgentView />);

    // Завершённый ход (status='done', не active) → финальный markdown-рендер.
    const h2 = container.querySelector('h2');
    expect(h2).not.toBeNull();
    expect(h2?.textContent).toBe('Heading');
    expect(container.querySelector('strong')?.textContent).toBe('bold');
    expect(container.querySelector('code')?.textContent).toBe('code');

    // Сырые markdown-маркеры НЕ остаются литеральным текстом в ответе.
    expect(container.textContent).not.toContain('## Heading');
    expect(container.textContent).not.toContain('**bold**');
  });

  // MD: во время стрима (active) ответ остаётся плейн-текстом (raw, быстро) — markdown только в финале.
  it('W-34: во время стрима ответ — ЖИВОЙ markdown (StreamingMarkdown), а не сырой текст', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'вопрос',
          assistantText: '## Heading\n\n**bold**',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'running', // active → живой markdown
        },
      ],
    });
    const { container } = render(<AgentView />);
    // Активный ход → markdown применяется по живому (первый кадр StreamingMarkdown синхронен):
    // заголовок → <h2>, жирный → <strong>, сырые маркеры НЕ остаются литеральным текстом.
    expect(container.querySelector('h2')?.textContent).toBe('Heading');
    expect(container.querySelector('strong')?.textContent).toBe('bold');
    expect(container.textContent).not.toContain('## Heading');
    expect(container.textContent).not.toContain('**bold**');
  });

  // P0-3 (аудит, Срез B): errored-прогон (применил N файлов в auto, затем упал) показывает статус-пилюлю
  // + кнопку Undo (откат работает на любом run_id). P0-1: Pause/Resume убраны из UI во ВСЕХ статусах.
  it('P0-3: errored-прогон показывает статус + кнопку Undo (клик → agent_undo); P0-1: нет Pause/Resume', async () => {
    const undoSpy = vi.spyOn(tauriApi.agent, 'undo');
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'задача',
          assistantText: 'частично применил, затем упал',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: 'движок упал',
          status: 'error',
        },
      ],
    });
    render(<AgentView />);

    // Статус-пилюля «Ошибка» (errored-прогон теперь даёт контролы — P0-3).
    expect(screen.getByText('Ошибка')).toBeInTheDocument();
    // Кнопка отката доступна и для error-статуса.
    const undoBtn = screen.getByRole('button', { name: /Откатить изменения/ });
    expect(undoBtn).toBeInTheDocument();
    fireEvent.click(undoBtn);
    await waitFor(() => expect(undoSpy).toHaveBeenCalledWith(1));

    // P0-1: сломанные Pause/Resume убраны — ни в каком статусе их нет.
    expect(screen.queryByRole('button', { name: 'Пауза' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Продолжить' })).not.toBeInTheDocument();
  });

  it('P0-1: даже у активного (running) хода нет кнопок Pause/Resume — только Остановить', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'задача',
          assistantText: 'работаю',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'running',
        },
      ],
    });
    render(<AgentView />);
    // Активный ход: есть «Остановить» (cancel), но НЕТ Pause/Resume.
    expect(screen.getByRole('button', { name: 'Остановить' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Пауза' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Продолжить' })).not.toBeInTheDocument();
  });

  // P1-11: Castor «Быстрый старт» → openAgent(seed) кладёт промпт в ui-стор; AgentView ПРЕДЗАПОЛНЯЕТ
  // композер (НЕ авто-отправляет), консьюмит сид. Прогон НЕ запускается сам — кнопка «Запустить» активна.
  it('P1-11: отложенный промпт (pendingAgentSeed) предзаполняет композер, не запуская прогон', async () => {
    useUIStore.setState({ pendingAgentSeed: { text: 'Разбери мои входящие заметки', seq: 1 } });
    render(<AgentView />);
    const input = screen.getByPlaceholderText('Поручите задачу агенту…') as HTMLTextAreaElement;
    await waitFor(() => expect(input.value).toBe('Разбери мои входящие заметки'));
    // Сид сконсьюмлен (не «прольётся» повторно), кнопка «Запустить» активна, прогон НЕ стартовал сам.
    expect(useUIStore.getState().pendingAgentSeed).toBeNull();
    expect(screen.getByRole('button', { name: 'Запустить' })).not.toBeDisabled();
    expect(useAgentStore.getState().turns).toHaveLength(0);
  });

  // P1-11 FIX-3: сценарий «ушёл при работающем прогоне (AgentView размонтирован, input='') → клик быстрого
  // старта → REMOUNT с active=true (ход ещё в сторе)». Прежняя active-ветка молча выбрасывала промпт.
  // Теперь при ПУСТОМ композере промпт прокидывается даже при active (защищаем только непустой ввод).
  it('P1-11 FIX-3: при активном прогоне и пустом композере промпт всё равно предзаполняется (не теряется)', async () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'идёт прогон',
          assistantText: 'работаю',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'running', // active=true (ход в сторе), композер disabled
        },
      ],
    });
    useUIStore.setState({ pendingAgentSeed: { text: 'Наведи порядок в тегах', seq: 1 } });
    render(<AgentView />);
    const input = screen.getByPlaceholderText('Поручите задачу агенту…') as HTMLTextAreaElement;
    // Промпт попал в композер, ХОТЯ прогон активен (поле disabled, но значение проставлено) → не потерян.
    await waitFor(() => expect(input.value).toBe('Наведи порядок в тегах'));
    expect(input).toBeDisabled(); // прогон идёт → поле disabled, но текст ждёт пользователя
    expect(useUIStore.getState().pendingAgentSeed).toBeNull(); // сид сконсьюмлен
  });

  it('W-14: план пуст (честный стейт), когда ход без действий', () => {
    useAgentStore.setState({
      turns: [
        {
          key: 0,
          epoch: 1,
          runId: 1,
          task: 'вопрос',
          assistantText: 'ответ',
          steps: [],
          changeset: [],
          plan: [],
          subagents: [],
          execItems: [],
          researchReport: null,
          report: null,
          error: null,
          status: 'done',
        },
      ],
    });
    render(<AgentView />);
    expect(screen.getByText(/Шагов пока нет|No steps yet/)).toBeInTheDocument();
  });
});
