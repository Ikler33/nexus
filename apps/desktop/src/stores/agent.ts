 ...
 *
 * F-11 (вырез agent в модуль): этот стор — data/domain-слой агента, импортируется ТОЛЬКО из
 * UI-зоны агента (agent-компоненты), как `stores/news.ts` — только из своей news-зоны. Остаётся в
 * `stores/`: инвариант выреза — про UI-компоненты агента, а НЕ про data-слой; ко-локация под
 * agent-namespace — косметика вне скоупа. Стейт ВИДИМОСТИ вью (`mainView`) и seed-handoff
 * (`pendingAgentSeed`) — отдельно, в ui-сторе (ядро), как для news/board. См. docs/dev/connector.md.
 */
