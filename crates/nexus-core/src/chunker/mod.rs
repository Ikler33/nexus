...
//!
//! Токены считает [`Tokenizer`]. В проде — реальный BPE-токенайзер задеплоенной модели
//! (`crate::ai::QwenTokenizer`, P0-c): кириллица считается верно (словесная эвристика врала в
//! 1.5–2×). [`WordTokenizer`] сохранён как детерминированный заглушка-токенайзер для тестов/моков.
...
/// Детерминированный токенайзер-заглушка: число слов (без зависимости от модели). В проде заменён
/// реальным `crate::ai::QwenTokenizer` (P0-c); оставлен для тестов чанкера и моков.
