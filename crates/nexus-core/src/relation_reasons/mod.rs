...
pub async fn explain_relation(
    ...
    // Кэш-хит при совпадении хэшей сниппетов (заметка не менялась) — ВКЛЮЧАЯ пустое объяснение:
    // пустой результат = «модель уже сгенерила, вышло нечего сказать», и пере-вызывать её на каждый
    // показ не нужно (находка аудита: `!expl.is_empty()` ронял кэш-хит → лишние LLM-вызовы). Пустое
    // объяснение фронт и так заменит сниппетом.
