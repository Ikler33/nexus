...
//!
//! **R-1 (развязка слоёв):** модуль НЕЙТРАЛЕН к слоям и живёт на верхнем уровне ядра — контракт
//! событий используют и агент (эмиттер цикла, `crate::agent`), и актуатор (эмиттер
//! [`AgentEvent::Proposal`]/[`AgentEvent::Diff`] в `crate::actuator::orchestrate`) БЕЗ ребра
//! actuator→agent. Старый путь `crate::agent::event` сохранён реэкспортом (потребители не правятся).
//!
