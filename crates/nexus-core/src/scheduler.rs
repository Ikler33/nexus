//! Планировщик фоновых задач (ADR-007) — **обобщённый движок** (CORE-1b): tauri-free, зависит только
//! на `crate::db` (= `nexus_core::db`) + std/tokio, чтобы headless agent-service переиспользовал его без
//! Tauri/UI. Очередь `jobs` (данные) + движок диспатча: реестр обработчиков (kind → handler), прогон
//! готовых джоб (`run_due`), воркер-луп (tokio-interval, S1) через [`WorkerHooks`] (хуки к окружению —
//! backpressure-проба и нотификация UI — инъектируются вызывающим, движок их не знает). App-specific
//! glue (`WorkerSpawner` с `AppHandle`, `emit_jobs_changed`, `GcHandler`/`KIND_GC`, `default_registry`)
//! живёт в приложении (`apps/desktop/.../scheduler/mod.rs`), которое ре-экспортит этот движок.
//!
...
/// Хуки воркера к окружению вызывающего — вынесены, чтобы [`worker_loop`] был tauri-free и
/// тестировался без `AppHandle`: backpressure-проба (S5) и нотификация UI. App-glue (`WorkerSpawner`)
/// строит эти замыкания из `AppHandle`/`AppState` и зовёт [`worker_loop`] напрямую.
...
/// `pub` — app-glue `WorkerSpawner::start()` спавнит его, инъектируя [`WorkerHooks`] и shutdown-канал.
