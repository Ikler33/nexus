...
/// композиционный корень (агент-сервис). При ВЫКЛ актуаторе прод-сессия оставляет его ПУСТЫМ (B7);
/// наполняют его гейтнутые актуаторы (при ВКЛ) и read-only skills/web.
#[derive(Default)]
