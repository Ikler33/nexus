...
mod tests {
    ...
    async fn recall_drops_low_priority_layer_when_over_budget() {
        ...
        let tk = QwenTokenizer::embedded();
        // Бюджет = стоимость САМОГО ПРИОРИТЕТНОГО слоя (факты, последний) + запас на дрейф токенов
        // маркера, но ЗАВЕДОМО меньше двух слоёв. Тогда влезает ровно факты, а переписка/эпизоды
        // (менее приоритетные) выпадают. (recall пересоздаёт per-request маркер → меряем по `full`,
        // запас +16 токенов покрывает дрейф BPE на 24-символьном hex-маркере.)
        let facts_cost = ContextBudget::message_cost(&tk, full.last().unwrap());
        let episodes_cost = ContextBudget::message_cost(&tk, &full[1]);
        let budget = facts_cost + 16;
        assert!(
            budget < facts_cost + episodes_cost,
            "тест-инвариант: бюджет должен быть меньше двух слоёв (facts={facts_cost}, ep={episodes_cost})"
        );

        let tight = mem.recall(query, budget).await;
        assert_eq!(tight.len(), 1, "в обрезанный бюджет влез ровно один слой");
        assert!(
