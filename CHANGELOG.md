...
- **Слайс 1 — оболочка раздела + «Оформление» + «О программе».** Разрозненная панель «Оформление»
  ...
  - Дальше: **слайс 2 — «AI / Модели»** (chat+embedding URL/модель из UI вместо ручного `local.json` +
    «Проверить связь» + применение через пере-открытие vault) — закрывает боль «чат/связи не работают
    без конфига». Затем слайс 3 — переназначение горячих клавиш.
