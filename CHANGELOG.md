...
### LLM R1b (фронт): рендер размышлений reasoning-модели в чате + fidelity AI-панели

- Бэкенд (R1) уже слал `reasoning` (сырой CoT) и `reasoningSummary` (живая сводка), но фронт их
  **игнорировал** — TS-тип `ChatStreamEvent` отставал от Rust-enum. Теперь оба события доходят до UI:
  **живая «💭 …»-сводка** во время стрима (вместо безликого «думаю») и **сворачиваемый спойлер
  сырого chain-of-thought** над ответом. Сырой CoT троттлится тем же rAF, что и токены (AC-Б10-4);
  сводка патчится напрямую (редкое событие, ~1.5с). Raw CoT/сводка **не персистятся** (эфемерный
  live-индикатор, CoT бывает большим) — в истории остаётся только финальный ответ.
- Заодно подтянута fidelity AI-панели к дизайну «Hermes»: пустой экран (глиф + заголовок +
  suggestion-pills вместо голой строки), подвал композера (hint «↵ отправить» / живой пульс при
  стриме). Баннеры cloud/offline и анимированный brand-mark отложены (см. `docs/BACKLOG.md`).
- +тесты: стор (троттлинг CoT через rAF, не-персист), компонент (💭-сводка, спойлер, pill);
  мок `streamChat` эмитит reasoning. `test-all.sh` зелёный.
