...
- **ОТЛОЖЕНО в R-12b (скоуп/риск):**
  - **п.1 socket-диагностика ×2→1** — реальные копии клиентской AF_UNIX-диагностики (classify + `initialize`-handshake) живут в `nexus-cli/main.rs::cmd_status` И в src-tauri (`commands/settings.rs::probe_local_socket`/`classify_socket`, `agent_backend.rs::connect`). Настоящий дедуп требует правок **src-tauri вне п.4** (settings.rs + agent_backend.rs), что за скоупом «фронт не трогать кроме wire-rows п.4»; вдобавок user-facing строки трёх копий НЕ байт-идентичны (нужен hint-параметр → риск смены сообщений). CLI-only-вынос дубль НЕ снимает (реестр ×2 остался бы) → не делаем ради одного потребителя. Хэндофф: общий `classify_socket`/`probe_initialize` в `agent/connect/afunix.rs` (рядом с bind-side `prepare_socket_path`), hint-суффикс параметром, route CLI+settings+agent_backend.
  - **п.3 undo UndoOpts + типизированный домен корня** — SECURITY-смежно (undo восстанавливает данные). Схлопывание 3 публичных врапперов (`undo_run`/`undo_run_full`/`undo_run_with_driver`) в один `UndoOpts`-вход рикошетит в ЗАПРЕТНЫЕ зоны: `nexus-agentd/sandbox.rs` (agentd) и `src-tauri/commands/agent.rs`; внутренний дедуп уже есть (`undo_run_inner`). Типизированный домен с НОВЫМ ledger-полем = миграция схемы + обратная совместимость старых строк (чтение домена по `tool_name`-fallback) + тест на историческом фикстуре — отдельный срез. Хэндофф в R-12b с обязательным тестом обратной совместимости.
