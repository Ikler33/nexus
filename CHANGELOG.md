...
### Планировщик задач (ADR-007) — slice 4a: первый LLM-kind «Дайджест изменений» (#35)

- **Первая настоящая фоновая LLM-фича.** Kind **`digest`**: собирает заметки, изменённые за окно (сутки,
  ...
  понятная ошибка вместо тихого dead-letter). +2 офлайн-теста (FakeChat). **Дальше:** slice 4b — UI-панель
  (последний дайджест + «сгенерировать сейчас» + refetch по `jobs:changed`) для живого теста на реальных
  моделях; затем backpressure чата (S5: приоритет интерактивного чата над дайджест-джобой), дедуп
  одинаковых kind в очереди, on-change-триггер (S4). clippy/тесты/`test-all.sh` зелёные. Контракт:
  `docs/dev/scheduler.md`.
