...
- **Защита explicit-фактов (§4.3):** факт, введённый юзером ЯВНО (`source=explicit`), даже в «Авто» **никогда не переписывается/не супридится молча** — всегда показывается чип-предложение. (`targetSource` из плана.)
- **Обратимость:** авто-консолидация показывает toast «Объединил/Заменил: было → стало» с кнопкой **«Отменить»** (откат группы `memory_consolidate_undo`, optimistic-безопасно). Soft-supersede оставляет всё восстановимым.
- Под-тоггл «Предлагать ↔ Авто» в Настройках→AI (виден только при включённой консолидации). 6 тестов авто-режима (применение/защита-explicit/undo/degraded-to-add/propose-сохранён) — 796 фронт-зелёных.
