...
- **Сценарии (21 активный, 3 `test.skip` с TODO(P0-2)):** nav-смоук (все main-вью из ActivityBar + попарные переключения + SWITCH_MAIN/ST-D1 гасит слои), overlay-смоук (12 оверлеев/панелей: якорь + Esc-семантика кодифицирована КАК ЕСТЬ — Граф/Sync/Плагины закрываются только явной кнопкой), editor/preview (CM6 → LP-алиас → буквица `p[data-dropcap]` + race-регрессия EDFIX-4 + чистые ссылки), чат (sources → стрим → done), агент (план, дерево субагентов, changeset → «Подтвердить» → свёртка в итог), персист настройки «Чистые ссылки» через reload. Skip-сценарии (exec-пара, report-карточка, демо-ошибка чата) ждут мержа P0-2 (`test/p0-2-mock-parity`) — снять при ребейзе.
- **CI:** блокирующая джоба `e2e-web-smoke` в `ci.yml` (ubuntu, кэш браузеров по версии @playwright/test, `--with-deps webkit`, артефакт playwright-report+скриншоты на фейле).
- Детерминизм: web-first assertions, без sleep; локальный гейт ×2 подряд зелёный (~33 с прогон).

### Исправлено · Смоук поймал два живых P0-бага фронта (console-гейт + Esc-baseline)
