...
- **Единый источник статуса — стор `stores/news.ts`.** Этап живого прогона (`stage`) переехал из
  локального стейта `NewsView` в стор; событие `news:progress` теперь зовёт `onProgress` стора.
- **Ливнес «похоже, зависло» (мягко, не ошибка).** Чистая `evaluateRun` разводит «долго/живо» и
  «встало/умерло»: `running` без движения прогресса дольше `STUCK_MS` (120с) → мягкий баннер
  `news.stuck` (не убивает джобу). Отвечает ровно на жалобу «долгая обработка или сломалось?».
  Прежний вотчдог (`pending`>60с / `running`>20мин → `stalled`) сохранён как жёсткий бэкстоп.
- **Честная ошибка ЭТАПА при смерти прогона.** Джоба ушла в `dead` → баннер `news.diedAt` называет
  ЭТАП (последний виденный `news:progress`) и причину (`last_error` мёртвой джобы) вместо вечного
  «Собираю…». НЕ дублирует W-2-баннер `llmDown` (иной путь: там Ok-прогон с записью run) — уступает ему.
- **Ливнес работает и для ПЛАНОВОГО прогона:** вотчдог поднимается не только из «Обновить», но и из
  `load()`, если прогон уже идёт (гард `watchdogActive` против двойного цикла).
- **Мок-паритет (MEM-5).** Добавлена мок-шина событий (`bridge.ts::mockEmit`/`subscribe`): мок ленты
  ...
  браузер-превью, а не мёртвый код. Тесты: `evaluateRun` (7 кейсов), `onProgress`, рендер
  зависания/смерти/этапа-со-счётчиком в `NewsView.test.tsx`. i18n RU/EN: `news.stuck`, `news.diedAt`,
  `news.diedAtUnknown`, `news.diedReasonUnknown`, `news.stageLabel.*`.
