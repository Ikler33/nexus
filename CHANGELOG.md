...
- **Метрики:** главная — **DELETE-precision** (доля предложенных DELETE, которые реально контрадикция по правильной цели; ложный DELETE = ошибочное устаревание факта); + **UPDATE-quality** (UPDATE на верной цели И объединённый текст сохранил ключевые токены `mergeMustContain` — прокси «не теряет деталь») + op-accuracy. Пороги: DELETE-precision ≥ 0.9, UPDATE-quality ≥ 0.8, op-accuracy ≥ 0.7.
- **Golden-набор** `eval/consolidation_eval.json` — 32 кейса (по 8: контрадикция→DELETE, уточнение→UPDATE, парафраз→NOOP, новое→ADD).
- **Гейт тестируем БЕЗ LLM:** фиктивный «триггер-хэппи DELETE»-предиктор проваливает гейт (ловит опасный регресс), «всегда ADD» безопасен но валится на UPDATE-quality, идеальный проходит — 5 детерминированных тестов в CI.
- **Live-точка** `live_consolidation_meets_gate` (`#[ignore]`) — реальный `consolidate::decide` (основная модель, t=0). **Прогон на gemma-26B:** op-accuracy **0.844**, DELETE-precision **1.000** (0 ложных удалений), UPDATE-quality **1.000** — гейт пройден; модель консервативна (поймала 3/8 контрадикций — пропуск безопаснее ложного удаления). Это число доверия для разблокировки авто-режима (MEM-8c-b).
