...
### LLM R1b (фронт): живая сводка размышления в индикаторе «думает» + fidelity AI-панели

- Бэкенд (R1) слал `reasoningSummary` (живая сводка CoT) и `reasoning` (сырой CoT), но фронт их
  **игнорировал** — TS-тип `ChatStreamEvent` отставал от Rust-enum. Теперь фаза размышления — как в
  дизайне «Hermes»: **анимированный brand-mark** (созвездие-логотип `BrandThinking`: дышит, узлы/рёбра
  пульсируют) **+ переливающийся label**, и в этот label **стримится живая сводка** размышления
  (обновляется по ходу) вместо статичного «думаю». До первой сводки — дефолтная фраза. Сырой
  chain-of-thought сознательно **не рендерим** (решение владельца): событие принимаем и игнорируем.
  Сводка эфемерна — показывается только во время стрима, в историю не персистится.
- Заодно подтянута fidelity AI-панели: пустой экран (глиф + заголовок + suggestion-pills вместо
  голой строки), подвал композера (hint «↵ отправить» / живой пульс при стриме). Баннеры
  cloud/offline отложены (см. `docs/BACKLOG.md`).
- +тесты: стор (сводка патчится; raw `reasoning` игнорируется; сводка не персистится), компонент
  (brand-mark + стриминг сводки в label; дефолтная фраза до сводки). `test-all.sh` зелёный.
