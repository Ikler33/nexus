...
- **Графем-гард буквицы**: кандидат-абзац засчитывается только если тримленный текст НАЧИНАЕТСЯ с буквы/цифры (`^[\p{L}\p{Nd}]`); лид `←`/эмодзи пропускается — поиск идёт к следующему «обычному» абзацу. Раньше `← [[00 - Карта проекта]]` получал data-cap='0', а `::first-letter` раздувал стрелку в гигантскую оранжевую буквицу (Рескорринг.md).
...
- **Алиас схлопывается сразу после набора** (strict-края LP): раскрытие под курсором теперь СТРОГО внутри `[[…]]` (`<`/`>` вместо `<=`/`>=`) — закрыл `]]` → ссылка мгновенно схлопнулась в лейбл/алиас; набор внутри держит раскрытой; переходы внутрь — через atomic-ranges. Прежние inclusive-тесты осознанно обновлены.
- **Режим source/preview запоминается + пилюля не уезжает**: новый персист-преф `nexus.editor.noteMode` — последний выбранный режим становится дефолтом для новых панелей/следующего запуска (открытые панели ретроактивно не трогаются — их унаследованный режим фиксируется в `modes` до смены префа). Пилюля `.modeFloat` вынесена из скролл-контейнера в `.editorCol` (не скроллится с контентом; z-index под InlineAIBar).
- Тесты: vitest 1302 (+18): race-регрессия (красная до фикса), графем-гард (←/цифра/единственный-стрелочный), data-clean-links ON/OFF+реактивность, strict-края (== matchStart/== matchEnd/строго внутри/частичное выделение/шов смежных), персист+наследование префа режима, пилюля вне .scroll. Стенд (webkit, vite :1520): race 6/6; owner-notes Kniga/SA-Agent — буквица ✅, Рескорринг — без стрелки; скобки ON/OFF скриншот-парой; алиас `[[X|Слово]]` → «Слово» сразу после `]]` без ухода курсора; пилюля неподвижна при скролле 600px; режим наследуется после reload.
