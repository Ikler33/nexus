...
- **Граф: тюнинг физики и узлов** (отзыв владельца по сравнению с Obsidian). Замечания «всё в центре /
  мешанина», «резкие анимации», «мало градаций размера» → правка `graph-sim.ts` (под юнит-тестами):
  сильнее отталкивание (6800→16000), слабее гравитация-к-центру (0.012→0.004), длиннее пружины (96→150),
  медленнее остывание (0.94→0.95) — узлы разлетаются по площади; **кламп скорости** (±26/кадр) гасит
  резкие рывки; радиус узла — **sqrt-шкала 5..30** (вместо линейной 6..15) для чёткой градации хабов.
  Числа подбираются по визуальной проверке владельца. Отложено в BACKLOG: **пан/зум-камера + авто-fit**
  (как в Obsidian — для больших графов) и worker/ref-рендер при тормозах единого графа.
