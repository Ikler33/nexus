...
- **`sync` → оверлей-модуль** (`connector/modules/sync.ts`, эталон оверлей-модуля F-10b). `SyncPanel`
  ...
  - **ConflictResolver — ОСТАЁТСЯ ядром** (safe-flow git-merge, standalone из пилюли статусбара по
    `conflictOpen`, DP-14 + внутри SyncPanel). Живёт в зоне `components/sync` вместе с вырезанным
    SyncPanel, поэтому App.tsx честно импортирует его из sync-зоны — задокументировано в
    `MODULE_BOUNDARY_EXCEPTIONS` (только App.tsx, только sync-зона; прочие модули App-у закрыты).
    grep-инвариант держит `App ⇏ SyncPanel` (App тянет ТОЛЬКО ConflictResolver). Это единственная
    оговорка границы модулей в проекте — цена совместного проживания safe-flow-компонента в фича-зоне.
