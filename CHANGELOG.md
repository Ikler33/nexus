...
- Тесты: vitest 1337 (+15): гейты полноты ×3; exec/report/error сценарии агента (порядок в ленте, формы байт-в-байт через `Object.keys`) ×4; опции чата (порядок и формы hit'ов, дедуп эпизоды↔память, sessionId, дефолты, deep, rerank-порядок, center-буст, кламп k, pinned-фильтр, agentMemory, error/deniedKind) ×8. Полный гейт: typecheck 0 / eslint --max-warnings 0 / vitest 123 файла зелёные.
